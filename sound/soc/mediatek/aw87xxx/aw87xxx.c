...
#if IS_ENABLED(CONFIG_WB_AW87XX_ATA_SUPPORT) //Leo 20230311
#if IS_ENABLED(CONFIG_CM_MIDMISC_V_SUPPORT) //Leo 20230308
...
#define AW87XXX_DRIVER_VERSION	"v2.13.0"
#define AW87XXX_FW_BIN_NAME	"aw87xxx_acf.bin"
#define OCS72XXX_FW_BIN_NAME	"ocs72xxx_cf.bin"
#define AW87391_FW_BIN_NAME	    "aw87391_acf.bin"
/*************************************************************************
 * aw87xxx variable
 ************************************************************************/
...
#if IS_ENABLED(CONFIG_WB_AW87XX_ATA_SUPPORT) //Leo 20230311
int aw87xx_get_enbale_speaker(void);
static int index_speaker = 0xFF;
int aw87xx_pa_count = 0;
static int bootmode;
#endif
...
#if IS_ENABLED(CONFIG_WB_AW87XX_ATA_SUPPORT) //Leo 20230311
#if IS_ENABLED(CONFIG_CM_MIDMISC_V_SUPPORT) //Leo 20230308
	if ((bootmode == META_BOOT) || (bootmode == ADVMETA_BOOT)) { // add for meta boot no enable speaker 
		return 0;
	}
	
	if (!strcmp("Music", profile)) {
		if ((aw87xx_get_enbale_speaker() != 0xFF) 
				&& (aw87xx_get_enbale_speaker() != dev_index)) {
			return 0;
		}
	}
#endif
...
#endif
	AW_DEV_LOGI(aw87xxx->dev, "enter");
	if (aw87xxx->aw_dev.chipid == AW_DEV_CHIPID_11) {
		snprintf(aw87xxx->fw_name, AW87XXX_FW_NAME_MAX, "%s", OCS72XXX_FW_BIN_NAME);
		
		AW_DEV_LOGI(aw87xxx->dev, "ocs no reset gpio provided, hardware reset unavailable");
		aw87xxx->aw_dev.rst_gpio = AW_NO_RESET_GPIO;
		aw87xxx->aw_dev.hwen_status = AW_DEV_HWEN_INVALID;
#if defined(CONFIG_WB_AW87390_COMPAT_AW87391)
     } else if (aw87xxx->aw_dev.chipid == AW_DEV_CHIPID_C1) {//aw87391
		snprintf(aw87xxx->fw_name, AW87XXX_FW_NAME_MAX, "%s", AW87391_FW_BIN_NAME);
#endif
	} else {
			...
			const char *buf, size_t count)
{
	...
	if (ret < 0) {
		...
		vfree(data_buf);
		data_buf = NULL;
		mutex_unlock(&aw87xxx->reg_lock);  //Leo 20230302
		return -EFAULT;
	}
	mutex_unlock(&aw87xxx->reg_lock);
...
#if IS_ENABLED(CONFIG_WB_AW87XX_ATA_SUPPORT) //Leo 20230311
static void aw87xx_set_enable_speaker(int index)
{
	index_speaker =  index;
}
...
static ssize_t aw87xx_chipcount_show(struct device_driver *ddri, char *buf)
{
	ssize_t _tLength = 0;

	_tLength = snprintf(buf, PAGE_SIZE, "%d",aw87xx_pa_count);

	return _tLength;
}

static ssize_t aw87xx_ata_profile_show(struct device_driver *ddri, char *buf)
{
	ssize_t _tLength = 0;

	_tLength = snprintf(buf, PAGE_SIZE, "enable_index:%d \r\n",aw87xx_get_enbale_speaker());

	return _tLength;
}

static ssize_t aw87xx_ata_profile_store(struct device_driver *ddri,
	const char *buf, size_t tCount)
{
	int enable_index = 0, ret = 0;

	ret = kstrtoint(buf, 10, &enable_index);
	
	if (((enable_index >= 0) && (enable_index < 4)) || (enable_index == 0xff)) {
		aw87xx_set_enable_speaker(enable_index);
	}

	return tCount;
}

static DRIVER_ATTR_RO(aw87xx_chipcount);
static DRIVER_ATTR_RW(aw87xx_ata_profile);

static struct driver_attribute *aw87xx_ata_attr_list[] = {
	&driver_attr_aw87xx_chipcount,	/*chip information */
	&driver_attr_aw87xx_ata_profile,	/*chip information */
};

static int aw87xx_ata_create_attr(struct device_driver *driver)
{
	int idx, err = 0;
	int num = (int)(ARRAY_SIZE(aw87xx_ata_attr_list));

	if (driver == NULL)
		return -EINVAL;

	for (idx = 0; idx < num; idx++) {
		err = driver_create_file(driver, aw87xx_ata_attr_list[idx]);
		if (err != 0) {
			pr_info("driver_create_file (%s) = %d\n",
				aw87xx_ata_attr_list[idx]->attr.name, err);
			break;
		}
	}
	return err;
...
#if IS_ENABLED(CONFIG_CM_MIDMISC_V_SUPPORT) //Leo 20230308
	if (is_init == false) {
		bootmode = cust_mid_misc_v_get_boot_mode();
		ret = aw87xx_ata_create_attr(client->dev.driver);
		if (ret) {
			pr_err("aw87xx_ata create attribute err = %d\n", ret);
		}
