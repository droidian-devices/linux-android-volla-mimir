...
/*end*/
struct gpio_desc *charger_control_gpio;
...
void ext_charger_ic_function_control(bool enable){
    if(IS_ERR(charger_control_gpio)){
        return;
    }

    return;
    if(enable){
        gpiod_set_value(charger_control_gpio, 0);
    }else{
        gpiod_set_value(charger_control_gpio, 1);
    };
}
EXPORT_SYMBOL(ext_charger_ic_function_control);
/************************************************************************/
...
static int sc8551_parse_dt(struct sc8551 *sc, struct device *dev)
{
    ...
    charger_control_gpio = devm_gpiod_get_optional(dev, "charger-control", GPIOD_OUT_LOW);
    if (IS_ERR(charger_control_gpio)){
        sc_err("Could not get named GPIO for CHARGER GPIO!\n");
    }else{
        gpiod_set_value(charger_control_gpio, 0);
    }
...
static const struct charger_ops sc8551_chg_ops = {
	.enable_direct_charging = sc8551_enable_chg,
	.is_enabled = sc8551_is_chg_enabled,
	.get_adc = sc8551_get_adc,
	.set_direct_charging_vbusov = sc8551_set_vbusovp,
	.set_direct_charging_ibusoc = sc8551_set_ibusocp,
                ...
                union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_CURRENT_MAX:
        //if (sc->chg_type == POWER_SUPPLY_TYPE_USB)
            val->intval = 3750000;//500*1000
        break;
    case POWER_SUPPLY_PROP_VOLTAGE_MAX:
        //if (sc->chg_type == POWER_SUPPLY_TYPE_USB)
            val->intval = 12000000;//12000*1000
        break;
    default:
        return -EINVAL;
