...
static const struct nvt_ts_trim_id_table trim_id_table[] = {
	{.id = {0x20, 0xFF, 0xFF, 0x23, 0x65, 0x03}, .mask = {1, 0, 0, 1, 1, 1},
		.mmap = &NT36523_memory_map,  .carrier_system = 0},
	...
	{.id = {0x0C, 0xFF, 0xFF, 0x72, 0x66, 0x03}, .mask = {1, 0, 0, 1, 1, 1},
		.mmap = &NT36675_memory_map,  .carrier_system = 0},
	{.id = {0xFF, 0xFF, 0xFF, 0x26, 0x65, 0x03}, .mask = {0, 0, 0, 1, 1, 1},
		.mmap = &NT36526_memory_map,  .carrier_system = 0},
...
#if 0//def CONFIG_TOUCHSCREEN_MTK_NT36xxx_X_REVERSE
				input_report_abs(ts->input_dev,
					ABS_MT_POSITION_X, (ts->abs_x_max-input_x));//ts->abs_x_max-
#else
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20230302
			if (csci_exist("touchpanel.nt36xxx.x.reverse")) {
				temp = csci_integer("touchpanel.nt36xxx.x.reverse",0);
				if (temp == 1) {
				...
				input_report_abs(ts->input_dev,
					ABS_MT_POSITION_X, (input_x));//ts->abs_x_max-
			}
#else
				input_report_abs(ts->input_dev,
					ABS_MT_POSITION_X, (input_x));//ts->abs_x_max-
#endif
#endif
#if 0//defCONFIG_TOUCHSCREEN_MTK_NT36xxx_Y_REVERSE
				input_report_abs(ts->input_dev,
					ABS_MT_POSITION_Y, (ts->abs_y_max-input_y));//ts->abs_y_max-
#else
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20231027
			if (csci_exist("touchpanel.nt36xxx.y.reverse")) {
				temp = csci_integer("touchpanel.nt36xxx.y.reverse",0);
				if (temp == 1) {
...
#else
				input_report_abs(ts->input_dev,
					ABS_MT_POSITION_Y, (input_y));//ts->abs_y_max-
#endif
#endif
...
static int8_t nvt_ts_check_chip_ver_trim(uint32_t chip_ver_trim_addr)
{
	...
	for (retry = 5; retry > 0; retry--) {
		...
		if ((buf[1] == 0xFC) ||
			((buf[1] == 0xFF) && (buf[2] == 0xFF) && (buf[3] == 0xFF))) {
			NVT_LOG("LQ >>>>  nvt_stop_crc_reboot\n");
			nvt_stop_crc_reboot();
			continue;
		}
...
static void hwinfo_update_tpinfo(void)
{
	int ret;
	unsigned char buf[3];
...

	{
		...
		#define HW_TYPE_TP    2
		sprintf(name,"%s ver:0x%.2x",NVT_TS_NAME, ver);
...
#if defined(M100TB_DG_P3PRO_527) //Leo 20231027
#if IS_ENABLED(CONFIG_CM_MIDMISC_SUPPORT) 
	const char *tp_name = "None";
	cust_midmisc_get_use_tp_name_from_lk((const char **)&tp_name);
	if (strcmp(tp_name, "nt36xxx_i2c")) {
		pr_err("%s no nt36xxx_i2c , skip probe !\n");
		return 0;
	}
#endif
#endif
