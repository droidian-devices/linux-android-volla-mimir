        ...
        u8 *buf, size_t len)
{
    ...
    // dump_spi("i2c r >> : ", cts_dev->pdata->i2c_fifo_buf, txlen);
    ret = cts_plat_i2c_read(cts_dev->pdata, CTS_DEV_NORMAL_MODE_I2CADDR,
        ...
        3, 10);
    // dump_spi("i2c r << : ", cts_dev->pdata->i2c_rbuf, size);
    if (ret == 0) {
        ...
        u16 cmd, u8 *wbuf, size_t wlen)
{
    ...
    // dump_spi("i2c w >> : ", cts_dev->pdata->i2c_fifo_buf, txlen);
        ...
        u16 cmd, u8 *buf, size_t len)
{
    ...
    if (ret == 0) {
        ...
        memcpy(buf, cts_dev->pdata->i2c_rbuf, len - TCS_TAIL_SIZE);
    }
    return ret;
}
#else
        ...
        u16 cmd, u8 *rdata, size_t rdatalen)
{
    ...
    ret = cts_tcs_spi_xtrans(cts_dev, cts_dev->pdata->spi_tx_buf, txlen,
            cts_dev->pdata->spi_rx_buf, rdatalen + TCS_TAIL_SIZE);
    // dump_spi("<< ", cts_dev->pdata->spi_rx_buf, rdatalen + TCS_TAIL_SIZE);
    if (ret) {
        return ret;
    }
        ...
        u16 cmd, u8 *data, size_t wlen)
{
    ...
    // dump_spi(">> ", cts_dev->pdata->spi_tx_buf, txlen);
    ret = cts_tcs_spi_xtrans(cts_dev, cts_dev->pdata->spi_tx_buf, txlen,
            cts_dev->pdata->spi_rx_buf, TCS_TAIL_SIZE);
    // dump_spi(">> ", cts_dev->pdata->spi_rx_buf, TCS_TAIL_SIZE);
    return ret;
}
#endif
        ...
        u8 *rx, size_t rxlen)
{
    ...
    if (txlen > 6) {
        ret = cts_plat_i2c_write(cts_dev->pdata, CTS_DEV_NORMAL_MODE_I2CADDR,
                cts_dev->pdata->i2c_fifo_buf, txlen, 3, 10);
    } else if (txlen == 6) {
        ret = cts_plat_i2c_read(cts_dev->pdata, CTS_DEV_NORMAL_MODE_I2CADDR,
