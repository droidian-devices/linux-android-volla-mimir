...
static int cts_disp_notifier_callback(struct notifier_block *nb,unsigned long value, void *v);
#endif
...
#if IS_ENABLED(CONFIG_DRM_MEDIATEK)  //jnier add 20231107
static int cts_disp_notifier_callback(struct notifier_block *nb,
	unsigned long value, void *v)
{
	int *data = (int *)v;
	const struct cts_platform_data *pdata = container_of(nb, struct cts_platform_data, disp_notifier);
	struct chipone_ts_data *cts_data = container_of(pdata->cts_dev, struct chipone_ts_data, cts_dev);

	if (pdata && v) {
		cts_info("%s IN", __func__);
		...
		cts_info("%s OUT", __func__);
	} else {
		cts_info("CTS touch IC can not suspend or resume");
		return -1;
	}
...
#endif
{
    ...
    int ret = 0;
#ifndef CONFIG_CTS_I2C_HOST  //jnier add 20231017
	int cts_spi_support = 0;
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT)
	if (csci_exist("touchpanel.cts.spi")) {
		if (csci_integer("touchpanel.cts.spi", 0) > 0) {
			cts_spi_support = csci_integer("touchpanel.cts.spi", 0);
		}
	}
#endif
	cts_info("%s start\n",__func__);
	if(cts_spi_support == 0){
		cts_err("do not support touchpanel cts spi %s\n", __func__);
		return -EINVAL;
	}
#endif
...
#endif
//	if (client->addr != 0x48) {
//		cts_info("Force client->addr 0xx%02x to 0x48",client->addr);
//		client->addr = 0x48;
//	}
#ifdef CONFIG_CTS_I2C_HOST
    if (client == NULL) {
...
#endif
#if IS_ENABLED(CONFIG_DRM_MEDIATEK)
	cts_data->pdata->disp_notifier.notifier_call = cts_disp_notifier_callback; 
	ret = mtk_disp_notifier_register("CTS Touch", &cts_data->pdata->disp_notifier);
	if (ret) {
		cts_err("CTS Failed to register disp notifier client:%d", ret);
...
cts_info("-------------------this03------------------");
#if IS_ENABLED(CONFIG_CM_HARDWAREINFO_SUPPORT)
        {
                ...
                #define HW_TYPE_TP    2
#ifdef CONFIG_CTS_I2C_HOST
                sprintf(name,"%s","chipone_tddi_i2c");
#else
                sprintf(name,"%s","chipone_tddi_spi");
#endif
...
err_register_disp_notif_failed:
	if (mtk_disp_notifier_unregister(&cts_data->pdata->disp_notifier))
		cts_err("Error occurred while unregistering disp_notifier.\n");
#endif
#endif
