        ...
        const char *filepath, bool to_flash)
{
    ...
    ret = cts_update_firmware(cts_dev, to_flash);
    if (ret) {
...
#ifdef CFG_CTS_DRIVER_BUILTIN_FIRMWARE
    ret = cts_request_newer_driver_builtin_firmware(cts_dev,
        hwid, fwid, curr_firmware_ver);
    if (ret) {
...
#ifdef CFG_CTS_FIRMWARE_IN_FS
    ret = cts_request_newer_firmware_from_fs(cts_dev,
...
int cts_i2c_enter_program_mode(struct cts_device *cts_dev)
{
    int ret;
    u8 val[] = { 0x5A };

    ret = cts_i2c_writesb(cts_dev, 0xCC3355, val, sizeof(val), 10, 1);
    if (ret) {
        cts_err("%s failed: ret = %d", __func__, ret);
        return -1;
    }
    return ret;
}

int cts_master_c35a_prog(struct cts_device *cts_dev)
{
    int ret;

    ret = cts_i2c_enter_program_mode(cts_dev);
    if (ret) {
        cts_err("%s failed: ret = %d", __func__, ret);
        return -1;
    }
...
int cts_host_reset(struct cts_device *cts_dev)
{
    int ret;
    int retries = 10;
    u8 val[] = { 0x7D };

    ret = cts_i2c_writesb(cts_dev, 0x70008, val, sizeof(val), retries, 1);
    if (ret) {
        cts_err("%s failed: ret = %d", __func__, ret);
        return -1;
    }
...
int cts_host_prgmwr_mst_spi2_master_cfg_2(struct cts_device *cts_dev)
{
    int ret;
    int retries = 10;
    u8 val[] = { 0x33 };

    ret = cts_i2c_writesb(cts_dev, 0x784C8, val, sizeof(val), retries, 1);
    if (ret) {
        cts_err("%s failed: ret = %d", __func__, ret);
        return -1;
    }
...
int cts_mst_sing_c35a_switch_slv_to_prgm_3a(struct cts_device *cts_dev)
{
    int ret;
    int retries = 10;
    ...
    u8 val01[] = { 0x00 };
    u8 val02[] = { 0x01 };
    u8 val03[] = { 0xCC, 0x33, 0x55, 0x5A };
    u8 val04[] = { 0x04, 0x00, 0x00, 0x00 };
    u8 val05[] = { 0x03 };
    u8 val06[] = { 0xFF };

    ret = cts_i2c_writesb(cts_dev, 0x7843C, val01, sizeof(val01), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x7841D, val02, sizeof(val02), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x7840C, val03, sizeof(val03), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x784D4, val04, sizeof(val04), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x784C0, val05, sizeof(val05), retries, 1);
    mdelay(5);
    ret = cts_i2c_readb(cts_dev, 0x78404, val06, 1, 1);
    if (val06[0] == 0x00) {
        return 0;
    }
...
int cts_mst_sing_prgmwr_slv_spi2_master_cfg_3(struct cts_device *cts_dev)
{
    int ret;
    int retries = 10;
    ...
    u8 val01[] = { 0x00 };
    u8 val02[] = { 0x01 };
    u8 val03[] = { 0x60, 0x07, 0x84, 0xC8 };
    u8 val04[] = { 0x22 };
    u8 val05[] = { 0x05, 0x00, 0x00, 0x00 };
    u8 val06[] = { 0x03 };
    u8 val07[] = { 0xFF };
    ...
    ret = cts_i2c_writesb(cts_dev, 0x7841D, val02, sizeof(val02), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x7840C, val03, sizeof(val03), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x7840C, val04, sizeof(val04), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x784D4, val05, sizeof(val05), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x784C0, val06, sizeof(val06), retries, 1);
    mdelay(5);
    ret = cts_i2c_readb(cts_dev, 0x78404, val07, 1, 1);
    if (val07[0] == 0x00) {
        return 0;
    }
...
int cts_mst_quad_prgmwr_copy_codedata_to_slv_4(struct cts_device *cts_dev, size_t code_len)
{
    int ret;
    int retries = 10;
    ...
    ret = cts_i2c_writesb(cts_dev, 0x784C9, val01, sizeof(val01), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x7843C, val02, sizeof(val02), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x7841D, val03, sizeof(val03), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x78440, val04, sizeof(val04), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x78444, val05, sizeof(val05), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x784D0, val06, sizeof(val06), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x784D4, val07, sizeof(val07), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x784C0, val08, sizeof(val08), retries, 1);
    mdelay(25);
    ret = cts_i2c_readb(cts_dev, 0x78404, val09, 1, 1);
    if (val09[0] == 0x00) {
        return 0;
    }
    return ret;
...
int cts_mst_quad_prgmwr_copy_crcdata_to_slv_4(struct cts_device *cts_dev)
{
    int ret;
    int retries = 10;
    ...
    u8 val02[] = { 0x03 };
    u8 val03[] = { 0x01 };
    u8 val04[] = { 0xf0, 0x7f, 0x03, 0x00 };
    u8 val05[] = { 0x10, 0x00, 0x00, 0x00 };

    u8 val06[] = { 0xf0, 0x7f, 0x03, 0x00 };
    u8 val07[] = { 0x10, 0x00, 0x00, 0x00 };
    u8 val08[] = { 0x61 };
    u8 val09[] = { 0xFF };
...
int cts_read_slave_boot_mode(struct cts_device *cts_dev)
{
    int ret;
    int retries = 10;
    ...
    u8 val01[] = { 0x00 };
    u8 val02[] = { 0x01 };
    u8 val03[] = { 0x10, 0x00, 0x07, 0x00 };
    u8 val04[] = { 0x04, 0x00, 0x00, 0x00 };
    u8 val05[] = { 0x62 };
    u8 val06[] = { 0xFF };
    u32 val07[] = { 0xFFFFFFFF };


    ret = cts_i2c_writesb(cts_dev, 0x7843C, val01, sizeof(val01), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x7841D, val02, sizeof(val02), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x784D0, val03, sizeof(val03), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x784D4, val04, sizeof(val04), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x784C0, val05, sizeof(val05), retries, 1);
    mdelay(5);
    ret = cts_i2c_readb(cts_dev, 0x78404, val06, 1, 1);
    mdelay(5);
    ret = cts_i2c_readl(cts_dev, 0x78408, val07, 1, 1);
    if (val07[0] == 0x00) {
        cts_err("slave boot mode = %#010x", val07);
        return 0;
    }
    return ret;
}

int cts_write_slave_boot_mode(struct cts_device *cts_dev, u8 boot_mode)
{
    int ret;
    int retries = 10;
    ...
    u8 val02[] = { 0x01 };
    u8 val03[] = { 0x03 };
    u8 val04[] = { 0x10, 0x00, 0x07, 0x00 };
    u8 val05[] = { 0x01, 0x00, 0x00, 0x00 };
    u8 val06[] = { 0x61 };
    u8 val07[] = { 0xFF };
    ...
    ret = cts_i2c_writesb(cts_dev, 0x784D0, val04, sizeof(val04), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x784D4, val05, sizeof(val05), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x784C0, val06, sizeof(val06), retries, 1);
    mdelay(5);
    ret = cts_i2c_readb(cts_dev, 0x78404, val07, 3, 1);
    if (val07[0] == 0x00) {
        return 0;
    }
    return ret;
...
int cts_host_prgmwr_mst_spi2_reg_postprocess_6(struct cts_device *cts_dev)
{
    int ret;
    int retries = 10;
    u8 val01[] = { 0x03 };
    ...
    ret = cts_i2c_writesb(cts_dev, 0x7843C, val01, sizeof(val01), retries, 1);
    ret = cts_i2c_writesb(cts_dev, 0x78470, val02, sizeof(val02), retries, 1);
        ...
        struct cts_firmware_sect_info firmware_info)
{
    ...
    ret = cts_master_c35a_prog(cts_dev);
    if (ret) {
        cts_err("cts_master_c35a_prog failed");
        return ret;
    }

    ret = cts_host_reset(cts_dev);
    if (ret) {
        cts_err("cts_host_reset failed");
        return ret;
    }
    ...
    ret = cts_host_prgmwr_mst_spi2_master_cfg_2(cts_dev);
    if (ret) {
        cts_err("cts_host_prgmwr_mst_spi2_master_cfg_2 failed");
        return ret;
    }

    ret = cts_mst_sing_c35a_switch_slv_to_prgm_3a(cts_dev);
    if (ret) {
        cts_err("cts_mst_sing_c35a_switch_slv_to_prgm_3a failed");
        return ret;
    }

    ret = cts_mst_quad_prgmwr_copy_codedata_to_slv_4(cts_dev,
        firmware_info.firmware_sect_size);
    if (ret) {
        cts_err("cts_mst_quad_prgmwr_copy_codedata_to_slv_4 failed");
        return ret;
    }

    ret = cts_mst_quad_prgmwr_copy_crcdata_to_slv_4(cts_dev);
    if (ret) {
        cts_err("cts_mst_quad_prgmwr_copy_crcdata_to_slv_4 failed");
        return ret;
    }
...
#else /* CFG_CTS_CASCADE_SUPPORTED */
    ret = cts_tcs_set_krang_stop(cts_dev);
    if (ret < 0) {
        cts_warn("Set krang stop failed");
    }
...
int cts_update_firmware(struct cts_device *cts_dev, bool to_flash)
{
    struct cts_firmware_sect_info firmware_info;
...
out:
    ...
    if (ret == 0) {
#ifdef CONFIG_CTS_I2C_HOST
        ret = cts_read_slave_boot_mode(cts_dev);
        if (ret) {
            cts_err("cts_read_slave_boot_mode failed");
            return ret;
        }

        ret = cts_write_slave_boot_mode(cts_dev, CTS_DEV_BOOT_MODE_SRAM);
        if (ret) {
            cts_err("cts_write_slave_boot_mode failed");
            return ret;
        }

        ret = cts_host_prgmwr_mst_spi2_reg_postprocess_6(cts_dev);
        if (ret) {
            cts_err("cts_host_prgmwr_mst_spi2_reg_postprocess_6 failed");
            return ret;
        }
        ...
        if (firmware_info.firmware_sect_size <=
            ...
            ret = cts_enter_normal_mode(cts_dev);// 38 + 58 = 96ms
            if (ret) {
