...
void tpd_debug_set_time(void)
{
	...
	tpd_last_2_int_time[1] = (t.tv_sec & 0xFFF) * 1000000 + (long)t.tv_nsec/1000;
#else
	do_gettimeofday(&t);
	tpd_last_2_int_time[0] = tpd_last_2_int_time[1];
			...
			int cal_x, int cal_y, int p, int down)
{
	...
	if ((tpd_down_status == 0 && down == 1) ||
		...
		trace_MET_touch("EV_KEY",
				tsec, tusec, "BTN_TOUCH", down);
		tpd_down_status = !tpd_down_status;
	}
	if (tpd_down_status) {
		trace_MET_touch("EV_ABS", tsec, tusec, "X", raw_x);
		trace_MET_touch("EV_ABS", tsec, tusec, "Y", raw_y);
	}
}
