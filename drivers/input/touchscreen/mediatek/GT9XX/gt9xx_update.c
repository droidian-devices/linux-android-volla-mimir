...
static u8 gup_check_update_file(struct i2c_client *client, struct st_fw_head *fw_head, u8 *path)
{
		...
		if (path) {
			...
			if (gtxx_get_tp_is_vaild()) {
				p_firmware_info = gtxx_tp_info[tp_index].pro_tp_data[pro_index].p_fw;
				tp_source_len = gtxx_tp_info[tp_index].pro_tp_data[pro_index].tp_source_len;
			} else {
				return -1;
			}
			
			update_msg.fw_total_len = tp_source_len - FW_HEAD_LENGTH;
			if (tp_source_len <
			...
			memcpy(fw_head, p_firmware_info, FW_HEAD_LENGTH);

			/* check firmware legality */
			fw_checksum = 0;
			for (i = 0; i < update_msg.fw_total_len; i += 2)
					fw_checksum += (p_firmware_info[FW_HEAD_LENGTH + i] << 8)
					+ p_firmware_info[FW_HEAD_LENGTH + i + 1];
		#else /*TP_COMPATIBLE*/
			GTP_INFO("Update by default firmware array");
...
static u8 gup_load_section_file(u8 *buf, u32 offset, u16 length, u8 set_or_end)
{
	...
	if (HEADER_FW_READY == got_file_flag) {
		if (SEEK_SET == set_or_end)
			memcpy(buf, &p_firmware_info[FW_HEAD_LENGTH + offset], length);
		else		/* seek end */
			memcpy(buf, &p_firmware_info[update_msg.fw_total_len + FW_HEAD_LENGTH - offset], length);
