...
#if defined(CONFIG_WB_CHARGER_DET_NOTIFIER_SUPPORT) //Leo 20220117
#include <linux/notifier.h>
#endif

#define GPIO_POWER_SUPPORT //Leo 20240717

#if defined(TP_COMPATIBLE) //Leo 20211130
#include  "include/gtxx_config.h"
extern ALL_TP_INFO_GT9XX gtxx_tp_info[];
#endif
...
#if IS_ENABLED(CONFIG_WB_CHARGER_DET_NOTIFIER_SUPPORT) //Leo 20221125
#if IS_ENABLED(CONFIG_GTP_CHARGER_DET_NOTIFIER)
#define CHARGER_IN_CHAIN      0x10U
#define CHARGER_OUT_CHAIN     0x11U
static bool is_chr_in = false;
extern int register_charger_det_notifier(struct notifier_block *nb);
static bool gt9xx_get_charger_status(void);
static void charger_is_pluged_in(void);
static void charger_is_pluged_out(void);
#endif
#endif
...
static unsigned int tpd_power_gpio;
#endif
...
static int of_get_gt9xx_platform_data(struct device *dev)
{
	...
	if (dev->of_node) {
		...
		GTP_ERROR("tpd_rst_gpio %d\n", tpd_rst_gpio);
		GTP_ERROR("tpd_int_gpio %d\n", tpd_int_gpio);

		tpd_power_gpio = of_get_named_gpio(dev->of_node, "power-gpio", 0);
		if (!gpio_is_valid(tpd_power_gpio)) {
			GTP_ERROR("No valid power gpio");
		} else {
			ret = gpio_request_one(tpd_power_gpio, GPIOF_OUT_INIT_LOW,
						 "touchtp_power_gpio");
			if (ret < 0) {
				GTP_ERROR("Unable to request gpio reset_pin\n");
				return -1;
			}
			gpio_direction_output(tpd_power_gpio, 1);
		}
	}
#endif
	return 0;
...
int i2c_write_bytes_non_dma(struct i2c_client *client, u16 addr, u8 *txbuf, int len)
{
		...
		while (left > 0) {
			...
			while (i2c_transfer(client->adapter, &msg, 1) != 1) {
				...
				if (retry == 5) {
					GTP_ERROR("I2C write client->addr:0x%x 0x%X%X length=%d failed\n",client->addr, buffer[0], buffer[1], len);
					return -1;
				}
			}
		}
...
s32 gtp_init_panel(struct i2c_client *client)
{
	...
	#if defined(TP_COMPATIBLE) //Leo 20211130
	u8 *p_cfg_info_group0;
	int tp_index = gtxx_get_tp_info_index();
	int pro_index = gtxx_get_project_data_index();
	#endif
	...
	#if defined(TP_COMPATIBLE) //Leo 20211130
	if (gtxx_get_tp_is_vaild()) {
		p_cfg_info_group0 = gtxx_tp_info[tp_index].pro_tp_data[pro_index].p_gtxx_config_data_id;
		send_cfg_buf[0]=p_cfg_info_group0;
		cfg_info_len[0]=gtxx_tp_info[tp_index].pro_tp_data[pro_index].tp_data_id_len;
	}
	#endif
...
#if defined(GPIO_POWER_SUPPORT) //Leo 20240717
	gpio_direction_output(tpd_power_gpio, true);
#else
...
static const struct proc_ops gt_upgrade_proc_fops = {
	.proc_write = gt91xx_config_write_proc, .proc_read = gt91xx_config_read_proc};
static int tpd_irq_registration(void)
{
	...
	#if defined(CONFIG_GTP_USE_PINCTRL) //Leo 20211223
	node = of_find_matching_node(NULL, touch_of_match);
	#else
	node = of_find_compatible_node(NULL, NULL, "mediatek,cap_touch");//Leo MT8168 should use this 20211223
	#endif
...
//			irq_enabled = false;
	...
	return ret;
		...
		&& IS_ENABLED(CONFIG_WB_TP_FW_SHOW_SUPPORT))
extern char tpd_fw_ver[16];

static void wb_gt9xx_set_fw_ver(struct i2c_client *client, u16 version_info)
{
	int ret;
	char cfg_version = 0x00;

	ret = gtp_i2c_read_dbl_check(client, GTP_REG_CONFIG_DATA, &cfg_version, 1);
	if (ret == SUCCESS) {
		sprintf(tpd_fw_ver,"0x%X cfg:0x%X",version_info,cfg_version);
	} else {
		sprintf(tpd_fw_ver,"0x%X ",version_info);
	}
}
#endif
...
{
	int ret;
	unsigned char chr_cmd[3] = {0x80, 0x40, 0x06};
	...
	if (ret > 0) {
		GTP_INFO("gt9xx Update status for Charger Plugin\n");
	}
...
{
	int ret;
	unsigned char chr_cmd[3] = {0x80, 0x40, 0x07};
	...
	ret = gtp_i2c_write(i2c_client_point, chr_cmd, 3); 
	if (ret > 0) {
		GTP_INFO("gt9xx Update status for Charger Plugout\n");
	}

	is_chr_in = false;
}

int charger_detect_event(struct notifier_block *nb, unsigned long event,
	void *v)
{
	switch(event){
		case CHARGER_IN_CHAIN:
			charger_is_pluged_in();
			break;
		case CHARGER_OUT_CHAIN:
			charger_is_pluged_out();
			break;
		default:
			break;
	}
...
static struct notifier_block charger_detect_notifier = {
	.notifier_call = charger_detect_event,
};
#endif
#endif
...
//161012
	...
	ret = gtp_read_version(client, &version_info);
	if (ret < 0) {
		GTP_ERROR("Read version again failed.");
		//goto out;
	}

	wb_gt9xx_set_fw_ver(client, version_info);
#endif


#if IS_ENABLED(CONFIG_WB_CHARGER_DET_NOTIFIER_SUPPORT) //Leo 20221125
#if IS_ENABLED(CONFIG_GTP_CHARGER_DET_NOTIFIER)
	register_charger_det_notifier(&charger_detect_notifier);
#endif
#endif
...
void force_reset_guitar(void)
{
	...
	/* Power off TP */
#if defined(GPIO_POWER_SUPPORT) //Leo 20240717
	gpio_direction_output(tpd_power_gpio, false);
#else
	...
	/* Power on TP */
#if defined(GPIO_POWER_SUPPORT) //Leo 20240717
	ret = gpio_direction_output(tpd_power_gpio, true);
#else
	ret = regulator_enable(tpd->reg);
	if (ret != 0)
		TPD_DMESG("Failed to enable reg-vgp6: %d\n", ret);
...
static void tpd_down(s32 x, s32 y, s32 size, s32 id)
{
	...
	#if defined(TP_COMPATIBLE) //Leo 20211130
	gtxx_control_input_orientation(&x, &y, DISP_GetScreenWidth(), DISP_GetScreenHeight());
	#endif
...
//				}
		...
		if (touch_num) {
			for (i = 0; i < touch_num; i++) {
				...
				//printk("Leo  %d)(%d, %d)[%d]", id, input_x, input_y, input_w);
...
#ifdef TP_COMPATIBLE //Leo 20211130
	gtxx_config_tp(); 
	gtxx_tp_list_config(); 
#endif	
...
#if defined(GPIO_POWER_SUPPORT) //Leo 20240717
	ret_p = gpio_direction_output(tpd_power_gpio, false);
#else
...
#if defined(CONFIG_WB_CHARGER_DET_NOTIFIER_SUPPORT) //Leo 20221125
#if IS_ENABLED(CONFIG_GTP_CHARGER_DET_NOTIFIER)
	if (gt9xx_get_charger_status() == true) {
		charger_is_pluged_in();
	} else {
		charger_is_pluged_out();
	}
...
#if defined(GPIO_POWER_SUPPORT) //Leo 20240717
	ret = gpio_direction_output(tpd_power_gpio, false);
#else
	ret = regulator_disable(tpd->reg);
	if (ret != 0)
		TPD_DMESG("Failed to disable reg-vgp6: %d\n", ret);
...
int tpd_driver_init_GT9XX(void)
{
...
void tpd_driver_exit_GT9XX(void)
{
