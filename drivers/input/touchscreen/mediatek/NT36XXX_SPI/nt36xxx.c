...
#if IS_ENABLED(CONFIG_CM_MIDMISC_SUPPORT) //Leo 20230308
...
struct pinctrl_state *spi_pull_down;
struct pinctrl_state *spi_pull_up;
...
info_retry:
	...
	if ((buf[1] + buf[2]) != 0xFF) {
		...
		if(retry_count < 3) {
			...
			ts->fw_ver = 0;
			ts->abs_x_max = DISP_GetScreenWidth();//TOUCH_DEFAULT_MAX_WIDTH;
			ts->abs_y_max = DISP_GetScreenHeight();//TOUCH_DEFAULT_MAX_HEIGHT;
...
static int32_t nvt_parse_dt(struct device *dev)
{
    ...
    if (IS_ERR(spi_pinctrl)) {
        ...
        NVT_LOG("Cannot find spi_pinctrl!\n");
        //return ret;
        pinctrl_ok = false;
    } else {
        spi_pull_down = pinctrl_lookup_state(spi_pinctrl, "spi_pull_down");
        if (IS_ERR(spi_pull_down)) {
            ret = PTR_ERR(spi_pull_down);
            NVT_LOG("Cannot find pinctrl spi_pull_down %d!\n", ret);
        }

        spi_pull_up = pinctrl_lookup_state(spi_pinctrl, "spi_pull_up");
        if (IS_ERR(spi_pull_up)) {
            ret = PTR_ERR(spi_pull_up);
            NVT_LOG("Cannot find pinctrl spi_pull_up %d!\n", ret);
        }
        pinctrl_ok = true;
    }
...
#ifdef CONFIG_TOUCHSCREEN_NT36xxx_SPI_X_REVERSE
...
#endif
#endif
#ifdef CONFIG_TOUCHSCREEN_NT36xxx_SPI_Y_REVERSE
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	if (csci_exist("touchpanel.nvt.spi")) {
		if (csci_integer("touchpanel.nvt.spi", 0) > 0) {
			nvt_spi_support = csci_integer("touchpanel.nvt.spi", 0);
		}
	}
#endif

	if(nvt_spi_support == 0){
		NVT_LOG("do not support touchpanel nvt spi %s\n", __func__);
		return -EINVAL;
	}
