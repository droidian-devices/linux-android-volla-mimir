...
static void post_test(struct cts_device *cts_dev)
{
    ...
    ret = cts_tcs_set_krang_workmode(cts_dev, CTS_KRANG_NORMAL_MODE);
    if (ret) {
...
// int cts_stylus_gesture_test(struct cts_device *cts_dev,
//         struct cts_test_param *param)
// {
//     cts_tcs_set_krang_workmode(cts_dev, CTS_KRANG_GS_DBG_MODE);
//     cts_tcs_set_pwr_mode(cts_dev, CTS_PWR_MODE_GSTR_DBG);
//     cts_set_int_data_types(cts_dev, INT_DATA_TYPE_RAWDATA|BIT(14));
//     cts_set_int_data_method(cts_dev, INT_DATA_METHOD_POLLING);
//     cts_tcs_polling_data(cts_dev,  u8 *buf, size_t size);
// }
