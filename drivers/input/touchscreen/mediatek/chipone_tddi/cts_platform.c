...
#endif
{
    ...
    /** - Init input device */
#if IS_ENABLED(CONFIG_CM_MIDMISC_SUPPORT) //jnier add 20240410
        ...
        struct cts_device_touch_msg *msgs, int num)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        case CTS_DEVICE_TOUCH_EVENT_STAY:
            ...
            if (csci_exist("touchpanel.cts.x.reverse")) {
                if (csci_integer("touchpanel.cts.x.reverse",0) == 1) {
                    input_report_abs(input_dev, ABS_MT_POSITION_X, pdata->res_x - x);
                } else {
...
#endif
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT)
            if (csci_exist("touchpanel.cts.y.reverse")) {
                if (csci_integer("touchpanel.cts.y.reverse",0) == 1) {
                    input_report_abs(input_dev, ABS_MT_POSITION_Y, pdata->res_y - y);
                } else {
