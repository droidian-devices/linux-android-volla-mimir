    ...
    case STYLUS_DEFAULT:
        if (pevt->tip && pevt->p) {
            if ((ts_data->log_level >= 2) || (!pevt->down))
                FTS_DEBUG("[PEN]x:%d,y:%d,p:%d,tip:%d,flag:%d,tilt:%d,%d max x:%d y:%d DOWN",
                          pevt->x, pevt->y, pevt->p, pevt->tip, pevt->flag,
                          pevt->tilt_x, pevt->tilt_y ,pen_x_max ,pen_y_max);
#if defined(CONFIG_TOUCHSCREEN_FTS_PEN_X_REVERSE)
            input_report_abs(pen_dev, ABS_X, pen_x_max - pevt->x);
#else
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT)
            if (csci_exist("touchpanel.gsl.pen.x.reverse")) {
                temp = csci_integer("touchpanel.gsl.pen.x.reverse",0);
                if (temp == 1) {
                    input_report_abs(pen_dev, ABS_X, pen_x_max - pevt->x);
                } else {
...
#endif
#endif
#if defined(CONFIG_TOUCHSCREEN_FTS_PEN_Y_REVERSE)
            input_report_abs(pen_dev, ABS_Y, pen_y_max - pevt->y);
#else
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT)
            if (csci_exist("touchpanel.gsl.pen.y.reverse")) {
                temp = csci_integer("touchpanel.gsl.pen.y.reverse",0);
                if (temp == 1) {
                    input_report_abs(pen_dev, ABS_Y, pen_y_max - pevt->y);
                } else {
...
int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    FTS_INFO("%s : ty---> for %d drivers probe!",FTS_CHIP_TYPE);
    fts_data = ts_data;
