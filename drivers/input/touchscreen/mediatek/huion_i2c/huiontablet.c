...
static u8 huion_change_x2y = TRUE;//FALSE;  20240323
static u8 huion_x_reverse = FALSE;
static u8 huion_y_reverse = TRUE;//FALSE;    20240323

#define VERSION_LENGTH 22   //detail in protocol of huion
#define PARA_LENGTH 19      //detail in protocol of huion
//#define HUION_EMR_DATA_LEN 12
#define HUION_EMR_DATA_LEN 8

char gVersionbuf[VERSION_LENGTH];
char gParabuf[PARA_LENGTH];
...
static u32 x_radio = 0;
static u32 y_radio = 0; 
//liuhao 20230818
static u32 x_get_max = 0;
static u32 y_get_max = 0;

void huion_reset_guitar(struct i2c_client *client, s32 ms);
void huion_int_sync(s32 ms, struct huion_ts_data *ts);
extern int ghuion_dfu_file(char *ver);
extern int ghuion_dfu(char *ver);
...
void huion_irq_disable(struct huion_ts_data *ts)
{
    ...
    if (!ts->irq_is_disable)
    {
        ...
        if (!tp_flags){
            tp_flags = 1;
            // printk("<cyt> disable tp_irq\n");
            //tp_notifier_call_chain(2, NULL);
        }
...
void huion_irq_enable(struct huion_ts_data *ts)
{
    ...
    if (ts->irq_is_disable)
    {
        ...
        //通知TP 电磁笔优先，报点完成了，启用TP
        if (tp_flags) 
        {          
            tp_flags = 0;
            //tp_notifier_call_chain(1, NULL);       
        }
...
static void huion_touch_down(struct huion_ts_data *ts, s32 id, s32 x, s32 y, s32 w)
{
    ...
    // printk("111 x_max:%d, y_max:%d, x:%d, y:%d w:%d\n", ts->abs_x_max,ts->abs_y_max, x, y,w);
    ...
    //y = SCREEN_MAX_X - y;
     ...
     printk("huion_touch_down ID:%d, ABS_X:%d, ABS_Y:%d, PRESSURE:%d\n", id, x, y, w);
    ...
    // // input_report_key(ts->input_dev, BTN_TOOL_PEN, 1);
     ...
     if(w > 10){
            ...
            input_report_key(ts->input_dev, BTN_TOUCH, 1); //20240327
            // printk("PRESS DOWN\n");
            input_report_key(ts->input_dev, BTN_TOOL_PEN, 1);     
            // input_report_key(ts->input_dev, BTN_LEFT, 1);
            // input_report_key(ts->input_dev, BTN_TOOL_MOUSE, 1);            
    }else {
            ...
            input_report_key(ts->input_dev, BTN_TOUCH, 0);  //20240327
            input_report_key(ts->input_dev, BTN_TOOL_PEN, 0);				
            // input_report_key(ts->input_dev, BTN_LEFT, 0);				    
            // input_report_key(ts->input_dev, BTN_TOOL_MOUSE, 0);          				
	 }  
...
Output:
    None.
*********************************************************/
// static void huion_touch_up(struct huion_ts_data *ts, s32 x, s32 y, s32 id)
// {
//     input_report_abs(ts->input_dev, ABS_X, x);
//     input_report_abs(ts->input_dev, ABS_Y, y);
//     input_report_abs(ts->input_dev, ABS_PRESSURE, 0);
...
static void huion_ts_work_func(struct work_struct *work)
{
    ...
    s32 i = 0;
    //s32 ret = -1;
    int input_realy_y,input_realy_x;
...
//GTP_DEBUG("x,y,w %d %d %d\n", input_x, input_y, input_w);
            // printk("touch: x,y,w %d %d %d\n", input_x, input_y, input_w);
#if SCREEN_MAP_SWITCH

            //input_x = (input_x / (X_AXIS_MAPPING));
            //input_y = (input_y / Y_AXIS_MAPPING);

            //<huion>2023.08.12 off
            // input_x = ((input_x*100)/x_radio);
            // input_y = ((input_y*100)/y_radio);                  

            input_realy_y = input_y;
            input_realy_x = input_x;
            input_x = input_x-390;
            input_y = input_y-490;

            //printk("input_realy_x = %d show_x = %d \n",input_realy_x,input_x);
            //printk("input_realy_y = %d show_y = %d \n",input_realy_y,input_y);
            printk("<huion> x = %d y = %d x_get_max = %ld y_get_max = %d rel_x = %ld rel_y = %ld \n",input_x,input_y,x_get_max,y_get_max,input_realy_x,input_realy_y);
...
//             input_y = ((input_y*100)/y_radio);
// #endif
//             huion_touch_up(ts, input_x, input_y, 0);
//         }
//     }
                                  ...
                                  struct huion_ts_data *ts)
{
    ...
    set_bit(INPUT_PROP_DIRECT, ts->input_dev->propbit); //这个不显示光标
    // set_bit(INPUT_PROP_POINTER, ts->input_dev->propbit); //这个显示光标
    set_bit(EV_ABS, ts->input_dev->evbit);
    set_bit(EV_KEY, ts->input_dev->evbit);
    set_bit(BTN_TOUCH, ts->input_dev->keybit);

    // set_bit(BTN_LEFT, ts->input_dev->keybit);  //20240327 kim
    // set_bit(BTN_TOOL_MOUSE, ts->input_dev->keybit); //20240327 kim

    set_bit(BTN_TOOL_PEN, ts->input_dev->keybit); //20240327启用  原来是禁用的，禁用应该也没影响
    ...
    if (huion_change_x2y)
    {
        ...
        printk("======huion_change_x2y is true");
        
        printk("0000X : %d  y: %d",x,y);
        
        input_set_abs_params(ts->input_dev, ABS_X, 0, SCREEN_MAX_Y, 0, 0);
        input_set_abs_params(ts->input_dev, ABS_Y, 0, SCREEN_MAX_X, 0, 0);
    }
    else
    {
         int x,y;
        ...
        printk("======huion_change_x2y is false");       
    ...
    input_set_abs_params(ts->input_dev,
                         ABS_PRESSURE, 0, MAX_PRESS_VALUE, 0, 0);  //MAX_PRESS_VALUE 8191  or 2047

    //sprintf(phys, "input/huion");
    ts->input_dev->name = huion_ts_name;
    ts->input_dev->phys = "input/ts";
    ts->input_dev->id.bustype = BUS_I2C;
    // ts->input_dev->id.vendor = 0xDEAD;
    // ts->input_dev->id.product = 0xBEEF;
    ts->input_dev->id.vendor = 0xFADE;
    ts->input_dev->id.product = 0xBADE;
   
    
    //ts->input_dev->id.version = 10427;
    ts->input_dev->id.version = ((unsigned short)gVersionbuf[13]<<8) | gVersionbuf[14];
...
int huion_ts_get_version(struct i2c_client *client,char*ver)
{
    ...
    {
        ret = ghuion_i2c_wxdata(client,huicmd,sizeof(huicmd));
        if(ret < 0)
            return ret; 
        udelay(100);
...
static ssize_t huion_update_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    // u8 result[23] = {0xff};
    // //int i = 0;
    // //char *buf1;
    // printk("<VIEWFUN>%s:%d, %s\n", __func__, __LINE__, buf);
    // huion_ts_get_version(to_i2c_client(dev), gVersionbuf);
    // result[0] = ghuion_dfu_file(&gVersionbuf[2]);
    // huion_ts_get_version(to_i2c_client(dev), &result[1]);
    // memcpy(buf, result, 23);
    // printk("<VIEWFUN>%s:%d, sizeof(result):%x\n", __func__, __LINE__, sizeof(result));
    return 23;
}

static DEVICE_ATTR(huion_update, 0444, huion_update_show, NULL);
...
static int huion_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    struct huion_ts_data *ts;
    //u16 version_info;
    char prod_info_txbuf[3] = {0x5A, 0xC8, 0xA5};
    unsigned char prod_info_rxbuf[19] = {0};
    u32 x_max = 0, y_max = 0;
...
#endif
    /*{
        huion_change_x2y = TRUE;
        huion_x_reverse = FALSE;
        huion_y_reverse = FALSE;
    }*/

    ts->tp_regulator = devm_regulator_get(&client->dev, "tp");
    //ts->tp_regulator = devm_regulator_get(&client->dev, "vdd_ana");
    ...
    GTP_GPIO_OUTPUT(ts->pwr_pin, 1);
    //printk("!!!kim parse property of tree,%d %d\n", ts->irq_pin, ts->rst_pin);
    ...
    ret = -1;
    while(ret < 0){
        ret = huion_ts_get_version(client,gVersionbuf);
        msleep(400);
        if(count++ > 5){
            ghuion_reset_fw( i2c_connect_client_hn, 50);
            msleep(1500);     
            // extern int ghuion_dfu(char *ver);
             ret = DFU_proc(&gVersionbuf[2]); //前面的两个字节和版本号无关
             printk("DFU_proc ret: %d",ret);
            //ret = ghuion_dfu(&gVersionbuf[2]);
            client->addr = (0x11 >> 1);
            if(ret){
            msleep(100);
            printk("need to read the version again ");
            huion_ts_get_version(client,gVersionbuf);
            // huion_ts_get_parameter(client,gParabuf);  
            }
    ...
    client->addr = (0x11 >> 1);
    if(gParabuf[1] != 0xc8) //the is the right response
    {
        goto probe_init_error;
    }
   
    ghuion_i2c_wxdata(ts->client, prod_info_txbuf, 3);
    ghuion_i2c_rxdata(ts->client, prod_info_rxbuf, 19, 0x0);
    

	printk("<CYT> x:%x, %x, %x, y: %x, %x, %x \n", prod_info_rxbuf[2], prod_info_rxbuf[3], 
                        prod_info_rxbuf[4], prod_info_rxbuf[5], prod_info_rxbuf[6], prod_info_rxbuf[7]);
    x_max = (prod_info_rxbuf[4] << 16) | (prod_info_rxbuf[3] << 8) | prod_info_rxbuf[2];
    y_max = (prod_info_rxbuf[7] << 16) | (prod_info_rxbuf[6] << 8) | prod_info_rxbuf[5];
	printk("<CYT> x:%u, y: %u \n", x_max, y_max);
    x_radio = (x_max*100)/1280;
    y_radio = (y_max*100)/800;
	...
	printk("<CYT> x_radio:%u, y_radio: %u \n", x_radio, y_radio);
    ...
    device_create_file(&client->dev, &dev_attr_huion_update);
