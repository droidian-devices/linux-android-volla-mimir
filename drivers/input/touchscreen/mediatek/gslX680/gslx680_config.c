...
//	int ret = 0;
//
//	if(csci_exist(items_key)) {
//		csci_string(STR_GPIO, items_key, 0);
...
//		} 
//	} else {
//		print_info("[%s] \"%s\" not exist in csci.ini !!! \n", __func__, items_key);
//	} 
//
//	return ret;
//}
...
/* 	char STR_PMIC[64];
	...
	print_info("[%s]\n", __func__);
	csci_string(STR_PMIC, items_key, 0);
...
{
	if(csci_exist("touchpanel.gsl.firmware"))
		csci_string(name_tp_mode,"touchpanel.gsl.firmware",0);
	else {
		strcpy(name_tp_mode, gsl_tp_info[0].pro_tp_data[0].name);
		print_info("[%s] <touchpanel.gsl.firmware> not exist in csci.ini."
							" use default name:%s\n", __func__, name_tp_mode);
		return -1;
	}
...
{
/*
	if(csci_exist("touchpanel.i2c.num"))
		i2c_num = csci_integer("touchpanel.i2c.num",0);
	else
		i2c_num = 0;
*/
	return 0;
...
{
	if(csci_exist("touchpanel.power.num")) {
...
/*
    if(csci_equal("touchpanel.gsl.ic", "GSL1686", 0) || \
    	csci_equal("touchpanel.gsl.ic", "GSL1686F", 0) || \
    	csci_equal("touchpanel.gsl.ic", "GSL3675", 0) || \
    	csci_equal("touchpanel.gsl.ic", "GSL3676", 0))
		touchpanel_gsl_ic = 0;	
    else if(csci_equal("touchpanel.gsl.ic", "GSL915", 0) || \
    	csci_equal("touchpanel.gsl.ic", "GSL3670", 0))
		touchpanel_gsl_ic = 1;
*/
...
int tp_input_orientation_config(void)
{
	...
	if(csci_exist("ro.vendor.sf.hwrotation"))
		sf_hwrotation = csci_integer("ro.vendor.sf.hwrotation",0);
	else
		sf_hwrotation = -1;

	if(csci_exist("touchpanel.gsl.xy.swap"))
		xy_swap = csci_integer("touchpanel.gsl.xy.swap",0);
	else
		xy_swap = -1;

	if(csci_exist("touchpanel.gsl.x.reverse"))
		x_reverse = csci_integer("touchpanel.gsl.x.reverse",0);
	else
		x_reverse = -1;

	if(csci_exist("touchpanel.gsl.y.reverse"))
		y_reverse = csci_integer("touchpanel.gsl.y.reverse",0);
	else
		y_reverse = -1;

	if(csci_exist("touchpanel.gsl.xy.deal"))
		xy_deal = csci_integer("touchpanel.gsl.xy.deal",0);
	else
		xy_deal = -1;

	if(csci_exist("touchpanel.gsl.xy.deal.minor"))
		xy_deal_minor = csci_integer("touchpanel.gsl.xy.deal.minor",0);
	else
		xy_deal_minor = -1;
