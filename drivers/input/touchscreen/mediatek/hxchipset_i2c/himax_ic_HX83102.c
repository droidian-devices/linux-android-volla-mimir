...
static void hx83102j_en_hw_crc(int en)
{
	...
	himax_parse_assign_cmd(HX83102j_ic_addr_hw_crc,
			addr,
			DATA_LEN_4);

	if (en) {
		himax_parse_assign_cmd(HX83102j_data_hw_crc,
			data,
			DATA_LEN_4);
	} else {
	...
	do {
		...
		usleep_range(1000, 1100);
		g_core_fp.fp_register_read(addr, data, DATA_LEN_4);
		I("%s:ECC data[1]=%d, data[0]=%d, retry_cnt=%d\n", __func__,
				data[1], data[0], retry_cnt);
		retry_cnt++;
...
static bool hx83102j_read_event_stack(uint8_t *buf, uint32_t length)
{
...
static bool hx83102e_read_event_stack(uint8_t *buf, uint32_t length)
{
	struct time_var t_start, t_end, t_delta;
	int len = length;
	int i2c_speed = 0;
	...
	for (i = 0; i < 5; i++) {
		...
		|| (tmp_data[1] == 0x2e) || (tmp_data[1] == 0x29))) {
			if (tmp_data[1] == 0x2d) {
				...
				ic_data->flash_size = HX83102JA_FLASH_SIZE;
