...
update_retry:
	...
	{
		char name[128];
		int fwver = ic_data->vendor_fw_ver;
		extern void Hwinfo_update_info_cust(int hw_type, char *name);
		#define HW_TYPE_TP    2
		I("%s: TP upgrade OK11111 \n", __func__);
		if (fwver != 0) {
			sprintf(name,"%s ver:0x%04x",THIS_MODULE->name, fwver);
		} else {
