...
#define HX_HWCRC_STR "%s: opt_crc %s: R%02X%02X%02X%02XH <= 0x%02X%02X%02X%02X\n"
static void hx_setup_opt_hw_crc(const struct firmware *fw)
{
	...
	if (g_zf_opt_crc.fw_addr != 0) {
		...
		I(HX_HWCRC_STR,
			__func__,
			"start",
			g_zf_opt_crc.start_addr[3], g_zf_opt_crc.start_addr[2],
			g_zf_opt_crc.start_addr[1], g_zf_opt_crc.start_addr[0],
		...
		I(HX_HWCRC_STR,
			__func__,
			"end",
			g_zf_opt_crc.end_addr[3], g_zf_opt_crc.end_addr[2],
			g_zf_opt_crc.end_addr[1], g_zf_opt_crc.end_addr[0],
...
int himax_zf_part_info(const struct firmware *fw, int type)
{
...
#endif
	if (g_core_fp._en_hw_crc != NULL)
		g_core_fp._en_hw_crc(1);
	...
	cfg_crc_sw = g_core_fp.fp_Calculate_CRC_with_AP(g_update_cfg_buf,
		0, cfg_sz);
	do {
