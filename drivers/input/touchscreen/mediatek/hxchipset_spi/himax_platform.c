...
#if defined(CONFIG_TOUCHSCREEN_HIMAX_PINCTRL)
...
int himax_parse_dt(struct himax_ts_data *ts, struct himax_platform_data *pdata)
{
	...
	if (ret == 0) {
		...
		pdata->abs_x_max = DISP_GetScreenWidth() - 1;//(coords[1] - 1);
		pdata->abs_y_min = coords[2];
		pdata->abs_y_max = DISP_GetScreenHeight() - 1;//(coords[3] - 1);
	...
	pdata->screenWidth  = DISP_GetScreenWidth();//coords[1];
	pdata->screenHeight = DISP_GetScreenHeight();//coords[3];
...
int himax_gpio_power_config(struct himax_platform_data *pdata)
{
	...
	if (pdata->gpio_reset >= 0) {
		#if !defined(CONFIG_WB_DOUBLE_CLICK_WAKEUP) //add by yxj 20241016
...
int himax_chip_common_probe(struct spi_device *spi)
{
	struct himax_ts_data *ts;
	int ret = 0;
	int himax_spi_support = 0;

#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT)
	if (csci_exist("touchpanel.himax.spi")) {
		if (csci_integer("touchpanel.himax.spi", 0) > 0) {
			himax_spi_support = csci_integer("touchpanel.himax.spi", 0);
		}
	}
#endif

	if(himax_spi_support == 0){
		I("do not support touchpanel himax spi %s\n", __func__);
		return -EINVAL;
	}
...
#if defined(CONFIG_TOUCHSCREEN_HIMAX_PINCTRL)
    spi_pinctrl = devm_pinctrl_get(ts->dev);
    if (IS_ERR(spi_pinctrl)) {
        ret = PTR_ERR(spi_pinctrl);
        E("Cannot find spi_pinctrl!\n");
        //return ret;
        pinctrl_ok = false;
    } else {
