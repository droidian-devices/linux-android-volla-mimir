...
static void himax_mcu_burst_enable(uint8_t auto_add_4_byte)
{
	...
	ret = himax_bus_write(on_pic_op->addr_conti[0], NULL, tmp_data, 1);
	if (ret < 0) {
		E("%s: bus access fail!\n", __func__);
		return;
	}
	...
	ret = himax_bus_write(on_pic_op->addr_incr4[0], NULL, tmp_data, 1);
	if (ret < 0) {
		E("%s: bus access fail!\n", __func__);
		return;
	}
...
static int himax_mcu_register_read(uint8_t *addr, uint8_t *buf, uint32_t len)
{
	...
	ret = himax_bus_write(on_pic_op->addr_ahb_addr_byte_0[0], tmp_data,
		NULL, sizeof(tmp_data));
	if (ret < 0) {
		E("%s: bus access fail!\n", __func__);
	...
	ret = himax_bus_write(on_pic_op->addr_ahb_access_direction[0], NULL,
		tmp_data, 1);
	if (ret < 0) {
		E("%s: bus access fail!\n", __func__);
		mutex_unlock(&private_ts->reg_lock);
	...
	ret = himax_bus_read(on_pic_op->addr_ahb_rdata_byte_0[0], buf, len);
	if (ret < 0) {
		E("%s: bus access fail!\n", __func__);
		mutex_unlock(&private_ts->reg_lock);
		return BUS_FAIL;
		...
		uint8_t *write_data, uint32_t length)
{
	...
	ret = himax_bus_write(on_pic_op->addr_ahb_addr_byte_0[0], reg_byte,
		write_data, length+ADDR_LEN_4);
	if (ret < 0)
		E("%s: bus access fail!\n", __func__);
}
...
static int himax_mcu_register_write(uint8_t *addr, uint8_t *val, uint32_t len)
{
	...
	himax_mcu_flash_write_burst_lenth(addr, val, len);
...
static void himax_mcu_interface_on(void)
{
	...
	do {
		...
		ret = himax_bus_write(on_pic_op->addr_conti[0], NULL, tmp_data,
			1);
		if (ret < 0) {
			E("%s: bus access fail!\n", __func__);
			return;
		...
		ret = himax_bus_write(on_pic_op->addr_incr4[0], NULL, tmp_data,
			1);
		if (ret < 0) {
			E("%s: bus access fail!\n", __func__);
			return;
...
static void himax_mcu_sense_on(uint8_t FlashMode)
{
	...
	ret = himax_bus_write(on_pic_op->adr_mcu_ctrl[0], NULL, tmp_data, 1);
	if (ret < 0)
		E("%s: bus access fail!\n", __func__);
...
static void himax_mcu_tcon_on(void)
{
	g_core_fp.fp_register_write(on_pic_op->adr_tcon_ctrl,
		on_pic_op->cmd_tcon_on, sizeof(on_pic_op->adr_tcon_ctrl));
}
...
static bool himax_mcu_watch_dog_off(void)
{
	...
	do {
		...
		g_core_fp.fp_register_read(on_pic_op->adr_wdg_ctrl, tmp_data,
			sizeof(tmp_data));
...
static bool himax_mcu_sense_off(bool check_en)
{
	...
	if (g_core_fp.fp_watch_dog_off()) {
		reval = himax_bus_write(on_pic_op->adr_mcu_ctrl[0], NULL,
			on_pic_op->cmd_mcu_off, 1);
...
static void himax_mcu_sleep_in(void)
{
	int ret = 0;
	/*tmp_data[0] = 0x80;*/
	ret = himax_bus_write(on_pic_op->adr_sleep_ctrl[0], NULL,
		on_pic_op->cmd_sleep_in, 1);
	if (ret < 0)
		E("%s: bus access fail!\n", __func__);
}
...
static uint32_t himax_mcu_check_CRC(uint8_t *start_addr, int reload_length)
{
	...
	ret = g_core_fp.fp_register_write(on_pfw_op->addr_reload_addr_cmd_beat,
				       tmp_data, DATA_LEN_4);
	if (ret < NO_ERR) {
		E("%s: i2c access fail!\n", __func__);
		return HW_CRC_FAIL;
	...
	ret =  g_core_fp.fp_register_read(on_pfw_op->addr_reload_addr_cmd_beat,
			tmp_data,
			DATA_LEN_4);

	if ((tmp_data[1] != (length >> 8)) || (tmp_data[0] != length)) {
	...
	ret = g_core_fp.fp_register_read(on_pfw_op->addr_crc_value, tmp_data,
		DATA_LEN_4); /*50*/
	if (ret < NO_ERR) {
		E("%s: bus access fail!\n", __func__);
		return HW_CRC_FAIL;
...
static void himax_mcu_set_SMWP_enable(uint8_t SMWP_enable, bool suspended)
{
	...
	do {
		if (SMWP_enable == 0x01 && suspended) {
			...
			himax_bus_write(on_pfw_op->addr_smwp_enable[0], NULL,
				tmp_data, 1);
		...
		himax_bus_read(on_pfw_op->addr_smwp_enable[0], tmp_data, 1);
...
static void himax_mcu_usb_detect_set(uint8_t *cable_config)
{
	...
	do {
		if (cable_config[1] == 0x01) {
			...
			himax_bus_write(on_pfw_op->addr_usb_detect[0], NULL,
				tmp_data, 1);
		...
		himax_bus_read(on_pfw_op->addr_usb_detect[0], tmp_data, 1);
		...
		uint8_t storage_type, bool is_dirly)
{
	...
	ret = himax_bus_write(on_pfw_op->addr_raw_out_sel[0], NULL, tmp_data,
		1);
	if (ret < 0) {
		E("%s: bus access fail!\n", __func__);
		return;
...
static void himax_mcu_control_reK(bool enable)
{
	...
	g_core_fp.fp_register_read(on_pfw_op->addr_reK_crtl, tmp_data,
		DATA_LEN_4);
	...
	g_core_fp.fp_register_write(on_pfw_op->addr_reK_crtl, tmp_data,
		DATA_LEN_4);
}
...
static int himax_mcu_chip_self_test(struct seq_file *s, void *v)
{
	 ...
	 */
	/*read test criteria*/
	himax_bus_read(on_pfw_op->addr_criteria_addr[0], data, DATA_LEN_8);
	...
	himax_bus_write(on_pfw_op->addr_selftest_addr_en[0], NULL, data, 1);
...
static void himax_mcu_idle_mode(int disable)
{
	...
	if (!disable) {
		ret = himax_bus_write(on_pfw_op->addr_fw_mode_status[0], NULL,
...
static bool himax_mcu_read_event_stack(uint8_t *buf, uint8_t length)
{
	...
	ret = himax_bus_write(on_pfw_op->addr_ahb_addr[0], NULL, cmd, 1);
	if (ret < 0) {
		E("%s: bus access fail!\n", __func__);
		return 0;
	}

	himax_bus_read(on_pfw_op->addr_event_addr[0], buf, length);
...
static void himax_mcu_chip_erase(void)
{
	g_core_fp.fp_register_write(on_pflash_op->addr_ctrl_auto,
		on_pflash_op->data_main_erase, DATA_LEN_4);
}
...
static bool himax_mcu_ahb_squit(void)
{
	...
	g_core_fp.fp_register_write(on_pflash_op->addr_ahb_ctrl,
		on_pflash_op->data_ahb_squit, DATA_LEN_4);
	/*msleep(1);*/
	usleep_range(1000, 1100);
		...
		uint8_t rw_ctrl) /*r:0, w:1*/
{
	...
	g_core_fp.fp_register_write(on_pflash_op->addr_ctrl_auto, tmp_data,
		DATA_LEN_4);
	...
	tmp_data[0] = 0x00;
	if (!rw_ctrl) {
		g_core_fp.fp_register_read(addr, data, length);
		I("%s:SFR!data0=0x%2X,data1=0x%2X,data2=0x%2X,data3=0x%2X\n",
			__func__, data[0], data[1], data[2], data[3]);
	} else {
		g_core_fp.fp_register_write(addr, data, length);
		/*msleep(1);*/
		usleep_range(1000, 1100);
		...
		uint8_t *write_data)
{
	...
	g_core_fp.fp_register_write(on_pflash_op->addr_ctrl_base, write_addr,
		DATA_LEN_4);
	...
	g_core_fp.fp_register_write(tmp_addr, write_data, length);
		...
		int length)
{
	...
	addr = addr + FW_PAGE_SZ) {
		...
		g_core_fp.fp_register_read(tmp_addr, tmp_data, FW_PAGE_SZ);
...
static bool himax_mcu_get_DSRAM_data(uint8_t *info_data, bool DSRAM_Flag)
{
	...
	if (DSRAM_Flag == false) {
		...
		g_core_fp.fp_register_write(on_psram_op->addr_rawdata_addr,
			on_psram_op->data_fin, DATA_LEN_4);
	} else {
		/*I("Continue to SRAM!\n");*/
		g_core_fp.fp_register_write(on_psram_op->addr_rawdata_addr,
			on_psram_op->data_conti, DATA_LEN_4);
	}
...
static void himax_mcu_pin_reset(void)
{
	...
	tmp_data[0] = 0x5A;
	himax_bus_write(0x80, NULL, tmp_data, 1);
	usleep_range(1000, 1100);
	tmp_data[0] = 0x00;
	himax_bus_write(0x80, NULL, tmp_data, 1);
	I("MCU Sleep Out\n");
...
static uint8_t himax_mcu_tp_info_check(void)
{
	...
	g_core_fp.fp_register_read(on_pdriver_op->addr_fw_rx_tx_maxpt_num, data,
		DATA_LEN_4);
	...
	g_core_fp.fp_register_read(on_pdriver_op->addr_fw_xy_rev_int_edge, data,
		DATA_LEN_4);
	...
	rx_num = data[0];
	tx_num = data[1];
	g_core_fp.fp_register_read(on_pdriver_op->addr_fw_maxpt_bt_num, data,
		DATA_LEN_4);
	max_pt = data[0];
	bt_num = data[1];
...
#endif
	stylus_func = false; /*Not support pen in on-cell*/
