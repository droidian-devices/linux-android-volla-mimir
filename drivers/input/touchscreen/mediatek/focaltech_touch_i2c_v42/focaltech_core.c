...
#else
	fts_reset_pre(ts_data, value);
	cust_gpio_set_value(CUST_GPIO_TP_RST, !!value);
#endif
...
static int fts_input_init(struct fts_ts_data *ts_data)
{
    ...
    input_dev->name = "mtk-tpd";//FTS_DRIVER_NAME;
...
#if IS_ENABLED(CONFIG_DRM_WB_EXTERN_ESD) //Leo 20231114
#define FTS_ESD_REG         0x9d

//true need do esd, false no need
static int fts_ts_get_esd_status(void)
{
	...
	ret = (val == 0x55) ? true : false;
	pr_err("%s ret:%d spi read %x val:%xfailed\n",__func__,ret,FTS_ESD_REG, val);
...
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20231214
	pr_info("%s ret:%d  disable_tp_esd:%d\n",__func__,ret, disable_tp_esd);
#endif
...
#if IS_ENABLED(CONFIG_DRM_WB_EXTERN_ESD) //Leo 20231114
    mtk_drm_extern_esd_register(EXTERN_ESD_FTS_I2C, &mtk_drm_ext_esd_ops);
#endif
