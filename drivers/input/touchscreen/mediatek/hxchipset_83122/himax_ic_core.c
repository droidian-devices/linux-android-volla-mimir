...
/* CORE_FW */
/* FW side start*/
static void diag_mcu_parse_raw_data(struct himax_report_data *hx_s_touch_data,
...
int himax_mcu_calc_crc_by_ap(unsigned char *fw_content,
...
bool himax_mcu_read_event_stack(uint8_t *buf, uint8_t length)
{
	...
	cmd[0] = hx_s_ic_setup._data_ahb_dis;

	ret = himax_bus_write(hx_s_ic_setup._addr_ahb, NUM_NULL, cmd, 1);
	if (ret < 0) {
		E("%s: bus access fail!\n", __func__);
		return 0;
	}
	...
	cmd[0] = hx_s_ic_setup._data_ahb_en;
...
void himax_mcu_diag_parse_raw_data(
		struct himax_report_data *hx_s_touch_data,
		int mul_num, int self_num, uint8_t diag_cmd,
		int32_t *mutual_data, int32_t *self_data)
{
	diag_mcu_parse_raw_data(hx_s_touch_data, mul_num, self_num,
...
static int himax_zf_part_info(const struct firmware *fw, int type)
{
	...
	cfg_crc_sw = hx_s_core_fp._calc_crc_by_ap(cfg_buf, 0, cfg_sz);
	do {
		hx_s_core_fp._write_sram_0f(sram_min, cfg_buf, cfg_sz);
		cfg_crc_hw = hx_s_core_fp._calc_crc_by_hw(sram_min, cfg_sz);
