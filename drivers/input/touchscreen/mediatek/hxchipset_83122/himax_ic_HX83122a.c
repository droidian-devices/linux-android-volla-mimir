...
struct ic_setup_collect g_hx83122a_setup = {
	...
	._func_hsen = HX83112A_FUNC_HSEN,
	._func_smwp = HX83112A_FUNC_SMWP,
	._func_usb_detect = HX83112A_FUNC_USB_DETECT,
	._func_ap_notify_fw_sus     = HX83112A_FUNC_AP_NOTIFY_FW_SUS,
	._func_en  = FUNC_EN,
	._func_dis = FUNC_DIS,
	...
	._addr_rawout_sel = HX83122A_ADDR_RAWOUT_SEL,
	...
	._addr_set_frame = HX83122A_ADDR_SET_FRAME,
	._data_set_frame = DATA_SET_FRAME,

	._para_idle_dis = PARA_IDLE_DIS,
	._para_idle_en = PARA_IDLE_EN,
	._addr_sorting_mode_en      = HX83122A_ADDR_SORTING_MODE_EN,
	._addr_fw_mode_status       = HX83122A_ADDR_FW_MODE_STATUS,
	._addr_fw_ver               = HX83122A_ADDR_FW_VER,
	._addr_fw_cfg               = HX83122A_ADDR_FW_CFG,
	._addr_fw_vendor            = HX83122A_ADDR_FW_VENDOR,
	._addr_cus_info             = HX83122A_ADDR_CUS_INFO,
	._addr_proj_info            = HX83122A_ADDR_PROJ_INFO,
	._addr_fw_state             = ADDR_FW_STATE, //useless
	._addr_fw_dbg_msg           = HX83122A_ADDR_FW_DBG_MSG,
...
#endif
	._addr_ctrl_mpap_ovl    = HX83122A_ADDR_CTRL_MPAP_OVL,
	...
	._addr_mkey             = HX83122A_ADDR_MKEY,
	._addr_rawdata_buf      = HX83122A_ADDR_RAWDATA_BUF,
	._data_rawdata_end      = DATA_RAWDATA_END,
	._pwd_get_rawdata_start = PWD_GET_RAWDATA_START,
	._pwd_get_rawdata_end   = PWD_GET_RAWDATA_END,

	._addr_chk_fw_reload    = HX83122A_ADDR_CHK_FW_RELOAD,
	._addr_chk_fw_reload2   = HX83122A_ADDR_CHK_FW_RELOAD2,
	...
	._addr_chk_irq_edge     = HX83122A_ADDR_CHK_IRQ_EDGE,
	._addr_info_channel_num = HX83122A_ADDR_INFO_CHANNEL_NUM,
	._addr_info_max_pt      = HX83122A_ADDR_INFO_MAX_PT,
	._addr_info_def_stylus  = HX83122A_ADDR_INFO_DEF_STYLUS,
	._addr_info_stylus_ratio = HX83122A_ADDR_INFO_STYLUS_RATIO,
	...
	/* for inspection */
	._addr_normal_noise_thx   = HX83122A_ADDR_NORMAL_NOISE_THX,
	._addr_lpwug_noise_thx    = HX83122A_ADDR_LPWUG_NOISE_THX,
	._addr_noise_scale        = HX83122A_ADDR_NOISE_SCALE,
	._addr_recal_thx          = HX83122A_ADDR_RECAL_THX,
	._addr_palm_num           = HX83122A_ADDR_PALM_NUM,
	._addr_weight_sup         = HX83122A_ADDR_WEIGHT_SUP,
	._addr_normal_weight_a    = HX83122A_ADDR_NORMAL_WEIGHT_A,
	._addr_lpwug_weight_a     = HX83122A_ADDR_LPWUG_WEIGHT_A,
	._addr_weight_b           = HX83122A_ADDR_WEIGHT_B,
	._addr_max_dc             = HX83122A_ADDR_MAX_DC,
	._addr_skip_frame         = HX83122A_ADDR_SKIP_FRAME,
	._addr_neg_noise_sup      = HX83122A_ADDR_NEG_NOISE_SUP,
...
void hx83122a_burst_enable(uint8_t auto_add_4_byte)
{
...
int hx83122a_register_write(uint32_t addr, uint8_t *val, uint32_t len)
{
	int ret = -1;
	// uint8_t tmp_addr[DATA_LEN_4];
	...
	if (addr == ADDR_FLASH_SPI200_DATA)
		hx83122a_burst_enable(0);
	else if (len > DATA_LEN_4)
		hx83122a_burst_enable(1);
	else
		hx83122a_burst_enable(0);

	ret = himax_bus_write(ADDR_AHB_ADDRESS_BYTE_0, addr, val,
		len + DATA_LEN_4);
	if (ret < 0) {
		E("%s: xfer fail!\n", __func__);
...
static int hx83122a_register_read(uint32_t addr, uint8_t *buf, uint32_t len)
{
	int ret = -1;
	uint8_t tmp_data[DATA_LEN_4];
	...
	ret = himax_bus_write(ADDR_AHB_ADDRESS_BYTE_0, addr, NULL, 4);
	if (ret < 0) {
		E("%s: bus access fail!\n", __func__);
		mutex_unlock(&hx_s_ts->reg_lock);
		return BUS_FAIL;
	...
	hx_parse_assign_cmd(
		...
		tmp_data,
		DATA_LEN_1);
	ret = himax_bus_write(ADDR_AHB_ACCESS_DIRECTION, NUM_NULL,
		&tmp_data[0], 1);
	if (ret < 0) {
		E("%s: bus access fail!\n", __func__);
		mutex_unlock(&hx_s_ts->reg_lock);
	...
	ret = himax_bus_read(ADDR_AHB_RDATA_BYTE_0, buf, len);
	if (ret < 0) {
		E("%s: bus access fail!\n", __func__);
		mutex_unlock(&hx_s_ts->reg_lock);
		return BUS_FAIL;
...
void hx83122a_system_reset(void)
{
	...
	hx83122a_register_write(ADDR_CTRL_FW,
		tmp_data, DATA_LEN_4);
	do {
		/* reset code*/
		/**
		...
		ret = himax_bus_write(ADDR_SENSE_ON_OFF_0, NUM_NULL, tmp_data,
			DATA_LEN_2);
		if (ret < 0)
			E("%s: bus access fail!\n", __func__);
		...
		hx83122a_register_read(ADDR_FLAG_RESET_EVENT,
			tmp_data, DATA_LEN_4);
...
static bool hx83122a_sense_off(bool check_en)
{
	...
	do {
		...
		/* check fw status */
		hx83122a_register_read(ADDR_CS_CENTRAL_STATE,
			tmp_data, DATA_LEN_4);
		...
		hx83122a_register_read(ADDR_CTRL_FW, tmp_data, 4);
	...
	do {
		...
		ret = himax_bus_write(tmp_addr[0], NUM_NULL, tmp_data,
			2);
		if (ret < 0) {
			E("%s: bus access fail!\n", __func__);
			return false;
		...
		if (tmp_data[0] == 0x0C) {
			...
			hx_parse_assign_cmd(DATA_CLEAR, tmp_data, DATA_LEN_4);
			hx83122a_register_write(HX83122A_ADDR_TCON_ON_RST,
				tmp_data, DATA_LEN_4);
			usleep_range(10000, 10001);
			goto SUCCEED;
		}
...
#if defined(HX_ZERO_FLASH)
static void hx83122a_reload_to_active(void)
{
...
static void hx83122a_en_hw_crc(int en)
{
	int cnt = 0;
	uint8_t tmp_data[DATA_LEN_4];
	...
	if (en) {
		hx_parse_assign_cmd(HX83122A_DATA_EN_HW_CRC,
			wrt_data, DATA_LEN_4);
	} else {
	...
	do {
		tmp_data[3] = wrt_data[3];
		tmp_data[2] = wrt_data[2];
		tmp_data[1] = wrt_data[1];
		tmp_data[0] = wrt_data[0];
		hx83122a_register_write(HX83122A_ADDR_EN_HW_CRC,
			tmp_data, DATA_LEN_4);
		usleep_range(1000, 1100);
		hx83122a_register_read(HX83122A_ADDR_EN_HW_CRC,
			tmp_data, DATA_LEN_4);
		I("%s: En ECC data[1]=%d, data[0]=%d, cnt=%d\n", __func__,
...
static void hx83122a_resume_ic_action(void)
{
#if !defined(HX_RESUME_HW_RESET)
	hx83122a_reload_to_active();
#endif
}
...
static void hx83122a_sense_on(uint8_t FlashMode)
{
	...
	if (!FlashMode) {
		...
		ret = himax_bus_write(tmp_addr[0], NUM_NULL,
			tmp_data, 2);
		if (ret < 0) {
...
bool hx83122a_read_event_stack(uint8_t *buf, uint8_t length)
{
...
bool hx83122a_bin_desc_get(unsigned char *fw, uint32_t max_sz)
{
	uint32_t addr_t = 0;
...
void hx83122a_init(void)
{
	...
	hx_s_core_fp._read_event_stack = hx83122a_read_event_stack;
	...
	hx_s_core_fp._bin_desc_get = hx83122a_bin_desc_get;
...
#endif
	hx_s_core_fp._system_reset = hx83122a_system_reset;
	...
	hx_s_core_fp._burst_enable = hx83122a_burst_enable;
	hx_s_core_fp._register_read = hx83122a_register_read;
	hx_s_core_fp._register_write = hx83122a_register_write;

	hx_s_core_fp._sense_off = hx83122a_sense_off;
	hx_s_core_fp._sense_on = hx83122a_sense_on;
	hx_s_core_fp._chip_init = hx83122a_chip_init;

#if defined(HX_ZERO_FLASH)
	hx_s_core_fp._resume_ic_action = hx83122a_resume_ic_action;
	hx_s_core_fp._0f_reload_to_active = hx83122a_reload_to_active;
	hx_s_core_fp._en_hw_crc = hx83122a_en_hw_crc;
#endif
...
static bool hx83122a_chip_detect(void)
{
	uint8_t tmp_data[DATA_LEN_4];
	bool ret_data = false;
	int ret = 0;
	...
	ret = himax_bus_read(ADDR_AHB_CONTINOUS, tmp_data, 1);
	if (ret < 0) {
		E("%s: bus access fail!\n", __func__);
		return false;
	}

	if (hx83122a_sense_off(false) == false) {
		ret_data = false;
		E("%s:_sense_off Fail:\n", __func__);
		return ret_data;
	}
	for (i = 0; i < 5; i++) {
		ret = hx83122a_register_read(HX83122A_REG_ICID,
			tmp_data, DATA_LEN_4);
		...
		&& ((tmp_data[1] == 0x2a))) {
			strlcpy(hx_s_ts->chip_name,
				HX_83122A_SERIES_PWON, 30);
			(hx_s_ic_data)->ic_adc_num =
				HX83122A_DATA_ADC_NUM;
			hx_s_ic_data->isram_sz =
				HX83122A_ISRAM_SZ;
			hx_s_ic_data->dsram_sz =
				HX83122A_DSRAM_SZ;
			hx_s_ic_data->flash_size = HX83122A_FLASH_SIZE;
			hx_s_ic_setup = g_hx83122a_setup;
			hx83122a_init();
...
bool _hx83122a_init(void)
{
	bool ret = false;

	I("%s\n", __func__);
	ret = hx83122a_chip_detect();
	return ret;
}
