...
static int himax_auto_update_check(void)
{
	...
	//add by LQ start
	if (csci_exist("touchpanel.himax.force.update")) {
		if(csci_integer("touchpanel.himax.force.update",0) == 1){
			ret = 0; //Need update
			return ret;
		}
	}
...
int himax_report_data_init(void)
{
	...
	hx_s_touch_data->touch_all_size = hx_s_core_fp._get_touch_data_size();
		...
		int ts_path, int ts_status)
{
	...
	case HX_REPORT_COORD:
		if ((g_hw_rst_activate)
			...
			if (!hx_s_core_fp._read_event_stack(buf, 128)) {
				E("%s: can't read data from chip!\n", __func__);
				ts_status = HX_TS_GET_DATA_FAIL;
			}
		} else {
			if (!hx_s_core_fp._read_event_stack(buf,
			hx_s_touch_data->touch_info_size)) {
				E("%s: can't read data from chip!\n", __func__);
				ts_status = HX_TS_GET_DATA_FAIL;
			}
		...
		&& (raw_data_sel != hx_s_touch_data->diag_cmd)) {
			...
			if (!hx_s_touch_data->diag_cmd) {
				...
				hx_s_core_fp._read_event_stack(buf,
					(128-hx_s_touch_data->touch_info_size));
...
#if defined(CONFIG_TOUCHSCREEN_HIMAX_X_REVERSE)
			input_report_abs(ts->input_dev, ABS_MT_POSITION_X,
					ts->pdata->abs_x_max - g_target_report_data->p[i].x);
#else
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20230302
            if (csci_exist("touchpanel.himax.x.reverse")) {
                if(csci_integer("touchpanel.himax.x.reverse",0) == 1){
			input_report_abs(ts->input_dev, ABS_MT_POSITION_X,
                        ts->pdata->abs_x_max - g_target_report_data->p[i].x);  
                }else{
                    input_report_abs(ts->input_dev, ABS_MT_POSITION_X,
...
#if defined(CONFIG_TOUCHSCREEN_HIMAX_Y_REVERSE)
			input_report_abs(ts->input_dev, ABS_MT_POSITION_Y,
					ts->pdata->abs_y_max - g_target_report_data->p[i].y);
#else
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20230302
            if (csci_exist("touchpanel.himax.y.reverse")) {
                if(csci_integer("touchpanel.himax.y.reverse",0) == 1){ 
                    input_report_abs(ts->input_dev, ABS_MT_POSITION_Y,
                        ts->pdata->abs_y_max - g_target_report_data->p[i].y);                
                }else{
                    input_report_abs(ts->input_dev, ABS_MT_POSITION_Y,
...
static void himax_stylus_report(struct himax_ts_data *ts)
{
	...
	if (valid) {/*stylus down*/
		...
		input_report_abs(ts->stylus_dev, ABS_X,
				ts->pdata->abs_x_max*10 - g_target_report_data->s[0].x);
#else
		input_report_abs(ts->stylus_dev, ABS_X,
				g_target_report_data->s[0].x);
#endif
		...
		input_report_abs(ts->stylus_dev, ABS_Y,
				ts->pdata->abs_y_max*10 - g_target_report_data->s[0].y);
#else
		input_report_abs(ts->stylus_dev, ABS_Y,
				g_target_report_data->s[0].y);
#endif
...
#if IS_ENABLED(CONFIG_CM_HARDWAREINFO_SUPPORT)
{
	sprintf(name,"%s cfg_ver:0x%.2x",HIMAX_NAME, hx_s_ic_data->vendor_touch_cfg_ver);
...
#if defined(CONFIG_TOUCHSCREEN_HIMAX_IC_HX8530K)
	if (_hx8530k_init()) {
		ret = NO_ERR;
		goto END;
	}
