...
/* CORE_FW */
	...
	int (*_calc_crc_by_ap)(unsigned char *FW_content,
			int CRC_from_FW,
			int len);
	...
	bool (*_read_event_stack)(uint8_t *buf, uint8_t length);
...
/* CORE_DRIVER */
	...
	void (*_diag_parse_raw_data)(struct himax_report_data *hx_s_touch_data,
			int mul_num,
			int self_num,
			uint8_t diag_cmd,
			int32_t *mutual_data,
...
extern int himax_mcu_calc_crc_by_ap(unsigned char *FW_content,
...
extern int himax_mcu_cal_data_len(int raw_cnt_rmd, int max_pt,
		int raw_cnt_max);
extern void himax_mcu_diag_parse_raw_data(
