...
static int himax_crc_test_read(struct seq_file *m)
{
	...
	msleep(20);
	if (hx_s_core_fp._diff_overlay_flash() == 1)
		size = FW_SIZE_128k;
	else
		size = FW_SIZE_64k;
...
static ssize_t himax_diag_cmd_write(char *buf, size_t len)
{
	...
	if (buf[0] == 'F') {
		...
		if (len > 0) {
			//remove 'F'
			strlcpy(&buf[0], &buf[1], len--);
			hx_s_debug_data->is_json = true;
		} else {
			E("%s:Fail command!\n", __func__);
			goto END;
		}
	...
	case 1:/*raw out select - diag,X*/
		if (!kstrtoint(buf, 16, &rst)) {
			...
			hx_s_core_fp._diag_register_set(ts->diag_cmd, 0, false);
			if (ts->diag_cmd == 0)
				hx_s_debug_data->is_json = false;
...
static void himax_diag_arrange_json(struct seq_file *s)
{
	int x_channel = hx_s_ic_data->rx_num;
	int y_channel = hx_s_ic_data->tx_num;
	int self_size = x_channel + y_channel;
	int mutual_size = x_channel * y_channel;
	int i;

	seq_puts(s, "\t\"mutual\":[\n");
	for (i = 0; i < mutual_size; i++) {
		if (i == mutual_size - 1)
			seq_printf(s, "%6d", diag_mutual[i]);
...
static int himax_diag_print(struct seq_file *s, void *v)
{
	...
	if (!hx_s_debug_data->is_json) {
		...
		himax_diag_arrange_json(s);

		himax_get_mutual_edge();
		himax_get_self_edge();
		seq_printf(s, "\t\"MutualMax\":%d,\n", g_max_mutual);
		seq_printf(s, "\t\"MutualMin\":%d,\n", g_min_mutual);
		seq_printf(s, "\t\"SelfMax\":%d,\n", g_max_self);
		seq_printf(s, "\t\"SelfMin\":%d\n", g_min_self);
		seq_puts(s, "}\n");
	}
...
static ssize_t himax_reset_write(char *buf, size_t len)
{
	...
	if (strnstr(buf, "help", len) != NULL) {
		...
		if (buf[0] == '1')
			...
			hx_s_core_fp._ic_reset(1);
		else if (buf[0] == '3')
			hx_s_core_fp._ic_reset(2);
		else if (buf[0] == '4')
