...
#if IS_ENABLED(CONFIG_CM_MIDMISC_SUPPORT) //Leo 20231010
...
#define FTS_CHIP_TYPE_MAPPING { \
	...
	{0x2A, 0x82, 0x06, 0x82, 0x06, 0x82, 0xA6, 0x82, 0xB6}, \
	{0x2C, 0x82, 0xC5, 0x82, 0xC5, 0x82, 0xD5, 0x82, 0xE5} \
}
...
#define FTS_DEBUG(fmt, args...) do { \
    printk("[FTS_TS_SPI]%s:"fmt"\n", __func__, ##args); \
} while (0)

#define FTS_FUNC_ENTER() do { \
    printk("[FTS_TS_SPI]%s: Enter\n", __func__); \
} while (0)

#define FTS_FUNC_EXIT() do { \
    printk("[FTS_TS_SPI]%s: Exit(%d)\n", __func__, __LINE__); \
} while (0)
...
#if IS_ENABLED(CONFIG_CM_MIDMISC_SUPPORT) //Leo 20231010
#if defined(DEBUG_BUILD) //Leo 20231011
#define FTS_INFO(fmt, args...) do { \
    if (cust_midmisc_get_log_en(MIDMISC_TS_LOG) == true) {\
        printk(KERN_INFO "[FTS_TS_SPI/I]%s:"fmt"\n", __func__, ##args); \
    } else { \
        pr_debug("[FTS_TS_SPI/I]%s:"fmt"\n", __func__, ##args); \
    } \
} while (0)
#else /*DEBUG_BUILD*/
...
#define FTS_ERROR(fmt, args...) do { \
    printk(KERN_ERR "[FTS_TS_SPI/E]%s:"fmt"\n", __func__, ##args); \
} while (0)
