...
static bool get_flow_cnt(struct fts_ts_data *ts_data)
{
    ...
    if (ret < 0) {
        ...
        if ( reg_value == fts_esdcheck_data.flow_work_cnt_last ) {
            pr_info("FTS:reg0x91,val:%x,last:%x", reg_value,
    ...
    if (fts_esdcheck_data.flow_work_hold_cnt >= 5) {
        pr_info("FTS: reg0x91 keep a value for 5 times, need execute TP reset");
        return true;
    }
...
#if IS_ENABLED(CONFIG_DRM_WB_EXTERN_ESD) //Leo 20231114
static bool cust_get_flow_cnt(struct fts_ts_data *ts_data)
{
    int     ret = 0;
    u8      reg_value[5] = {0};
    u8      reg_addr = 0;
    ...
    reg_addr = FTS_REG_FLOW_WORK_CNT;
    for (i = 0; i < 5; i++) {
        ret = fts_read(&reg_addr, 1, &reg_value[i], 1);
        if (ret < 0) {
            pr_info("read reg0x91 fail,ret:%d", ret);
            fts_esdcheck_data.nack_cnt++;
        } else {
            if (reg_value[i] == fts_esdcheck_data.flow_work_cnt_last ) {
                pr_info("FTS:reg0x91,val:%x,last:%x", reg_value[i],
                          fts_esdcheck_data.flow_work_cnt_last);
                fts_esdcheck_data.flow_work_hold_cnt++;
            } else {
                fts_esdcheck_data.flow_work_hold_cnt = 0;
            }
            fts_esdcheck_data.flow_work_cnt_last = reg_value[i];
        }
        mdelay(25);

        if (reg_value[0] != reg_value[1]) {
            pr_info("FTS: esd: reg_value[0]:%d != reg_value[1]:%d !\n",reg_value[0],reg_value[1]);
            break;
        }
    }
    ...
    /* Flow Work Cnt keep a value for 5 times, need execute TP reset */
    pr_info("FTS: reg0x91 keep a value for %d times !",fts_esdcheck_data.flow_work_hold_cnt);
    if (fts_esdcheck_data.flow_work_hold_cnt >= 5) {
        pr_info("FTS: reg0x91 keep a value for 5 times, need execute TP reset");
        return true;
    }
...
static int esdcheck_algorithm(struct fts_ts_data *ts_data)
{
    ...
    if (time_before(jiffies, intr_timeout)) {
        pr_info("intr_timeout, not check esd");
        return 0;
    }

    /* 2. check power state, if suspend, no need check esd */
    if (fts_esdcheck_data.suspend == 1) {
        pr_info("In suspend, not check esd");
    ...
    if (fts_esdcheck_data.proc_debug == 1) {
        pr_info("In apk/adb command mode, not check esd");
        return 0;
    }
    ...
    if ( ret < 0 ) {
        ...
        pr_info("in factory mode(%x), no check esd", reg_value);
        return 0;
    }

    /* 5. Get Chip ID */
    hardware_reset = get_chip_id(ts_data);
    pr_info("FTS hardware_reset:%d \n",hardware_reset);
    /* 6. get Flow work cnt: 0x91 If no change for 5 times, then ESD and reset */
#if IS_ENABLED(CONFIG_DRM_WB_EXTERN_ESD) //Leo 20231114
    if (0) {
        hardware_reset = cust_get_flow_cnt(ts_data);
    } else {
        hardware_reset = get_flow_cnt(ts_data);
    }
    pr_info("FTS after get_flow_cnt hardware_reset:%d \n",hardware_reset);
#else
    if (!hardware_reset) {
        hardware_reset = get_flow_cnt(ts_data);
    }
    ...
    if (hardware_reset == 1) {
        pr_info("NoACK=%d, Error Data=%d, Hardware Reset=%d",
                  fts_esdcheck_data.nack_cnt,
                  fts_esdcheck_data.dataerror_cnt,
                  fts_esdcheck_data.hardware_reset_cnt);
