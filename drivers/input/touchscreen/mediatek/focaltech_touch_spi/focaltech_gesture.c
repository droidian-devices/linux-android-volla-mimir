...
*****************************************************************************/
#if defined(CONFIG_WB_DOUBLE_CLICK_WAKEUP) //Leo 20231205
 ...
 *
 */
static DEVICE_ATTR(ts_gesture, S_IRUGO | S_IWUSR, fts_gesture_show,
                   fts_gesture_store); //20231215
/*
...
static struct attribute *fts_gesture_mode_attrs[] = {
    &dev_attr_ts_gesture.attr, //Leo 2021215
...
static int fts_create_gesture_sysfs(struct device *dev)
{
    ...
    ret = sysfs_create_link(NULL, &dev->kobj, "touchscreen"); //Leo 20231215
    if (ret < 0) { 
...
int fts_gesture_readdata(struct fts_ts_data *ts_data, u8 *touch_buf)
{
    ...
    pr_info("gesture_id=%d, point_num=%d",
...
int fts_gesture_init(struct fts_ts_data *ts_data)
{
    ...
    if (csci_exist("touchpanel.fts.gesture.support")) {
        if (csci_integer("touchpanel.fts.gesture.support", 0) >= 0) {
            ts_data->gesture_support = csci_integer("touchpanel.fts.gesture.support", 0);
        }
    }
#endif
