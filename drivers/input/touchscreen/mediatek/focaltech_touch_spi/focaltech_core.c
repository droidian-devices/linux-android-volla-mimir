...
#if IS_ENABLED(CONFIG_CM_CUST_GPIOS_EXT_SUPPORT) || IS_ENABLED(CONFIG_CM_CUST_GPIOS_SUPPORT) //Leo 20230131
...
#if IS_ENABLED(CONFIG_CM_MIDMISC_SUPPORT) //Leo 20230302
...
#if IS_ENABLED(CONFIG_CM_MIDMISC_SUPPORT) && defined(M100TBR210_KJ_965)//Leo 20230302
...
#if defined(CONFIG_TOUCHSCREEN_FTS_PINCTRL) //Leo 20231009
static struct pinctrl *spi_pinctrl;
static struct pinctrl_state *spi_pull_down;
static struct pinctrl_state *spi_pull_up;
static bool pinctrl_ok;
#endif
...
static int fts_input_report_b(struct fts_ts_data *ts_data, struct ts_event *events)
{
    ...
    for (i = 0; i < ts_data->touch_event_num; i++) {
        ...
        if (EVENT_DOWN(events[i].flag)) {
...
#if defined(CONFIG_TOUCHSCREEN_FTS_X_REVERSE)
            input_report_abs(input_dev, ABS_MT_POSITION_X, ts_data->pdata->x_max - events[i].x);
#else /*CONFIG_TOUCHSCREEN_FTS_X_REVERSE*/
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT)
            if (csci_exist("touchpanel.gsl.x.reverse")) {
                temp = csci_integer("touchpanel.gsl.x.reverse",0);
                if (temp == 1) {
                    input_report_abs(input_dev, ABS_MT_POSITION_X, ts_data->pdata->x_max - events[i].x);
                } else {
            ...
            input_report_abs(input_dev, ABS_MT_POSITION_X, events[i].x);
#endif
#endif /*CONFIG_TOUCHSCREEN_FTS_X_REVERSE*/
#if defined(CONFIG_TOUCHSCREEN_FTS_Y_REVERSE)
            input_report_abs(input_dev, ABS_MT_POSITION_Y, ts_data->pdata->y_max - events[i].y);
#else /*CONFIG_TOUCHSCREEN_FTS_Y_REVERSE*/
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT)
            if (csci_exist("touchpanel.gsl.y.reverse")) {
                temp = csci_integer("touchpanel.gsl.y.reverse",0);
                if (temp == 1) {
                    input_report_abs(input_dev, ABS_MT_POSITION_Y, ts_data->pdata->y_max - events[i].y);
                } else {
            ...
            input_report_abs(input_dev, ABS_MT_POSITION_Y, events[i].y);
#endif
#endif /*CONFIG_TOUCHSCREEN_FTS_Y_REVERSE*/
            ...
            input_mt_report_slot_state(input_dev, MT_TOOL_FINGER, false);
            touch_point_pre &= ~(1 << events[i].id);
            if (ts_data->log_level >= 1) FTS_INFO("[B]P%d UP!", events[i].id);
        }
    }
    ...
    if (unlikely(touch_point_pre ^ touch_down_point_cur)) {
        for (i = 0; i < max_touch_num; i++)  {
            if ((1 << i) & (touch_point_pre ^ touch_down_point_cur)) {
                if (ts_data->log_level >= 1) FTS_INFO("[B]P%d UP!", i);
...
#if defined(CONFIG_TOUCHSCREEN_FTS_X_REVERSE)
    input_report_abs(pen_dev, ABS_X, ts_data->pdata->x_max - pevt->x);
#else
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT)
    if (csci_exist("touchpanel.gsl.x.reverse")) {
        temp = csci_integer("touchpanel.gsl.x.reverse",0);
...
#if defined(CONFIG_TOUCHSCREEN_FTS_Y_REVERSE)
    input_report_abs(pen_dev, ABS_Y, ts_data->pdata->y_max - pevt->y);
#else
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT)
    if (csci_exist("touchpanel.gsl.y.reverse")) {
        temp = csci_integer("touchpanel.gsl.y.reverse",0);
...
#if 0//IS_ENABLED(CONFIG_CM_MIDMISC_SUPPORT) && defined(M100TBR210_KJ_965) //Leo 20231017
    if (!((cust_mid_misc_get_boot_mode() == FACTORY_BOOT)
        || (cust_mid_misc_get_boot_mode() == ATE_FACTORY_BOOT))) {
        if (is_boot_completed == false) {
            FTS_INFO("no boot complete, skip report !!!!! ");
            return -EINVAL;
        }
    }
#endif
...
#if IS_ENABLED(CONFIG_CM_MIDMISC_SUPPORT) //Leo 20230326
    if ((cust_mid_misc_get_boot_mode() == FACTORY_BOOT)
        || (cust_mid_misc_get_boot_mode() == ATE_FACTORY_BOOT)) {
...
static int fts_gpio_configure(struct fts_ts_data *data)
{
    ...
    if (gpio_is_valid(data->pdata->reset_gpio)) {
...
#if !(IS_ENABLED(CONFIG_WB_TP_INCELL_SUPPORT) && IS_ENABLED(CONFIG_CM_CUST_GPIOS_SUPPORT)) //Leo 20230131
            goto err_irq_gpio_dir;
#endif
        }
        ...
        cust_gpio_ext_init("fts_reset_gpio", data->pdata->reset_gpio);
#endif
#endif
#endif
    }
...
#if IS_ENABLED(CONFIG_CM_MIDMISC_SUPPORT) //Leo 20230323
                                 ...
                                 unsigned long value, void *v)
{
    ...
    if ((value == MTK_DISP_EVENT_BLANK)
        ...
        if (ts_data->gesture_support == DISABLE) {
            ...
            mdelay(3);
            cust_gpio_set_value(CUST_GPIO_BIAS_NEG, false);
            mdelay(2);
            cust_gpio_set_value(CUST_GPIO_BIAS_POS, false);
            mdelay(5);
            cust_gpio_set_value(CUST_GPIO_TP_RST, false);
            mdelay(10);
            cust_gpio_set_value(CUST_GPIO_LCM_RST, false);
            mdelay(10);
            cust_gpio_set_value(CUST_GPIO_LCM_1V8, false);
            mdelay(2);
            cust_gpio_set_value(CUST_GPIO_IT6112_EN, false);
        } else {
            FTS_INFO("gesture enable, don`t into sleep mode!");
            cust_gpio_set_value(CUST_GPIO_IT6112_EN, false);
        }
    }
#endif
    ...
    if ((value == MTK_DISP_EARLY_EVENT_BLANK)
        ...
        if (pinctrl_ok) {
            FTS_INFO("select spi_pull_up \n");
            pinctrl_select_state(spi_pinctrl, spi_pull_up);
        }
    }
    ...
    if ((value == MTK_DISP_EVENT_BLANK)
        ...
        if (pinctrl_ok) {
            FTS_INFO("select spi_pull_down \n");
            pinctrl_select_state(spi_pinctrl, spi_pull_down);
        }
    }
#endif

    if (ts_data && v) {
        FTS_INFO("%s IN value:%d *data:%d ", __func__,value,*data);
        ...
        if (value == MTK_DISP_EARLY_EVENT_BLANK) {
            ...
            FTS_INFO("%s OUT", __func__);
    } else {
            FTS_INFO("FTS touch IC can not suspend or resume");
            return -1;
    }
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    //if (active_panel) {
        ...
        ret = mtk_disp_notifier_register("FTS Touch", &ts_data->fb_notif);
        if (ret)
...
static int fts_ts_remove_entry(struct fts_ts_data *ts_data)
{
    ...
    if (active_panel)
         if (mtk_disp_notifier_unregister(&ts_data->fb_notif))
            FTS_ERROR("Error occurred while unregistering disp_notifier.\n");
...
* TP Driver
*****************************************************************************/
#if IS_ENABLED(CONFIG_CM_MIDMISC_SUPPORT) && defined(M100TBR210_KJ_965) //Leo 20231017
...
#if IS_ENABLED(CONFIG_DRM_WB_EXTERN_ESD) //Leo 20231114
#define FTS_ESD_REG         0xED
#define FTS_ESD_REG1        0x05
#define FTS_ESD_REG2        0xAC

int is_tp_esd_reset = false;
static int fts_ts_get_esd_status(void)
{
	u8 val;
	int ret = 0;
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20231214
	static bool is_init = false, disable_tp_esd = false;
#endif

	ret = fts_read_reg(FTS_ESD_REG, &val);
	if (ret < 0) {
		pr_err("%s ret:%d spi read 0xed failed\n",__func__,ret);
	}
	ret = (val == 0xaa) ? true : false;

	ret = fts_read_reg(FTS_ESD_REG1, &val);
	if (ret < 0) {
		pr_err("%s ret:%d spi read 0x05 failed\n",__func__,ret);
	}
	ret |= (val == 0xaa) ? true : false;

	ret = fts_read_reg(FTS_ESD_REG2, &val);
	if (ret < 0) {
		pr_err("%s ret:%d spi read 0xac failed\n",__func__,ret);
	}
	ret |= (val == 0xaa) ? true : false;
...
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20231214
	if (is_init == false) {
		if (csci_exist("touchpanel.fts.disable_tplcm_esd")) {
			if (csci_integer("touchpanel.fts.disable_tplcm_esd", 0) >= 0) {
				disable_tp_esd = !!csci_integer("touchpanel.fts.disable_tplcm_esd", 0);
			}
		}
		is_init = true;
	}

	if (disable_tp_esd == true) {
		is_tp_esd_reset = false;
	}
#endif /*CONFIG_MID_CSCI_SUPPORT*/

	if ((ret == false) && (is_tp_esd_reset > 0)) {
		ret = true;
		is_tp_esd_reset = 0;
	}
	pr_info("%s ret:%d is_tp_esd_reset:%d disable_tp_esd:%d\n",__func__,ret,is_tp_esd_reset, disable_tp_esd);

	return ret;
}

static const struct mtk_drm_extern_esd mtk_drm_ext_esd_ops = {
	.get_esd_status = fts_ts_get_esd_status,
};
#endif
...
static int fts_ts_probe(struct spi_device *spi)
{
	...
	int fts_spi_support = 0;
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT)
	if (csci_exist("touchpanel.fts.spi")) {
		if (csci_integer("touchpanel.fts.spi", 0) > 0) {
			fts_spi_support = csci_integer("touchpanel.fts.spi", 0);
		}
	}
#endif

	if(fts_spi_support == 0){
		FTS_INFO("do not support touchpanel fts spi %s\n", __func__);
		return -EINVAL;
	}
...
#if defined(CONFIG_TOUCHSCREEN_FTS_PINCTRL)
    spi_pinctrl = devm_pinctrl_get(ts_data->dev);
    if (IS_ERR(spi_pinctrl)) {
        ret = PTR_ERR(spi_pinctrl);
        FTS_ERROR("Cannot find spi_pinctrl!\n");
        //return ret;
        pinctrl_ok = false;
    } else {
        spi_pull_down = pinctrl_lookup_state(spi_pinctrl, "fts_spi0_cs_pull_down");
        if (IS_ERR(spi_pull_down)) {
            ret = PTR_ERR(spi_pull_down);
            FTS_ERROR("Cannot find pinctrl spi_pull_down %d!\n", ret);
        }

        spi_pull_up = pinctrl_lookup_state(spi_pinctrl, "fts_spi0_cs_pull_up");
        if (IS_ERR(spi_pull_up)) {
            ret = PTR_ERR(spi_pull_up);
            FTS_ERROR("Cannot find pinctrl spi_pull_up %d!\n", ret);
        }
        pinctrl_ok = true;
    }
#endif
...
#if 0//IS_ENABLED(CONFIG_CM_HARDWAREINFO_SUPPORT)//Leo 20210827
    {
        char name[128];
        u8 fwver = 0;
        ...
        if (fwver != 0) 
            sprintf(name,"%s ver:%02x",FTS_DRIVER_NAME, fwver);
...
#if IS_ENABLED(CONFIG_DRM_WB_EXTERN_ESD) //Leo 20231114
    mtk_drm_extern_esd_register(EXTERN_ESD_FTS_SPI, &mtk_drm_ext_esd_ops);
#endif
