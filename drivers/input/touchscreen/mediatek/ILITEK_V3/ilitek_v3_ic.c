...
#endif
	} else if (ilits->chip->core_ver >= CORE_VER_1700 && ilits->chip->core_ver < CORE_VER_2100) {
#if ENABLE_COMPRESS_MODE
		if (ilits->info_from_hex) {
			buf[1] = ilits->fw_info[0];
			goto out;
...
int ili_ic_get_all_info(void)
{
	...
	if (buf[0] == P5_X_GET_ALL_INFORMATION) {
		...
		if (buf[46] != cmd) {
			ILI_INFO("Invalid panel info, use default resolution\n");
			ilits->panel_wid = DISP_GetScreenHeight();//TOUCH_SCREEN_X_MAX;
			ilits->panel_hei = DISP_GetScreenWidth();//TOUCH_SCREEN_Y_MAX;
			ilits->trans_xy = OFF;
		} else {
