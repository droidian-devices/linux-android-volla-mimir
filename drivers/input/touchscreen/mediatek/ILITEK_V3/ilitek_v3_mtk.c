...
{
	if(ilits->pinctrl_ok){
		ILI_ERR("select spi_pull_up \n");
		pinctrl_select_state(ilits->spi_pinctrl, ilits->spi_pull_up);
	}
...
static void tpd_suspend(struct device *h)
{
	...
	if(ilits->pinctrl_ok){
		ILI_ERR("select spi_pull_down \n");
		pinctrl_select_state(ilits->spi_pinctrl, ilits->spi_pull_down);
	}
}
