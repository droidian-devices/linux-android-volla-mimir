...
static int ilitek_spi_probe(struct spi_device *spi)
{
    ...
    ilits->spi_pinctrl = devm_pinctrl_get(ilits->dev);
    if (IS_ERR(ilits->spi_pinctrl)) {
        ret = PTR_ERR(ilits->spi_pinctrl);
        ILI_ERR("Cannot find spi_pinctrl!\n");
        ilits->pinctrl_ok = false;
    } else {
       ilits->spi_pull_down = pinctrl_lookup_state(ilits->spi_pinctrl, "spi_pull_down");
        if (IS_ERR(ilits->spi_pull_down)) {
            ret = PTR_ERR(ilits->spi_pull_down);
            ILI_ERR("Cannot find pinctrl spi_pull_down %d!\n", ret);
        }

        ilits->spi_pull_up = pinctrl_lookup_state(ilits->spi_pinctrl, "spi_pull_up");
        if (IS_ERR(ilits->spi_pull_up)) {
            ret = PTR_ERR(ilits->spi_pull_up);
            ILI_ERR("Cannot find pinctrl spi_pull_up %d!\n", ret);
        }
        ilits->pinctrl_ok = true;
    }
