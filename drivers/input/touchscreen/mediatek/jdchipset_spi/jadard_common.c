...
const uint32_t fw_size = sizeof(jd_i_firmware);
#else
											...
											size_t len, loff_t *pos)
{
	...
	if (!proc_proximity_detect_flag) {
		buf_tmp = kzalloc(len * sizeof(char), GFP_KERNEL);
		if (buf_tmp != NULL) {
			ret += snprintf(buf_tmp + ret, len - ret, "Proximity_enable = %d\n", ts->proximity_enable);
			if (copy_to_user(buf, buf_tmp, len))
				JD_E("%s, copy_to_user fail: %d\n", __func__, __LINE__);
			kfree(buf_tmp);
		} else {
...
#else
#if IS_ENABLED(CONFIG_DRM_MEDIATEK) //Leo 20220505
	if (mtk_disp_notifier_register("Touch", &ts->fb_notif)) {
		pr_info("register mtk_disp_notifier_register fail!\n");
	}
#else
