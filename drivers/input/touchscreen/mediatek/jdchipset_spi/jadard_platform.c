...
static int jadard_regulator_configure(struct jadard_i2c_platform_data *pdata)
{
	...
	pdata->vcc_dig = regulator_get(pjadard_ts_data->dev, "vcc_dig");
	...
	pdata->vdd_ana = regulator_get(pjadard_ts_data->dev, "vdd_ana");

	if (IS_ERR(pdata->vdd_ana)) {
		JD_E("%s: Failed to get regulator avdd\n",
		  __func__);
		retval = PTR_ERR(pdata->vdd_ana);
...
static int jadard_power_on(struct jadard_i2c_platform_data *pdata, bool on)
{
	...
	if (on) {
		...
		msleep(100);
		retval = regulator_enable(pdata->vdd_ana);
...
#if IS_ENABLED(CONFIG_WB_TP_INCELL_SUPPORT)
        cust_gpio_ext_init("jadard_reset_gpio", pdata->gpio_reset);
#endif
	}
#endif
