...
#ifdef JD_ZERO_FLASH
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,14,0)
		kernel_write(fn, fw_buffer, g_common_variable.RAM_LEN * sizeof(uint8_t), &fn->f_pos);
#else
		kernel_write(fn, fw_buffer, g_common_variable.RAM_LEN * sizeof(uint8_t), fn->f_pos);
#endif
#else
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,14,0)
		kernel_write(fn, fw_buffer, g_common_variable.FW_SIZE * sizeof(uint8_t), &fn->f_pos);
#else
		kernel_write(fn, fw_buffer, g_common_variable.FW_SIZE * sizeof(uint8_t), fn->f_pos);
#endif
#endif
...
static struct proc_ops jadard_proc_display_std_ops  = {
	//.owner = THIS_MODULE,
	.proc_read = jadard_proc_display_std_read ,
