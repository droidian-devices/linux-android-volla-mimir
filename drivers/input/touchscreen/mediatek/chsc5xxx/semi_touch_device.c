...
static int input_device_init(struct sm_touch_dev *st_dev)
{
    ...
    st_dev->input->name = "mtk-tpd";//client->name;
	...
	//alter by LQ start
    input_set_abs_params(st_dev->input, ABS_MT_POSITION_X, 0,DISP_GetScreenWidth(), 0, 0);
    input_set_abs_params(st_dev->input, ABS_MT_POSITION_Y, 0,DISP_GetScreenHeight(), 0, 0);
	//alter by LQ end
