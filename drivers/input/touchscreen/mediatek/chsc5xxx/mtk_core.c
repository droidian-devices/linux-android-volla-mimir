...
int semi_touch_power_ctrl(unsigned char level)
{
    ...
    if(1 == level && 0 == power_status){
		...
		//add by LQ start 
        if(st_dev.pwr_pin > 0){
            semi_io_direction_out(st_dev.pwr_pin, 1);
        }		
		//add by LQ end
		...
		//add by LQ start 
        if(st_dev.pwr_pin > 0){
            semi_io_direction_out(st_dev.pwr_pin, 0);
        }		
		//add by LQ end
        if(st_dev.rst_pin > 0){
...
{
    int ret = SEMI_DRV_ERR_OK;
	struct device_node* of_node = NULL;//add by LQ
...
//add by LQ for get tp-vdd gpio control start
	
	of_node = of_find_matching_node(NULL, sm_of_match);

	st_dev.pwr_pin = of_get_named_gpio(of_node, "chipsemi,power-gpio", 0);
	printk("LQ >>> %s pwr_pin:%d \n",__func__,st_dev.pwr_pin);
	gpio_request(st_dev.pwr_pin, "chsc_pwr_pin");
...
#elif defined(CONFIG_DRM)  //add 20240712
...
static int semi_touch_fb_notifier_callback(struct notifier_block *self,unsigned long event, void *data)
{
	...
	if (data) {
		kernel_log_d("semi_touch_notifier_fb IN");
		...
		kernel_log_d("semi_touch_notifier_fb OUT");
	} else {
		kernel_log_d("semi_touch IC can not suspend or resume");
		return -1;
	}
	return 0;
}
