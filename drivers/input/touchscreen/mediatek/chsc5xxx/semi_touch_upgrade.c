...
int semi_touch_bootup_update_check(void)
{
    ...
    while(NULL != firmware)
    {
        ...
        for(iter = 0; (iter < udp_header->n_cfg) && (iter < 5); iter++)
        {
            ...
            if((st_dev.vid_pid & 0xffffff00) != (*(udp_header->vlist + iter) & 0xffffff00)) continue;
...
int semi_touch_online_update_check(char* file_path)
{
	...
	if (!file->f_op) {
		kernel_log_d("File Operation Method Error!!\n");
		return -1;
	}

    fpos = 0;
    file->f_op->llseek(file, 0, SEEK_SET);
    file->f_op->read(file, (char*)&file_sig, sizeof(file_sig), &fpos);
    ...
    file_sig = 0;
    file_size = file->f_op->llseek(file, 0, SEEK_END);
    file_buffer = kmalloc(file_size, GFP_KERNEL);
    
    fpos = 0;
    file->f_op->llseek(file, 0, SEEK_SET);
    if(file_size == file->f_op->read(file, (char*)file_buffer, file_size, &fpos))
    {
        semi_touch_start_up_check(&bootCheckOk, check_backup_if_fail);
