...
{
        struct goodix_ts_core *core_data = container_of(work, struct goodix_ts_core,
                resume_work);
        ...
        unsigned long event, void *data)
{
	...
	if (core_data && disp_status) {
		if (event == MTK_DISP_EARLY_EVENT_BLANK &&
			*disp_status == MTK_DISP_BLANK_POWERDOWN) {
			if (core_data->ts_workqueue) {
				...
				*disp_status == MTK_DISP_BLANK_UNBLANK) {
			if (core_data->ts_workqueue)
		...
		ts_info("goodix_ts touch can not suspend or resume");
		return -1;
	}
...
int goodix_ts_stage2_init(struct goodix_ts_core *cd)
{
	...
	/* resume workqueue */
	cd->ts_workqueue = create_singlethread_workqueue("ts_resume");
        if (!cd->ts_workqueue)
		ts_info("create goodix workqueue fail");
	else
