...
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20231009
...
static int fts_read_file(char *file_name, u8 **file_buf)
{
    ...
    *file_buf = (u8 *)vmalloc(file_len);
    ...
    set_fs(KERNEL_DS);
    pos = 0;
#if defined(DEBUG_BUILD) && defined(M101TB_DG_PT2_531) //Leo 20230927
...
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20231009
    if (csci_exist("touchpanel.fts.auto.update")) {
        FTS_INFO("FTS_AUTO_UPGRADE_EN is enable by csci");
    } else {
