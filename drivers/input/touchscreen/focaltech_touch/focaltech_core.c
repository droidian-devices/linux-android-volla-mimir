...
#if IS_ENABLED(CONFIG_CM_CUST_GPIOS_EXT_SUPPORT) //Leo 20230131
...
*****************************************************************************/
#define FTS_DRIVER_NAME                     "fts_ts_i2c"
#define FTS_DRIVER_PEN_NAME                 "fts_ts_i2c,pen"
...
//#define WB_PINCTRL_SUPPORT //just for debug 20230404

#if defined(WB_PINCTRL_SUPPORT) //Leo 20230401
struct pinctrl *i2c_pinctrl;
struct pinctrl_state *i2c_pull_down;
struct pinctrl_state *i2c_pull_up;
#endif
...
#if defined(CONFIG_TOUCHSCREEN_FTS_X_REVERSE) //Leo 20230105
...
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20230426
    int temp;
    temp = 0;
#endif
...
#if defined(CONFIG_TOUCHSCREEN_FTS_X_REVERSE) //Leo 20230105
    input_report_abs(pen_dev, ABS_X, data->pdata->x_max - pevt->x);
#else
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20230302
    if (csci_exist("touchpanel.gsl.x.reverse")) {
        temp = csci_integer("touchpanel.gsl.x.reverse",0);
...
#if defined(CONFIG_TOUCHSCREEN_FTS_Y_REVERSE) //Leo 20230105
    input_report_abs(pen_dev, ABS_Y, data->pdata->y_max - pevt->y);
#else
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20230302
    if (csci_exist("touchpanel.gsl.y.reverse")) {
        temp = csci_integer("touchpanel.gsl.y.reverse",0);
...
#if defined(M101TB_DG_PT2_531) //Leo 20240419
...
#if defined(WB_PINCTRL_SUPPORT) //Leo 20230401
   pr_info(" select i2c_pull_down \n");
   pinctrl_select_state(i2c_pinctrl, i2c_pull_down);
#endif
...
#if defined(WB_PINCTRL_SUPPORT) //Leo 20230401
    pr_info(" select i2c_pull_up \n");
    pinctrl_select_state(i2c_pinctrl, i2c_pull_up);
#endif
...
int fts_lic_get_ver_in_tp_1(u8 *ver)
{
    int ret = 0;
    *ver = 0;
    if (NULL == ver) {
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#if defined(WB_PINCTRL_SUPPORT) //Leo 20230327
    i2c_pinctrl = devm_pinctrl_get(ts_data->dev);
    if (IS_ERR(i2c_pinctrl)) {
        ret = PTR_ERR(i2c_pinctrl);
        dev_info(ts_data->dev, "Cannot find i2c_pinctrl!\n");
        //return ret;
    } else {
        i2c_pull_down = pinctrl_lookup_state(i2c_pinctrl, "i2c0_pull_down");
        if (IS_ERR(i2c_pull_down)) {
            ret = PTR_ERR(i2c_pull_down);
            dev_info(ts_data->dev,"Cannot find pinctrl i2c_pull_down %d!\n", ret);
        }

        i2c_pull_up = pinctrl_lookup_state(i2c_pinctrl, "i2c0_pull_up");
        if (IS_ERR(i2c_pull_up)) {
            ret = PTR_ERR(i2c_pull_up);
            dev_info(ts_data->dev,"Cannot find pinctrl i2c_pull_up %d!\n", ret);
        } 
    }
#endif
...
#if IS_ENABLED(CONFIG_CM_HARDWAREINFO_SUPPORT)//Leo 20210827
    {
        ...
        #define HW_TYPE_TP    2
        fts_lic_get_ver_in_tp_1(&ver);
        sprintf(name,"%s ver:0x%.2x",FTS_DRIVER_NAME, ver);
