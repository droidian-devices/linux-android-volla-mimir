...
#ifdef CONFIG_TOUCHSCREEN_JDCHIPSET_X_REVERSE
			input_report_abs(ts->input_dev, ABS_MT_POSITION_X, ts->pdata->abs_x_max - pjadard_host_data->x[i]);
#else
			if(csci_exist("touchpanel.jdchiptp.x.reverse")){
				if(csci_integer("touchpanel.jdchiptp.x.reverse",0))
					input_report_abs(ts->input_dev, ABS_MT_POSITION_X, (ts->pdata->abs_x_max -pjadard_host_data->x[i]));
				else
...
#endif
#ifdef CONFIG_TOUCHSCREEN_JDCHIPSET_Y_REVERSE
			input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, ts->pdata->abs_y_max - pjadard_host_data->y[i]);
#else
			if(csci_exist("touchpanel.jdchiptp.y.reverse")){
				if(csci_integer("touchpanel.jdchiptp.y.reverse",0))
					input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, (ts->pdata->abs_y_max -pjadard_host_data->y[i]));
				else
...
static const struct proc_ops jadard_proc_SMWP_ops = {
...
static const struct proc_ops jadard_proc_Gesture_ops = {
...
static const struct proc_ops jadard_proc_sorting_test_ops = {
	.proc_read = jadard_sorting_test_read,
};

#if defined(JD_OPPO_FUNC)
static const struct proc_ops jadard_proc_baseline_test_ops = {
	.proc_read = jadard_sorting_test_read,
};


static const struct proc_ops jadard_proc_black_test_ops = {
	.proc_read = jadard_sorting_test_read,
};
#endif
...
static const struct proc_ops jadard_proc_high_sensitivity_ops = {
...
static const struct proc_ops jadard_proc_rotate_border_ops = {
...
static const struct proc_ops jadard_proc_earphone_ops = {
...
static const struct proc_ops jadard_proc_proximity_ops = {
...
int jadard_chip_common_suspend(struct jadard_ts_data *ts)
{
	...
	atomic_set(&ts->suspend_mode, 1);
#endif
#ifdef CONFIG_WB_TP_INCELL_SUPPORT
	cust_gpio_set_value(CUST_GPIO_LCM_RST, 0);
	gpio_direction_output(pjadard_ts_data->rst_gpio, 0);
	mdelay(5);
	cust_gpio_set_value(CUST_GPIO_BIAS_NEG, 0);
	mdelay(5);
	cust_gpio_set_value(CUST_GPIO_BIAS_POS, 0);
	mdelay(5);
...
int jadard_chip_common_resume(struct jadard_ts_data *ts)
{
	...
	bool skip_fw_upgrade = false;
		...
		gpio_direction_output(pjadard_ts_data->rst_gpio, 1);
		mdelay(5);
		cust_gpio_set_value(CUST_GPIO_BIAS_POS, 1);
		mdelay(5);
		cust_gpio_set_value(CUST_GPIO_BIAS_NEG, 1);
		mdelay(5);
