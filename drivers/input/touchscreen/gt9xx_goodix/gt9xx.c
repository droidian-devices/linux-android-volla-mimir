...
static const char *goodix_ts_name = "mtk-tpd";
...
static int gtp_request_io_port(struct goodix_ts_data *ts)
{
...
#if IS_ENABLED(CONFIG_WB_TP_INCELL_SUPPORT) //Leo 20240820
#else
			if (gpio_is_valid(ts->pdata->irq_gpio))
	...
	if (gpio_is_valid(ts->pdata->power_gpio)) {
		ret = gpio_request(ts->pdata->power_gpio, "goodix_ts_power");
		if (ret < 0) {
			dev_err(&ts->client->dev,
				"Failed to request GPIO:%d, ERRNO:%d\n",
				(s32)ts->pdata->power_gpio, ret);
			return -ENODEV;
		}

		gpio_direction_output(ts->pdata->power_gpio, 1);
		gpio_set_value(ts->pdata->power_gpio, 1);
		dev_info(&ts->client->dev,  "Success request power-gpio\n");
	}
			...
			struct goodix_ts_platform_data *pdata)
{
	...
	of_property_read_u32(np, "goodix,reverse-x", &pdata->reverse_x);
	if (pdata->reverse_x)
		dev_info(dev, "reverse-x enabled\n");

	of_property_read_u32(np, "goodix,reverse-y", &pdata->reverse_y);
	if (pdata->reverse_y)
		dev_info(dev, "reverse-yx enabled\n");
	...
	pdata->power_gpio = of_get_named_gpio(np, "power-gpios", 0);
...
static int gtp_power_off(struct goodix_ts_data *ts)
{
	...
	// if (gpio_is_valid(ts->pdata->power_gpio)) {
	// 	set_bit(POWER_OFF_MODE, &ts->flags);
	// 	gpio_set_value(ts->pdata->power_gpio, 0);
	// }
...
static int gtp_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct goodix_ts_data *ts;
	struct goodix_ts_platform_data *pdata;
	int gtp_i2c_support = 0;

#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT)
	if (csci_exist("touchpanel.gtp.i2c")) {
		if (csci_integer("touchpanel.gtp.i2c", 0) > 0) {
			gtp_i2c_support = csci_integer("touchpanel.gtp.i2c", 0);
		}
	}
#endif

	if(gtp_i2c_support == 0){
		printk("do not support touchpanel gtp i2c %s\n", __func__);
		return -EINVAL;
	}
...
#if IS_ENABLED(CONFIG_CM_HARDWAREINFO_SUPPORT)//Leo 20210827
	{
		...
		ret = gtp_i2c_read_dbl_check(ts->client, GTP_REG_CONFIG_DATA,
							&cfg_var, 1);
		if (ret == SUCCESS) {
			sprintf(name,"%s  var:0x%02x ","goodix_ts",cfg_var);
		} else {
			sprintf(name,"%s","goodix_ts");
		}
...
#if defined(CONFIG_FB) || IS_ENABLED(CONFIG_DRM_MEDIATEK) //Leo 20240820
				    ...
				    unsigned long event, void *data)
{
	...
	dev_info(&ts->client->dev,"mtk disp notifier event:%lu, blank:%d\n", event, *blank);

	switch (event) {
		case MTK_DISP_EARLY_EVENT_BLANK://0x00
			if (*blank == MTK_DISP_BLANK_POWERDOWN) {
				dev_info(&ts->client->dev, "ts_suspend");
		...
		case MTK_DISP_EVENT_BLANK://0x01
			if (*blank == MTK_DISP_BLANK_UNBLANK) {
				dev_info(&ts->client->dev, "ts_resume");
				if (ts->pdata->resume_in_workqueue)
		...
		default:
				dev_info(&ts->client->dev,"nuknown event :%lu\n", event);
				break;
	}
...
#elif IS_ENABLED(CONFIG_DRM_MEDIATEK) //Leo 20240820
	INIT_WORK(&ts->fb_notify_work, fb_notify_resume_work);
	ts->notifier.notifier_call = gtp_fb_notifier_callback;
	if (mtk_disp_notifier_register("Touch", &ts->notifier))
		dev_err(&ts->client->dev,"register mtk_disp_notifier_register fail!\n");
#endif
...
#elif IS_ENABLED(CONFIG_DRM_MEDIATEK) //Leo 20240820
	mtk_disp_notifier_unregister(&ts->notifier);
#endif
...
#endif
#if !defined(CONFIG_FB) && defined(CONFIG_PM) && !IS_ENABLED(CONFIG_DRM_MEDIATEK)
		.pm		  = &gtp_pm_ops,
#endif
	},
};
