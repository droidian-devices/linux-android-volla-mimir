...
#include "ov02b10_mipi_raw_Sensor.h"


/****************************Modify Following Strings for Debug****************************/
#define PFX "OV02B10SUB_camera_sensor"
#define LOG_1 LOG_INF("OV02B10SUB,MIPI 1LANE\n")
...
//#define DEBUG_SENSOR_OV02B10SUB
#ifdef DEBUG_SENSOR_OV02B10SUB	//sp_zze
#include <asm/uaccess.h>
#include <linux/fs.h>
#include <linux/string.h>

#define OV02B10SUB_SD_FILE_PATH					"/mnt/sdcard/OV02B10SUB_sd"
#define OV02B10SUB_SD_PREVIEW					"/mnt/sdcard/OV02B10SUB_sd_preview"
#define OV02B10SUB_SD_CAPTURE					"/mnt/sdcard/OV02B10SUB_sd_capture"
//#define OV02B10SUB_SD_VIDEO						"/mnt/sdcard/OV02B10SUB_sd_video"
...
static U32 OV02B10SUB_initialize_from_t_flash(char *file_path)
{
	U8 *curr_ptr = NULL;
	U8 *tmp = NULL;
	U32 file_size = 0;
	...
	OV02B10SUB_init_regs_struct OV02B10SUB_reg;
	...
	while (curr_ptr < (data_buff + file_size)) {
		...
		if (!strcmp((const char *)func_ind, "REG")) {		/* REG */
			curr_ptr += 4;				/* Skip "REG{" */
			OV02B10SUB_reg.addr = sp_strtoul((const char *)curr_ptr, &tmp, 16);
			curr_ptr = tmp;
			curr_ptr ++;	/* Skip "," */
			OV02B10SUB_reg.val = sp_strtoul((const char *)curr_ptr, &tmp, 16);
			curr_ptr += 2;	/* Skip "};" */

			//init
			SP_I2C_WRITE(OV02B10SUB_reg.addr, OV02B10SUB_reg.val);
			regs_total++;
		} else if (!strcmp((const char *)func_ind, "DLY")) {	/* DLY */
			curr_ptr += 4;				/* Skip "DLY(" */
			OV02B10SUB_reg.val = sp_strtoul((const char *)curr_ptr, &tmp, 10);
			curr_ptr = tmp;

			SP_DELAY(OV02B10SUB_reg.val);
		}
...
kal_bool OV02B10SUBDuringTestPattern = KAL_FALSE;

static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV02B10_SENSOR_ID,

	.checksum_value = 0xb7c53a42,        //checksum value for Camera Auto Test
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	{   
		...
		if(iReg<=0x10)
		{
			...
			write_cmos_sensor(0xfe, 0x02);
			LOG_INF("OV02B10SUBMIPI_SetGain = 16");
		}
		else if(iReg>= 0xa0)//gpw
		{
			...
			write_cmos_sensor(0xfe, 0x02); 
			LOG_INF("OV02B10SUBMIPI_SetGain = 160"); 
		}        	
		else
		{
			write_cmos_sensor(0xfd, 0x01);
			...
			write_cmos_sensor(0xfe, 0x02);
			LOG_INF("OV02B10SUBMIPI_SetGain = %d",iReg);		 
		}	
	}	
	else
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x18, 0x01); //fixed the FPS 
	...
	write_cmos_sensor(0x7d, 0x03);
	write_cmos_sensor(0x7e, 0x03);
	write_cmos_sensor(0x7f, 0x0b);
	...
	write_cmos_sensor(0xfd, 0x01);	 
	write_cmos_sensor(0xfd, 0x03);
	write_cmos_sensor(0xc2, 0x01);// mipi_EN	    
...
static kal_uint32 open(void)
{
...
#ifdef DEBUG_SENSOR_OV02B10SUB
	if(read_from_extern_flash(OV02B10SUB_SD_FILE_PATH)) {
		SP_LOG("init from t!\n");
		OV02B10SUB_initialize_from_t_flash(OV02B10SUB_SD_FILE_PATH);
	} else {
		sensor_init();
	}
#else  
	...
	spin_unlock(&imgsensor_drv_lock);
	OV02B10SUBDuringTestPattern = KAL_FALSE;
	return ERROR_NONE;
}    /*    open  */
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	imgsensor.autoflicker_en = KAL_FALSE;
	spin_unlock(&imgsensor_drv_lock);
#ifndef DEBUG_SENSOR_OV02B10SUB
	preview_setting();
#endif
...
#if 0 //def DEBUG_SENSOR_OV02B10SUB
	if(read_from_extern_flash(OV02B10SUB_OTHER_DEBUG_PATH)) {
		printk("sp_zze: other debug!\n");
		OV02B10SUB_other_debug(OV02B10SUB_OTHER_DEBUG_PATH);
	}
#endif
