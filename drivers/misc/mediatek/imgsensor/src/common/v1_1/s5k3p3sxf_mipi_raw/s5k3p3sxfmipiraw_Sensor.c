...
#include "s5k3p3sxfmipiraw_Sensor.h"

#define PFX "S5K3P3SXF_camera_sensor"
...
#ifdef CONFIG_S5K3P3SXF_MIRROR_HV
...
{{ 2320, 1748,	  0,	0, 2320, 1748, 2320, 1748, 0, 0, 2320, 1748, 0,	0, 2320, 1748}, // Preview
 ...
 { 2320, 1748, 200,	334, 1920, 1080, 1920, 1080, 0, 0, 1920, 1080, 0, 0, 1920, 1080}};// slim video
...
{
#if IS_ENABLED(CONFIG_WB_BOARD_ID_SUPPORT)
	if(cust_midmisc_get_board_id() == 0){
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	kal_uint8 itemp;
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20220909
    if (csci_exist("camera.mirror.support.s5k3p3sxf")) {
        if (csci_integer("camera.mirror.support.s5k3p3sxf", 0) > 0) {
...
{
		//LOG_INF("s5k3p3sxf %s E! currefps:%d\n", __func__, currefps);
		// full size 29.76fps
		// capture setting
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id ==imgsensor_info.sensor_id) {
				printk("s5k3p3sxf i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				return ERROR_NONE;
			}
			printk("s5k3p3sxf  Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 5;
...
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20220909
    if (csci_exist("camera.output.dataformat.s5k3p3sxf")) {
        sensor_info->SensorOutputDataFormat = csci_integer("camera.output.dataformat.s5k3p3sxf", 0);
    }
#endif
    ...
    case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
        *(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 3000000;
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:	
			...
			read_3P3_eeprom_v2((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;			
...
//kin0603
UINT32 S5K3P3SXF_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
