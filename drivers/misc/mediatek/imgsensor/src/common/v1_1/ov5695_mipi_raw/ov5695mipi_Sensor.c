...
int read_otp(struct otp_struct *otp_ptr)
{
	...
	if((otp_flag & 0xc0) == 0x40) {
    	addr = 0x700D; // base address of WB Calibration gro
	}
	else if((otp_flag & 0x30) == 0x10) {
    	addr = 0x7012; // base address of WB Calibration grou
	}
	...
	LOG_INF("flag:0x%x, module_integrator_id:0x%x, lens_id:0x%x, rg_ratio:0x%x, bg_ratio:0x%x\n", 
		(*otp_ptr).flag, (*otp_ptr).module_integrator_id, (*otp_ptr).lens_id, (*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio);
...
{
	int RG_Ratio_Typical = 0x13C, BG_Ratio_Typical = 0x131;
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	switch (image_mirror) {
		case IMAGE_NORMAL:
			write_cmos_sensor(0x3820,((read_cmos_sensor(0x3820) & 0xE1)));
...
write_cmos_sensor(0x0302,0x69);      //7d  2017.1.9
...
write_cmos_sensor(0x3811,0x00);  //06
...
write_cmos_sensor(0x4837,0x13);      // 10  2017.1.9
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			...
			printk("ov5695 Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	struct otp_struct *otp_ptr =
				kmalloc(sizeof(struct otp_struct), GFP_KERNEL);
	#endif
