...
#include "sc520csb_mipi_raw.h"

#define PFX "SC520CSB_mipi_raw"
...
static kal_uint16 sc520csb_table_write_cmos_sensor(
...
{
	return ((read_cmos_sensor(0x3107) << 8) | read_cmos_sensor(0x3108)) - 1; //0xee4b
}
...
#define SC520CSB_SENSOR_GAIN_BASE             0x400
#define SC520CSB_SENSOR_GAIN_MAX              (160 * SC520CSB_SENSOR_GAIN_BASE / 10)
#define SC520CSB_SENSOR_GAIN_MAP_SIZE  6

static kal_uint16 gain2reg(const kal_uint16 gain)
{
	kal_uint16 reg_gain = gain << 4;

	if (reg_gain < SC520CSB_SENSOR_GAIN_BASE)
		reg_gain = SC520CSB_SENSOR_GAIN_BASE;
	else if (reg_gain > SC520CSB_SENSOR_GAIN_MAX)
		reg_gain = SC520CSB_SENSOR_GAIN_MAX;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 SC520CSB_AGC_Param[SC520CSB_SENSOR_GAIN_MAP_SIZE][2] = {
		{  1024,  0x00 },
		{  2048,  0x08 },
		{  4096,  0x09 },
		{  8192,  0x0B },
	...
	for (gain_index = SC520CSB_SENSOR_GAIN_MAP_SIZE - 1; gain_index > 0; gain_index--)
		if (reg_gain >= SC520CSB_AGC_Param[gain_index][0])
			break;

	write_cmos_sensor(0x3e09, SC520CSB_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * SC520CSB_SENSOR_GAIN_BASE / SC520CSB_AGC_Param[gain_index][0];
	write_cmos_sensor(0x3e07, (temp_gain >> 3) & 0xff);

	LOG_INF("Exit! SC520CSB_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, gain = 0x%x, reg_gain = %d\n",
		SC520CSB_AGC_Param[gain_index][1], temp_gain, gain, reg_gain);
...
//static kal_uint16 read_cmos_sensor_sc520csb(kal_uint32 addr)
//{
//	kal_uint16 get_byte = 0;
//	char pu_send_cmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF) };
...
static void get_sensor_module_info(void)
{
    ...
    for (i = 0; i < 15; i++) {
        calmoduleversion[i] = read_cmos_sensor_sc520csb(0x01+i);
        //pr_err("%s %d sc520csb module_id[%d]=0x%2x\n",__func__, __LINE__, i, calmoduleversion[i]);
    }
			...
			do {
				*sensor_id =return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {
					printk("sc520csb i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);

					return ERROR_NONE;
				}
				printk("sc520csb get_imgsensor_id Read sensor id fail, i2c write id: 0x%x,sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				retry--;
			} while(retry > 0);
			i++;
			retry = 2;
...
static kal_uint32 open(void)
{
	...
	if (imgsensor_info.sensor_id != sensor_id) {
		printk("sc520csb open sensor id fail: 0x%x\n", sensor_id);
...
UINT32 SC520CSB_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc =  &sensor_func;
	return ERROR_NONE;
}	/*	SC520CSB_MIPI_RAW_SensorInit	*/
