 ...
 *============================================================================
 *
 * Version:  V20240131145650 by GC-S-TEAM
 *
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
  { 3264, 2448, 0, 0, 3264, 2448, 1632, 1224, 0, 0, 1632, 1224, 0, 0, 1632, 1224}, /* Preview */
	...
	{ 3264, 2448, 0, 0, 3264, 2448, 1632, 1224, 176, 252, 1280, 720, 0, 0, 1280, 720}, /* hs video */
...
static void gc08a8_stream_on(void)
{
	write_cmos_sensor_8bit(0x0100, 0x01);
}

static void gc08a8_stream_off(void)
{
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				printk("[gc08a8_camera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			printk("[gc08a8_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
