...
#include "sc500csmipi_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "SC500CS_camera_sensor"
#define LOG_1 LOG_INF("SC500CS,MIPI 2LANE\n")
/****************************   Modify end    *******************************************/
#define SC500CS_DEBUG 0
#if SC500CS_DEBUG
...
#ifdef CONFIG_SC500CS_MIRROR_HV
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
 ...
 { 2592, 1944,	  0,	0, 2592, 1944, 1296,  972,    0,    0, 1296,  972,    0,	0, 1296,  972}, // normal video 
 { 2592, 1944,   16,  252, 2560, 1440, 1280,  720,    0,    0, 1280,  720,    0,    0, 1280,  720}, // highspeed mode video
...
static void write_shutter(kal_uint16 shutter)
{
	...
	LOG_INF("gsl_debug shutter =%d, framelength =%d,ExpReg 0x3e20=%x, 0x3e00=%x, 0x3e01=%x, 0x3e02=%x\n", 
		shutter,imgsensor.frame_length,(shutter >> 20) & 0x0F,(shutter >> 12) & 0xFF,(shutter >> 4)&0xFF, (shutter<<4) & 0xF0); 
}   /*  write_shutter  */
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (reg_gain < SC500CS_SENSOR_GAIN_BASE)
		reg_gain = SC500CS_SENSOR_GAIN_BASE;
	else if (reg_gain > SC500CS_SENSOR_GAIN_MAX)
		reg_gain = SC500CS_SENSOR_GAIN_MAX;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 SC500CS_AGC_Param[SC500CS_SENSOR_GAIN_MAP_SIZE][2] = {
		{  1024,  0x03 },
		{  2048,  0x07 },
		{  4096,  0x0f },
		{  8192,  0x1f },
	...
	for (gain_index = SC500CS_SENSOR_GAIN_MAX_VALID_INDEX - 1; gain_index >= 0; gain_index--)
		if (reg_gain >= SC500CS_AGC_Param[gain_index][0])
			break;

	write_cmos_sensor(0x3e08, SC500CS_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * SC500CS_SENSOR_GAIN_BASE / SC500CS_AGC_Param[gain_index][0];
	write_cmos_sensor(0x3e09, (temp_gain >> 5) & 0xff);
	
	LOG_INF("Exit! SC500CS_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, gain = 0x%x, reg_gain = %d, gain = %d\n",
		SC500CS_AGC_Param[gain_index][1], temp_gain, gain, reg_gain, gain);
...
{
	LOG_INF("E:sensor_init:SC500CS_setting_V01P12 ");
...
{
	LOG_INF("Enter = preview_setting! currefps:%d\n",currefps);
	write_cmos_sensor(0x0103, 0x01);
	mDELAY(1);
	write_cmos_sensor(0x0100, 0x00);
	write_cmos_sensor(0x36e9, 0x80);
...
{
	LOG_INF("Enter = normal_video_setting! currefps:%d\n",currefps);
	write_cmos_sensor(0x0103, 0x01);
	mDELAY(1);
	write_cmos_sensor(0x0100, 0x00);
	write_cmos_sensor(0x36e9, 0x80);
...
{
	LOG_INF("Enter = hs_video_setting! currefps:%d\n",currefps);
	write_cmos_sensor(0x0103, 0x01);
	mDELAY(1);
	write_cmos_sensor(0x0100, 0x00);
	write_cmos_sensor(0x36e9, 0x80);
...
{
	LOG_INF("Enter = slim_video_setting! currefps:%d\n",currefps);
	write_cmos_sensor(0x0103, 0x01);
	mDELAY(1);
	write_cmos_sensor(0x0100, 0x00);
	write_cmos_sensor(0x36e9, 0x80);
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {               
                printk("sc500cs i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);      
                return ERROR_NONE;
            }   
            printk("sc500cs Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
UINT32 SC500CS_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
