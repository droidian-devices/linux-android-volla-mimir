...
BYTE S5K3P9SPF_eeprom_data[DATA_SIZE]= {0};
...
static bool _read_eeprom(kal_uint16 addr, kal_uint32 size ){
	...
	for(i=0; i<DATA_SIZE1; i++){
		S5K3P9SPF_eeprom_data[i] = read_cmos_sensor_byte(START_ADDR1+i);
		LOG_INF("add = 0x%x,\tvalue = 0x%x\n",START_ADDR1+i, S5K3P9SPF_eeprom_data[i]);
	}
	
	for(j=0; j<DATA_SIZE2; j++){
		S5K3P9SPF_eeprom_data[i+j] = read_cmos_sensor_byte(START_ADDR2+j);
		LOG_INF("add = 0x%x,\tvalue = 0x%x\n",START_ADDR2+j, S5K3P9SPF_eeprom_data[i+j]);
	}
	
	return true;
}

bool S5K3P9SPF_read_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size){
	//addr = START_ADDR;
	//size = DATA_SIZE;
	...
	memcpy(data, S5K3P9SPF_eeprom_data, size);
	return true;
}
