...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.i2c_addr_table = {0x20, 0x6c, 0xff}, /* 0x6c no need for 8168 evb */
};
...
static int capture_first_flag;
static int pre_currefps;
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("ov13855 i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			printk("ov13855 Read sensor id fail:0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id,
				*sensor_id);
			retry--;
		} while (retry > 0);
