...
#define LOG_INF(fmt, args...)   printk(PFX "[%s] " fmt, __func__, ##args)
#define LOGE(fmt, args...)   printk(PFX "[%s] " fmt, __func__, ##args)
...
static imgsensor_info_struct imgsensor_info = {
	...
	.exp_step = 2,
    .gain_step = 1,
	.gain_type = 0,			//sensor framelength & shutter margin
	.min_shutter = 2,
	...
	.mipi_settle_delay_mode = MIPI_SETTLEDELAY_AUTO,	/* 0,MIPI_SETTLEDELAY_AUTO;
								 ...
								 */
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gb, /*B */ //  SENSOR_OUTPUT_FORMAT_RAW_Gr,
	.mclk = 24,
...
#ifdef CONFIG_S5K3M2_MIRROR_HV
	.mirror = IMAGE_HV_MIRROR,
#else	
	.mirror = IMAGE_NORMAL,	/* mirrorflip information */
#endif // CONFIG_S5K3M2_MIRROR_HV
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {
	...
	{4192, 3104, 0, 0, 4192, 3104, 4192, 3104, 0000, 0000, 4192, 3104, 0, 0, 4192, 3104},	/* slim video */
...
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20220909
    if (csci_exist("camera.mirror.support.s5k3m2")) {
        if (csci_integer("camera.mirror.support.s5k3m2", 0) > 0) {
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	#if IS_ENABLED(CONFIG_WB_IMGSENSOR_INDEX_SUPPORT) //Leo 20211108
	if (IMGSENSOR_SENSOR_IDX_MAIN != cust_get_imgsensor_alive_idx()) {
		...
		do {
			...
			*sensor_id = read_cmos_sensor(0x6F12);
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("s5k3m2 i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			LOG_INF("s5k3m2 Read sensor id fail, write id: 0x%x, sensor id = 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20220909
    if (csci_exist("camera.output.dataformat.s5k3m2")) {
        sensor_info->SensorOutputDataFormat = csci_integer("camera.output.dataformat.s5k3m2", 0);
    }
#endif
