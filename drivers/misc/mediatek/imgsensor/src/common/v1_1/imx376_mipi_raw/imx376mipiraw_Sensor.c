...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info = {
	...
	.i4SubBlkH = 8,
	.i4PosL = {
		{9, 13},{17, 13},{13, 25},{21, 25} },	  
	.i4PosR = {
		{9, 17},{17, 17},{13, 21},{21, 21} },
	.i4BlockNumX = 161,
	.i4BlockNumY = 120,
};
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0016) << 8) |
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("imx376 i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);

			/*zhaozhengtao 2016/02/19,modify for different module*/
				imgsensor_info.module_id = read_module_id();

				printk("imx376 IMX376_module_id=%d\n",
					imgsensor_info.module_id);
				return ERROR_NONE;
			}
			printk("imx376 Read sensor id fail, i2c_write_id: 0x%x  sensor_id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id =
				...
				read_cmos_sensor_8(0x0017));
			if (sensor_id == imgsensor_info.sensor_id) {
				printk("imx376 open i2c write id:0x%x, sensor id:0x%x\n",
			imgsensor.i2c_write_id, sensor_id);
			break;
		}
			printk("imx376 open Read sensor id fail, id: 0x%x sensor_id=0x%x\n",
			imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
{
	int i = 0;
	int addr = 0x078A;/*Start of 4 cell data*/
