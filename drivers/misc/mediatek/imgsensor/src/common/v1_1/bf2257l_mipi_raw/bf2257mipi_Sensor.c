...
//#ifdef CONFIG_BF2257L_MIRROR_HV
...
#if 1 //Leo 
	write_cmos_sensor(0x00, 0x01);//0x01);//Bit[3]: Mirror Bit[2]: vFlip
#else
	write_cmos_sensor(0x00, 0x09);//0x01);//Bit[3]: Mirror Bit[2]: vFlip
#endif
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			//if (cust_get_imgsensor_alive_idx() != 3) {
				retry--;
			//}
		} while (retry > 0);
		i++;
...
UINT32 BF2257L_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
