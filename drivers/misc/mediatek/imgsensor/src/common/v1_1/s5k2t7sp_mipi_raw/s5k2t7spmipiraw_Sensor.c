...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	{ 5200, 3880,    0,    		0, 5184, 3880, 2592, 1940,	 0,    0, 2592, 1940,    0,    0, 2592, 1940},
	{ 5200, 3880,    0,    		0, 5184, 3880, 5184, 3880,	 0,    0, 5184, 3880,    0,    0, 5184, 3880},
	{ 5200, 3880,    0,    		0, 5184, 3880, 2592, 1940,	 0,    0, 2592, 1940,    0,    0, 2592, 1940},
	{ 5200, 3880,    1312,   1228, 2560, 1440, 1280,  720,   0,    0, 1280,  720,    0,    0, 1280,  720},
	{ 5200, 3880,    1320,   1220, 2560, 1440, 1280,  720,   0,    0, 1280,  720,    0,    0, 1280,  720},
};
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		(read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001));
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("s5k2t7sp i2c write id: 0x%x, sensor id: 0x%x\n",
		 ...
		 */
			...
			printk("s5k2t7sp Read sensor id fail, id: 0x%x\n",
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		(read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001));
			...
			if (sensor_id == imgsensor_info.sensor_id) {
				printk("s5k2t7sp open i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}

			printk("s5k2t7sp open Read sensor id fail, id: 0x%x\n",
				imgsensor.i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
