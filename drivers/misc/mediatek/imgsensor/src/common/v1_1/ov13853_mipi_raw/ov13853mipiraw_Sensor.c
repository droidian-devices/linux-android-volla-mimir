...
static void set_mirror_flip(kal_uint8 image_mirror)
{
    ...
    /********************************************************
       ...
       ********************************************************/
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20220909
    if (csci_exist("camera.ov13853.mirror")) {
        if (csci_integer("camera.ov13853.mirror", 0) == 1) {
            image_mirror = IMAGE_HV_MIRROR;
        }else if(csci_integer("camera.ov13853.mirror", 0) == 2){
            image_mirror = IMAGE_H_MIRROR;
        }else if(csci_integer("camera.ov13853.mirror", 0) == 3){
            image_mirror = IMAGE_V_MIRROR;
        }else{
            image_mirror = IMAGE_NORMAL;
        }

        LOG_INF("ov13853 image_mirror = %d\n", image_mirror);
    }
#endif
...
int ov13853_capture_first_flag = 0;
int ov13853_pre_currefps = 0;
static void capture_setting(kal_uint16 currefps)
{
    LOG_INF("E! currefps:%d\n",currefps);	
if (ov13853_pre_currefps != currefps)
{
	ov13853_capture_first_flag = 0;
	ov13853_pre_currefps = currefps;
}
else
{
	ov13853_capture_first_flag = 1;
}
if (ov13853_capture_first_flag == 0)
{
        ...
        do {
            *sensor_id = return_sensor_id();
            ...
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("LQ >>>  ov13853 getid ok i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("LQ ov13853 getid fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            //sensor_id = imgsensor_info.sensor_id;
            if (sensor_id == imgsensor_info.sensor_id) {
                printk("yaoxu ov13853 open ok i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            printk("yaoxu ov13853 open fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
