 ...
 *	PengtaoFan
 *  0528:?????¡À1get_info,??mipi ??? *  0604:??¨¦?init preview ?????a???0ms
 *  0604:??-init setting???stream on,?otp??¡§¡§??
 *  0608:????ov???setting
 *  0703:for  ?????¡À1?non continue mode 4800
 *  0703:for  crc test 0x5002=00
 *  ---RD ??????? *  0714 ??¡ê¡ècapture setting full size@30fps 24fps 15fps
 *  15072115172729: ??¡ê¡ènick??full size 30fps setting???for ???size ??????¡§|??
 *------------------------------------------------------------------------------
...
#if defined(CONFIG_OV8858_MIRROR_HV) //Leo 20210826
	#define OV8858_MIRROR_20		0x00
	#define OV8858_MIRROR_21		0x06
#else
	#define OV8858_MIRROR_20		0x00
	#define OV8858_MIRROR_21		0x06
#endif
...
printk("dsy log cam : ov8858mipiraw_Sensor.c  %s %d \n",__FUNCTION__,__LINE__);
	tosend = 0;
	IDX = 0;
	while (len > IDX) {
		addr = para[IDX];
...
static kal_uint16 read_cmos_sensor(kal_uint32 addr)
{
	...
	printk("dsy log : cam ov8858mipiraw_Sensor.c  %s %d get_byte =%d\n",__FUNCTION__,__LINE__,get_byte);
	return get_byte;
}
...
kal_uint16 addr_data_pair_init_ov8858[] = {
	...
	0x3814, 0x03,
	0x3815, 0x01,
	#if IS_ENABLED(CONFIG_OV8858_MIRROR_HV) //Leo 20210825  0 6/ 0 0/6 0/ 6 6
	0x3820, (0x00 & ~0x06) | OV8858_MIRROR_20,
	0x3821, (0x67 & ~0x06) | OV8858_MIRROR_21,
	#else
	0x3820, 0x06,		/* mirror */
...
kal_uint16 addr_data_pair_preview_ov8858[] = {
	...
	0x380f, 0xb8,
	0x3814, 0x03,
	#if IS_ENABLED(CONFIG_OV8858_MIRROR_HV) //Leo 20210825
	0x3820, (0x00 & ~0x06) | OV8858_MIRROR_20,
	0x3821, (0x67 & ~0x06) | OV8858_MIRROR_21,
	#else
	0x3821, 0x61,
...
kal_uint16 addr_data_pair_capture_30fps_ov8858[] = {
	...
	0x380f, 0xe2,
	0x3814, 0x01,
	#if IS_ENABLED(CONFIG_OV8858_MIRROR_HV) //Leo 20210826
	0x3821, (0x46 & ~0x06) | OV8858_MIRROR_21,
	#else
	0x3821, 0x40,
	#endif
	0x382a, 0x01,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	 ...
	 */
printk("dsy log : cam ov8858mipiraw_Sensor.c 1111 %s %d *sensor_id = 0x%x imgsensor_info.sensor_id = 0x%x\n",__FUNCTION__,__LINE__,*sensor_id ,imgsensor_info.sensor_id);
	...
	printk("dsy log : cam ov8858mipiraw_Sensor.c 222 %s %d *sensor_id = 0x%x imgsensor_info.sensor_id = 0x%x\n",__FUNCTION__,__LINE__,*sensor_id ,imgsensor_info.sensor_id);

	  pr_debug("cam: i2c write id: 0x%x, id: 0x%x, version = %d(0=r2a,1=r1a)\n",
			...
			return ERROR_NONE;
				...
				printk("dsy log : cam ov8858mipiraw_Sensor.c 333 %s %d *sensor_id = 0x%x imgsensor_info.sensor_id = 0x%x\n",__FUNCTION__,__LINE__,*sensor_id ,imgsensor_info.sensor_id);
				pr_debug("=R1A sensor,contact OV please!=\n");

					return ERROR_SENSOR_CONNECT_FAIL;
		}
		printk("dsy log : cam ov8858mipiraw_Sensor.c 4444 %s %d *sensor_id = 0x%x imgsensor_info.sensor_id = 0x%x\n",__FUNCTION__,__LINE__,*sensor_id ,imgsensor_info.sensor_id);
				pr_debug("read ov8858 R1A R2A bate fail\n");
				return ERROR_SENSOR_CONNECT_FAIL;

		}
printk("dsy log : cam ov8858mipiraw_Sensor.c 555 %s %d *sensor_id = 0x%x imgsensor_info.sensor_id = 0x%x\n",__FUNCTION__,__LINE__,*sensor_id ,imgsensor_info.sensor_id);
