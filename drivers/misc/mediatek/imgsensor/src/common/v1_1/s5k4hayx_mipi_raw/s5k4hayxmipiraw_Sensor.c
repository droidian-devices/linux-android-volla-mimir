...
static struct imgsensor_info_struct imgsensor_info = { 
	...
	.cap1 = {
		.pclk = 280000000,	/* record different mode's pclk */
		.linelength = 3688,	/* record different mode's linelength */
		.framelength = 5060,	/* record different mode's framelength */
		.startx = 0,	/* record different mode's startx of grabwindow */
		.starty = 0,	/* record different mode's starty of grabwindow */
		.grabwindow_width = 3264,	/* record different mode's width of grabwindow */
		.grabwindow_height = 2448,	/* record different mode's height of grabwindow */
	...
	.cap2 = {
		.pclk = 280000000,	/* record different mode's pclk */
		.linelength = 3688,	/* record different mode's linelength */
		.framelength = 3163,	/* record different mode's framelength */
		.startx = 0,	/* record different mode's startx of grabwindow */
		.starty = 0,	/* record different mode's starty of grabwindow */
		.grabwindow_width = 3264,	/* record different mode's width of grabwindow */
		.grabwindow_height = 2448,	/* record different mode's height of grabwindow */
...
static void sensor_init(void)
  {
	LOG_INF("E\n");
	printk("cam [Kernel/s5k4hayx]: %s %d  \n", __func__,__LINE__);
	/* +++++++++++++++++++++++++++// */
	/* Reset for operation */
	/* Streaming off */
	write_cmos_sensor(0x0100, 0x00);  //	Streaming off
	//Global Setting		

	write_cmos_sensor(0x0A02, 0x7F);
	write_cmos_sensor(0x3B45, 0x01);
	write_cmos_sensor(0x3264, 0x01);
	write_cmos_sensor(0x3290, 0x10);
	write_cmos_sensor(0x0B05, 0x01);
	write_cmos_sensor(0x3069, 0xC7);
	write_cmos_sensor(0x3074, 0x06);
	write_cmos_sensor(0x3075, 0x32);
	write_cmos_sensor(0x3068, 0xF7);
	write_cmos_sensor(0x30C6, 0x01);
	write_cmos_sensor(0x301F, 0x20);
	write_cmos_sensor(0x306B, 0x9A);
	write_cmos_sensor(0x3091, 0x1F);
...
{	
	...
	write_cmos_sensor(0x3929, 0x3F);
	write_cmos_sensor(0x0101, 0x00);
	write_cmos_sensor(0x3941, 0x0E);
	write_cmos_sensor(0x3942, 0x11);
...
static void capture_setting(kal_uint16 currefps)
{
	...
	write_cmos_sensor(0x030F, 0xA7);                  
	write_cmos_sensor(0x3C17, 0x00);                  
	write_cmos_sensor(0x0340, 0x13);                  
	write_cmos_sensor(0x0341, 0xC4);  //15fps  5060   
	write_cmos_sensor(0x0342, 0x0E);                  
	write_cmos_sensor(0x0343, 0x68);  //3688          
	write_cmos_sensor(0x0344, 0x00);                  
	write_cmos_sensor(0x0345, 0x08);                  
	write_cmos_sensor(0x0346, 0x00);                  
	write_cmos_sensor(0x0347, 0x08);                  
	...
	write_cmos_sensor(0x0200, 0x0D);                  
	write_cmos_sensor(0x0201, 0xD8);                  
	write_cmos_sensor(0x0202, 0x13);  //66ms          
	...
	write_cmos_sensor(0x0341, 0x5B);//3163
	write_cmos_sensor(0x0342, 0x0E);
	write_cmos_sensor(0x0343, 0x68);//3688
	write_cmos_sensor(0x0344, 0x00);
	write_cmos_sensor(0x0345, 0x08);
