...
#include "gc8034f_mipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC8034F_camera_sensor"
#define LOG_1 LOG_INF("GC8034F,MIPI 2LANE\n")
/****************************   Modify end    *******************************************/
#define GC8034F_DEBUG
#if defined(GC8034F_DEBUG)
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC8034F_SENSOR_ID,       /*record sensor id defined in Kd_imgsensor.h*/
...
static struct gc8034f_otp_t gc8034f_otp_info = {0};

static kal_uint8 gc8034f_read_otp(kal_uint8 page, kal_uint8 addr)
{
	LOG_INF("E!\n");
...
static void gc8034f_read_otp_kgroup(kal_uint8 page,
...
#if defined(GC8034FOTP_FOR_CUSTOMER)
static void gc8034f_read_otp_group(kal_uint8 page, kal_uint8 addr,
...
static void gc8034f_gcore_read_otp_info(void)
{
	kal_uint8  flagdd = 0;
	kal_uint16 i = 0, j = 0;
	kal_uint8  temp = 0;
	...
	memset(&gc8034f_otp_info, 0, sizeof(struct gc8034f_otp_t));

	/* Static Defective Pixel */
	flagdd = gc8034f_read_otp(0, 0x0b);
	LOG_INF("GC8034F_OTP_DD : flag_dd = 0x%x\n", flagdd);

	switch (flagdd & 0x03) {
	case 0x00:
		LOG_INF("GC8034F_OTP_DD is Empty !!\n");
		gc8034f_otp_info.dd_flag = 0x00;
		break;
	case 0x01:
		LOG_INF("GC8034F_OTP_DD is Valid!!\n");
		total_number = gc8034f_read_otp(0, 0x0c) + gc8034f_read_otp(0,
				0x0d);
		gc8034f_otp_info.dd_flag = 0x01;
		gc8034f_read_otp_kgroup(0, 0x0e, &ddtempbuff[0],
				       4 * total_number);
		for (i = 0; i < total_number; i++) {
			if ((ddtempbuff[4 * i + 3] & 0x80) == 0x80) {
				if ((ddtempbuff[4 * i + 3] & 0x03) == 0x03) {
					gc8034f_otp_info.dd_param[cnt].x = (((kal_uint16)ddtempbuff[4 * i
									    + 1] & 0x0f) << 8) + ddtempbuff[4 * i];
					gc8034f_otp_info.dd_param[cnt].y = ((kal_uint16)ddtempbuff[4 * i +
									   2] << 4) + ((ddtempbuff[4 * i + 1] & 0xf0) >> 4);
					gc8034f_otp_info.dd_param[cnt++].t = 2;
					gc8034f_otp_info.dd_param[cnt].x = (((kal_uint16)ddtempbuff[4 * i
									    + 1] & 0x0f) << 8) + ddtempbuff[4 * i];
					gc8034f_otp_info.dd_param[cnt].y = ((kal_uint16)ddtempbuff[4 * i +
					...
					gc8034f_otp_info.dd_param[cnt].y = ((kal_uint16)ddtempbuff[4 * i +
									   2] << 4) + ((ddtempbuff[4 * i + 1] & 0xf0) >> 4);
					gc8034f_otp_info.dd_param[cnt++].t = ddtempbuff[4 * i + 3] & 0x03;
				}
			}
		}
		gc8034f_otp_info.dd_cnt = cnt;
		LOG_INF("GC8034F_OTP : total_number = %d\n",
			gc8034f_otp_info.dd_cnt);
		break;
	case 0x02:
	case 0x03:
		LOG_INF("GC8034F_OTP_DD is Invalid !!\n");
		gc8034f_otp_info.dd_flag = 0x02;
		break;
	default:
		break;
	}
...
#if defined(GC8034FOTP_FOR_CUSTOMER)
	flag_module = gc8034f_read_otp(9, 0x6f);
	flag_wb = gc8034f_read_otp(9, 0x5e);
	flag_af = gc8034f_read_otp(3, 0x3a);
	LOG_INF("GC8034F_OTP : flag_module = 0x%x , flag_wb = 0x%x , flag_af = 0x%x\n",
	...
	for (index = 0; index < 2; index++) {
		...
		case 0x00:
			LOG_INF("GC8034F_OTP_INFO group %d is Empty !!\n", index + 1);
			break;
		case 0x04:
			LOG_INF("GC8034F_OTP_INFO group %d is Valid !!\n", index + 1);
			check = 0;
			gc8034f_read_otp_group(9, INFO_ROM_START + index * INFO_WIDTH,
					      &info[0], INFO_WIDTH);
			for (i = 0; i < INFO_WIDTH - 1; i++)
				check += info[i];

			if ((check % 255 + 1) == info[INFO_WIDTH - 1]) {
				gc8034f_otp_info.module_id = info[0];
				gc8034f_otp_info.lens_id = info[1];
				gc8034f_otp_info.vcm_driver_id = info[2];
				gc8034f_otp_info.vcm_id = info[3];
				gc8034f_otp_info.year = info[4];
				gc8034f_otp_info.month = info[5];
				gc8034f_otp_info.day = info[6];
			} else
				LOG_INF("GC8034F_OTP_INFO Check sum %d Error !!\n", index + 1);
			break;
		case 0x08:
		case 0x0c:
			LOG_INF("GC8034F_OTP_INFO group %d is Invalid !!\n", index + 1);
			break;
		default:
			break;
		}
		...
		case 0x00:
			LOG_INF("GC8034F_OTP_WB group %d is Empty !!\n", index + 1);
			gc8034f_otp_info.wb_flag = gc8034f_otp_info.wb_flag | 0x00;
			break;
		case 0x04:
			LOG_INF("GC8034F_OTP_WB group %d is Valid !!\n", index + 1);
			check = 0;
			gc8034f_read_otp_group(9, WB_ROM_START + index * WB_WIDTH, &wb[0],
					      WB_WIDTH);
			for (i = 0; i < WB_WIDTH - 1; i++)
				check += wb[i];

			if ((check % 255 + 1) == wb[WB_WIDTH - 1]) {
				gc8034f_otp_info.rg_gain = (wb[0] | ((wb[1] & 0xf0) << 4)) > 0 ?
							  (wb[0] | ((wb[1] & 0xf0) << 4)) : 0x400;
				gc8034f_otp_info.bg_gain = (((wb[1] & 0x0f) << 8) | wb[2]) > 0 ?
							  (((wb[1] & 0x0f) << 8) | wb[2]) : 0x400;
				gc8034f_otp_info.wb_flag = gc8034f_otp_info.wb_flag | 0x01;
			} else
				LOG_INF("GC8034F_OTP_WB Check sum %d Error !!\n", index + 1);
			break;
		case 0x08:
		case 0x0c:
			LOG_INF("GC8034F_OTP_WB group %d is Invalid !!\n", index + 1);
			gc8034f_otp_info.wb_flag = gc8034f_otp_info.wb_flag | 0x02;
			break;
		default:
			break;
		}
		...
		case 0x00:
			LOG_INF("GC8034F_OTP_GOLDEN group %d is Empty !!\n", index + 1);
			gc8034f_otp_info.golden_flag = gc8034f_otp_info.golden_flag | 0x00;
			break;
		case 0x40:
			LOG_INF("GC8034F_OTP_GOLDEN group %d is Valid !!\n", index + 1);
			check = 0;
			gc8034f_read_otp_group(9, GOLDEN_ROM_START + index * GOLDEN_WIDTH,
					      &golden[0], GOLDEN_WIDTH);
			for (i = 0; i < GOLDEN_WIDTH - 1; i++)
				check += golden[i];

			if ((check % 255 + 1) == golden[GOLDEN_WIDTH - 1]) {
				gc8034f_otp_info.golden_rg = (golden[0] | ((golden[1] & 0xf0) <<
							     4)) > 0 ?
							    (golden[0] | ((golden[1] & 0xf0) << 4)) : RG_TYPICAL;
				gc8034f_otp_info.golden_bg = (((golden[1] & 0x0f) << 8) |
							     golden[2]) > 0 ?
							    (((golden[1] & 0x0f) << 8) | golden[2]) : BG_TYPICAL;
				gc8034f_otp_info.golden_flag = gc8034f_otp_info.golden_flag | 0x01;
			} else
				LOG_INF("GC8034F_OTP_GOLDEN Check sum %d Error !!\n", index + 1);

			break;
		case 0x80:
		case 0xc0:
			LOG_INF("GC8034F_OTP_GOLDEN group %d is Invalid !!\n", index + 1);
			gc8034f_otp_info.golden_flag = gc8034f_otp_info.golden_flag | 0x02;
			break;
		default:
			break;
		}
		...
		case 0x00:
			LOG_INF("GC8034F_OTP_AF group %d is Empty !!\n", index + 1);
			gc8034f_otp_info.af_flag = gc8034f_otp_info.af_flag | 0x00;
			break;
		case 0x04:
			LOG_INF("GC8034F_OTP_AF group %d is Valid !!\n", index + 1);
			check = 0;
			gc8034f_read_otp_group(3, AF_ROM_START + index * AF_WIDTH, &af[0],
					      AF_WIDTH);
			for (i = 0; i < AF_WIDTH - 1; i++)
				check += af[i];

			if ((check % 255 + 1) == af[AF_WIDTH - 1]) {
				gc8034f_otp_info.af_infinity = ((af[0] << 4) & 0x0f00) + af[1];
				gc8034f_otp_info.af_macro = ((af[0] << 8) & 0x0f00) + af[2];
				gc8034f_otp_info.af_flag = gc8034f_otp_info.af_flag | 0x01;
			} else
				LOG_INF("GC8034F_OTP_AF Check sum %d Error !!\n", index + 1);
			break;
		case 0x08:
		case 0x0c:
			LOG_INF("GC8034F_OTP_AF group %d is Invalid !!\n", index + 1);
			gc8034f_otp_info.af_flag = gc8034f_otp_info.af_flag | 0x02;
			break;
		default:
			break;
		}
	...
	/* LSC */
	flag_lsc = gc8034f_read_otp(3, 0x43);
	LOG_INF("GC8034F_OTP_LSC : flag_lsc = 0x%x\n", flag_lsc);
	gc8034f_otp_info.lsc_flag = 0;
	for (index = 0; index < 2; index++) {
		switch ((flag_lsc << (2 * index)) & 0x0c) {
		case 0x00:
			LOG_INF("GC8034F_OTP_LSC group %d is Empty !\n", index + 1);
			gc8034f_otp_info.lsc_flag = gc8034f_otp_info.lsc_flag | 0x00;
			break;
		case 0x04:
			LOG_INF("GC8034F_OTP_LSC group %d is Valid !\n", index + 1);
			check = 0;
			if (index == 0) {
				gc8034f_read_otp_kgroup(3, 0x44, &gc8034f_otp_info.lsc_param[0],
						       396);
				lsc_checksum = gc8034f_read_otp(6, 0x50);
			} else {
				gc8034f_read_otp_kgroup(6, 0x51, &gc8034f_otp_info.lsc_param[0],
						       396);
				lsc_checksum = gc8034f_read_otp(9, 0x5d);
			}
			for (i = 0; i < 396; i++)
				check += gc8034f_otp_info.lsc_param[i];
			if ((check % 255 + 1) == lsc_checksum)
				gc8034f_otp_info.lsc_flag = gc8034f_otp_info.lsc_flag | 0x01;
			else {
				gc8034f_otp_info.lsc_flag = gc8034f_otp_info.lsc_flag | 0x02;
				LOG_INF("GC8034F_OTP_LSC check sum error, check sum read : 0x%x, calculate:0x%x!!\n",
					lsc_checksum, check % 255 + 1);
			}
			break;
		case 0x08:
		case 0x0c:
			LOG_INF("GC8034F_OTP_LSC group %d is Invalid !!\n", index + 1);
			gc8034f_otp_info.lsc_flag = gc8034f_otp_info.lsc_flag | 0x02;
			break;
		default:
			break;
	...
	LOG_INF("GC8034F_OTP_INFO:module_id=0x%x\n",
		gc8034f_otp_info.module_id);
	LOG_INF("GC8034F_OTP_INFO:lens_id=0x%x\n",
		gc8034f_otp_info.lens_id);
	LOG_INF("GC8034F_OTP_INFO:vcm_id=0x%x\n", gc8034f_otp_info.vcm_id);
	LOG_INF("GC8034F_OTP_INFO:vcm_driver_id=0x%x\n",
		gc8034f_otp_info.vcm_driver_id);
	LOG_INF("GC8034F_OTP_INFO:data=%d-%d-%d\n", gc8034f_otp_info.year,
		gc8034f_otp_info.month, gc8034f_otp_info.day);
	LOG_INF("GC8034F_OTP_WB:r/g=0x%x\n", gc8034f_otp_info.rg_gain);
	LOG_INF("GC8034F_OTP_WB:b/g=0x%x\n", gc8034f_otp_info.bg_gain);
	LOG_INF("GC8034F_OTP_GOLDEN:golden_rg=0x%x\n",
		gc8034f_otp_info.golden_rg);
	LOG_INF("GC8034F_OTP_GOLDEN:golden_bg=0x%x\n",
		gc8034f_otp_info.golden_bg);
	LOG_INF("GC8034F_OTP_AF:infitiy=0x%x\n",
		gc8034f_otp_info.af_infinity);
	LOG_INF("GC8034F_OTP_AF:macro=0x%x\n", gc8034f_otp_info.af_macro);
#endif

	/* chip regs */
	gc8034f_otp_info.reg_flag = gc8034f_read_otp(2, 0x4e);

	if (gc8034f_otp_info.reg_flag == 1)
		for (i = 0; i < 5; i++) {
			temp = gc8034f_read_otp(2, 0x4f + 5 * i);
			for (j = 0; j < 2; j++)
				if (((temp >> (4 * j + 3)) & 0x01) == 0x01) {
					gc8034f_otp_info.reg_page[gc8034f_otp_info.reg_num] = (temp >>
							(4 * j)) & 0x03;
					gc8034f_otp_info.reg_addr[gc8034f_otp_info.reg_num] =
						gc8034f_read_otp(2, 0x50 + 5 * i + 2 * j);
					gc8034f_otp_info.reg_value[gc8034f_otp_info.reg_num] =
						gc8034f_read_otp(2, 0x50 + 5 * i + 2 * j + 1);
					gc8034f_otp_info.reg_num++;
				}
		}

#if defined(GC8034FOTP_DEBUG)
	for (i = 0; i < 10; i++) {
		gc8034f_read_otp_group(i, 0, &debug[0], 128);
		for (j = 0; j < 128; j++)
			LOG_INF("GC8034F_OTP_DEBUG: Page%d: addr = 0x%x, value = 0x%x;\n",
				i, j, debug[j]);
	}
#endif

#ifdef CONFIG_IMPROVE_CAMERA_PERFORMANCE
	otp_read_success = gc8034f_otp_info.wb_flag &
			   gc8034f_otp_info.lsc_flag & gc8034f_otp_info.golden_flag;
#endif

}

static void gc8034f_gcore_check_prsel(void)
{
	kal_uint8 product_level = 0;

	product_level = gc8034f_read_otp(2, 0x68) & 0x07;
...
static void gc8034f_gcore_update_dd(void)
{
	kal_uint8 i = 0, j = 0;
	kal_uint8 temp_val0 = 0, temp_val1 = 0, temp_val2 = 0;
	struct gc8034f_dd_t dd_temp = {0, 0, 0};
	if (gc8034f_otp_info.dd_flag == 0x01) {
		LOG_INF("GC8034F_OTP_AUTO_DD start !\n");
		for (i = 0; i < gc8034f_otp_info.dd_cnt; i++) {
#if defined(GC8034F_MIRROR_H) || defined(GC8034F_MIRROR_HV)
			switch (gc8034f_otp_info.dd_param[i].t) {
			case 0:
				gc8034f_otp_info.dd_param[i].x = DD_WIDTH -
								gc8034f_otp_info.dd_param[i].x + 1;
				break;
			case 1:
				gc8034f_otp_info.dd_param[i].x = DD_WIDTH -
								gc8034f_otp_info.dd_param[i].x - 1;
			...
			default:
				gc8034f_otp_info.dd_param[i].x = DD_WIDTH -
								gc8034f_otp_info.dd_param[i].x;
				break;
			}
#endif
#if defined(GC8034F_MIRROR_V) || defined(GC8034F_MIRROR_HV)
			gc8034f_otp_info.dd_param[i].y = DD_HEIGHT - gc8034f_otp_info.
							dd_param[i].y + 1;
#endif
		}
		for (i = 0; i < gc8034f_otp_info.dd_cnt - 1; i++) {
			for (j = i + 1; j < gc8034f_otp_info.dd_cnt; j++) {
				if (gc8034f_otp_info.dd_param[i].y * DD_WIDTH +
				    gc8034f_otp_info.dd_param[i].x
				    > gc8034f_otp_info.dd_param[j].y * DD_WIDTH +
				    gc8034f_otp_info.dd_param[j].x) {
					dd_temp.x = gc8034f_otp_info.dd_param[i].x;
					dd_temp.y = gc8034f_otp_info.dd_param[i].y;
					dd_temp.t = gc8034f_otp_info.dd_param[i].t;
					gc8034f_otp_info.dd_param[i].x = gc8034f_otp_info.dd_param[j].x;
					gc8034f_otp_info.dd_param[i].y = gc8034f_otp_info.dd_param[j].y;
					gc8034f_otp_info.dd_param[i].t = gc8034f_otp_info.dd_param[j].t;
					gc8034f_otp_info.dd_param[j].x = dd_temp.x;
					gc8034f_otp_info.dd_param[j].y = dd_temp.y;
					gc8034f_otp_info.dd_param[j].t = dd_temp.t;
				}
			}
		}
		...
		for (i = 0; i < gc8034f_otp_info.dd_cnt; i++) {
			temp_val0 = gc8034f_otp_info.dd_param[i].x & 0x00ff;
			temp_val1 = ((gc8034f_otp_info.dd_param[i].y & 0x000f) << 4) + ((
						gc8034f_otp_info.dd_param[i].x & 0x0f00) >> 8);
			temp_val2 = (gc8034f_otp_info.dd_param[i].y & 0x0ff0) >> 4;
			write_cmos_sensor(0xaa, i);
			write_cmos_sensor(0xac, temp_val0);
			write_cmos_sensor(0xac, temp_val1);
			write_cmos_sensor(0xac, temp_val2);
			write_cmos_sensor(0xac, gc8034f_otp_info.dd_param[i].t);

			LOG_INF("GC8034F_OTP_GC val0 = 0x%x , val1 = 0x%x , val2 = 0x%x \n",
				temp_val0, temp_val1, temp_val2);
			LOG_INF("GC8034F_OTP_GC x = %d , y = %d \n",
...
#if defined(GC8034FOTP_FOR_CUSTOMER)
static void gc8034f_gcore_update_wb(void)
{
	...
	if (0x01 == (gc8034f_otp_info.golden_flag & 0x01)) {
		rg_typical = gc8034f_otp_info.golden_rg;
		bg_typical = gc8034f_otp_info.golden_bg;
		LOG_INF("GC8034F_OTP_UPDATE_AWB:rg_typical = 0x%x, bg_typical = 0x%x\n",
	...
	if (0x01 == (gc8034f_otp_info.wb_flag & 0x01)) {
		r_gain_current = 2048 * rg_typical / gc8034f_otp_info.rg_gain;
		b_gain_current = 2048 * bg_typical / gc8034f_otp_info.bg_gain;
		g_gain_current = 2048;
		...
		LOG_INF("GC8034F_OTP_UPDATE_AWB:r = 0x%x, g = 0x%x, b = 0x%x\n",
...
static void gc8034f_gcore_update_lsc(void)
{
	kal_uint8 i = 0, j = 0;
	kal_uint16 base = 0;

	if ((gc8034f_otp_info.lsc_flag & 0x01) == 0x01) {
		write_cmos_sensor(0xfe, 0x01);
		...
		for (i = 0; i < 9; i++) {
			...
			for (j = 0; j < 11; j++) {
				...
				base = 4 * (11 * (8 - i) + 10 - j);
#endif
				write_cmos_sensor(0xcc, gc8034f_otp_info.lsc_param[base + 0]);
				write_cmos_sensor(0xcc, gc8034f_otp_info.lsc_param[base + 1]);
				write_cmos_sensor(0xcc, gc8034f_otp_info.lsc_param[base + 2]);
				write_cmos_sensor(0xcc, gc8034f_otp_info.lsc_param[base + 3]);
			}
		}
...
#endif
static void gc8034f_gcore_update_chipversion(void)
{
	kal_uint8 i = 0;

	LOG_INF("GC8034F_OTP_UPDATE_CHIPVERSION:reg_num = %d\n",
		gc8034f_otp_info.reg_num);

	if (gc8034f_otp_info.reg_flag)
		for (i = 0; i < gc8034f_otp_info.reg_num; i++) {
			write_cmos_sensor(0xfe, gc8034f_otp_info.reg_page[i]);
			write_cmos_sensor(gc8034f_otp_info.reg_addr[i],
					  gc8034f_otp_info.reg_value[i]);
			LOG_INF("GC8034F_OTP_UPDATE_CHIP_VERSION: P%d:0x%x -> 0x%x\n",
				gc8034f_otp_info.reg_page[i], gc8034f_otp_info.reg_addr[i],
				gc8034f_otp_info.reg_value[i]);
		}
}

static void gc8034f_gcore_update_otp(void)
{
	gc8034f_gcore_update_dd();
	gc8034f_gcore_check_prsel();
#if defined(GC8034FOTP_FOR_CUSTOMER)
	gc8034f_gcore_update_wb();
	gc8034f_gcore_update_lsc();
#endif
	gc8034f_gcore_update_chipversion();
}

static void gc8034f_gcore_enable_otp(kal_uint8 state)
{
	...
	if (state) {
		...
		LOG_INF("GC8034F_OTP: Enable OTP!\n");
	} else {
		otp_en = otp_en & 0xf7;
		otp_clk = otp_clk & 0xfe;
		write_cmos_sensor(0xf4, otp_en);
		write_cmos_sensor(0xf2, otp_clk);
		LOG_INF("GC8034F_OTP: Disable OTP!\n");
	}
}

static void gc8034f_gcore_identify_otp(void)
{
	...
	gc8034f_gcore_enable_otp(otp_open);
	gc8034f_gcore_read_otp_info();
	gc8034f_gcore_update_otp();
	gc8034f_gcore_enable_otp(otp_close);
	write_cmos_sensor(0xfc, 0x00);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x17, GC8034F_MIRROR);
	...
	write_cmos_sensor(0x92, GC8034F_BinStartY);
	write_cmos_sensor(0x94, GC8034F_BinStartX);
...
static void fullsize_setting(void)
{
	...
	/* Crop window */
	write_cmos_sensor(0x90, 0x01);
	write_cmos_sensor(0x92, GC8034F_FullStartY);
	write_cmos_sensor(0x94, GC8034F_FullStartX);
	write_cmos_sensor(0x95, 0x09);
	write_cmos_sensor(0x96, 0x90);
	write_cmos_sensor(0x97, 0x0c);
	write_cmos_sensor(0x98, 0xc0);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		#if 1 //Leo 20211108
		if (IMGSENSOR_SENSOR_IDX_SUB != cust_get_imgsensor_alive_idx()) {
...
UINT32 GC8034F_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT
				  **pfFunc)
{
