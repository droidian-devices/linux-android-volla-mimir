...
#include "s5k3p3sxfgmsmipiraw_Sensor.h"

#define PFX "S5K3P3SXFGMS_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K3P3SXFGMS_SENSOR_ID,
...
{
		//LOG_INF("s5k3p3sxfgms %s E! currefps:%d\n", __func__, currefps);
		// full size 29.76fps
		// capture setting
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id ==imgsensor_info.sensor_id) {
				printk("s5k3p3sxfgms i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				return ERROR_NONE;
			}
			printk("s5k3p3sxfgms  Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 5;
    ...
    case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
        *(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 3000000;
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:	
			...
			read_3P3_eeprom_v2_gms((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;			
...
//kin0603
UINT32 S5K3P3SXFGMS_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
