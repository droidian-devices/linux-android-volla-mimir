...
#include "imx219fmipiraw_Sensor.h"

/****************************Modify following Strings for debug****************************/
#define PFX "imx219f_camera_sensor"
#define LOG_1 LOG_INF("imx219f,MIPI 2LANE\n")
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX219F_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
{{ 3280, 2464,	0,	0, 3280, 2464, 3280, 2464, 0, 0, 3280, 2464,	4,	4, 3264, 2448}, // Preview 
...
#define IMX219FMIPI_MaxGainIndex (97)
kal_uint16 IMX219FMIPI_sensorGainMapping[IMX219FMIPI_MaxGainIndex][2] ={
{ 64 ,0  },   
{ 68 ,12 },   
{ 71 ,23 },   
{ 74 ,33 },   
...
{
	kal_uint8 iI;
    LOG_INF("[IMX219FMIPI]enter IMX219FMIPIGain2Reg function\n");
    for (iI = 0; iI < (IMX219FMIPI_MaxGainIndex-1); iI++) 
	{

        if(gain <IMX219FMIPI_sensorGainMapping[iI][0])
		{    
            break;
        }
		if(gain < IMX219FMIPI_sensorGainMapping[iI][0])
		{
			return IMX219FMIPI_sensorGainMapping[iI][1];
		}
    ...
    if(gain != IMX219FMIPI_sensorGainMapping[iI][0])
    {
         LOG_INF("Gain mapping don't correctly:%d %d \n", gain, IMX219FMIPI_sensorGainMapping[iI][0]);
    }
	LOG_INF("exit IMX219FMIPIGain2Reg function\n");
    return IMX219FMIPI_sensorGainMapping[iI-1][1];  //here,if gain < 64, stack overflow happen.
	//return NONE;
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 240) { //24fps for PIP
		...
		write_cmos_sensor(0x0100,	0x00); 
		  ...
		  write_cmos_sensor(0x30EB,   0x05); 
		  write_cmos_sensor(0x30EB,   0x09); 
		  write_cmos_sensor(0x0114,   0x03); //03 4lane //01 2lane
		  write_cmos_sensor(0x0128,   0x00); 
		  write_cmos_sensor(0x012A,   0x18); 
		  write_cmos_sensor(0x012B,   0x00); 
		  ...
		  write_cmos_sensor(0x0305,   0x03); 
		  write_cmos_sensor(0x0306,   0x00); 
		  write_cmos_sensor(0x0307,   0x53); //0x51 //53 4lane //36 2lane
		  write_cmos_sensor(0x0309,   0x0A); 
		  write_cmos_sensor(0x030B,   0x01); 
		  write_cmos_sensor(0x030C,   0x00); 
		  write_cmos_sensor(0x030D,   0x56);//0x54 //56 4lane //6c 2lane
		  write_cmos_sensor(0x455E,   0x00); 
		  write_cmos_sensor(0x471E,   0x4B); 
		  write_cmos_sensor(0x4767,   0x0F); 
		  write_cmos_sensor(0x4750,   0x14); 
...
#endif
//
kal_uint8  test_pattern_flagf=0;
...
UINT32 IMX219F_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
