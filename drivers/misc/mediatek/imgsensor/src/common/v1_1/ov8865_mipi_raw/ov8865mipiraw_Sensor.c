...
static kal_uint16 read_cmos_sensor(kal_uint32 addr)
{
	...
	//pr_info("ov8865 read_cmos_sensor addr:%x \n", addr);
	return get_byte;
}
...
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20220909
    if (csci_exist("camera.mirror.support")) {
        ...
        if (csci_exist("ov8865.mirror")) { //Leo 20240409
            image_mirror = (csci_integer("ov8865.mirror", 0) == 1) ? IMAGE_HV_MIRROR : IMAGE_NORMAL;
        }
    }
#endif
    switch (image_mirror)
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x3820,0x06);// ; flip off
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	pr_info("ov8865 get_imgsensor_id 0");
	write_cmos_sensor(0x0103,0x01);// Reset sensor
    mdelay(2);
	
	pr_info("ov8865 get_imgsensor_id 1");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			pr_info("ov8865 get_imgsensor_id sensor_id:%x  imgsensor_info.sensor_id:%x \n", *sensor_id, imgsensor_info.sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {				
				pr_info("ov8865: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}
			pr_info("ov8865: Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
	...
	pr_info("ov8865 get_imgsensor_id 2");
	
	if (*sensor_id != imgsensor_info.sensor_id) {
		// if Sensor ID is not correct, Must set *sensor_id to 0xFFFFFFFF 
		*sensor_id = 0xFFFFFFFF;
		pr_info("ov8865 get_imgsensor_id error!");
