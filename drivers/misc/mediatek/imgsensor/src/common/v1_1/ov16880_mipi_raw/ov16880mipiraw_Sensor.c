...
static void sensor_init(void)
{
	...
	read_eeprom(0xA0, data2, size);
	printk("frank final data2 ");
	for(j=0;j<size;j++)
		printk("frank  %d\n",data2[j]);
	printk("frank \n");
#endif
...
static void preview_setting(void)
{
	...
	*/
	...
	//flip_off_mirror_on
		...
		#endif
	#endif
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20240204
	set_mirror_flip(cust_mirror);
#endif
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {	
			*sensor_id = (read_cmos_sensor_8(0x300A) << 16) | (read_cmos_sensor_8(0x300B)<<8) | read_cmos_sensor_8(0x300C);
			printk("Read sensor id ok, write id:0x%x ,sensor Id:0x%x\n", imgsensor.i2c_write_id,*sensor_id);
        ...
        *(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 3000000;
		...
		if (is_init == false) {
			if (csci_integer("camera.mirror.support", 0) > 0) {
				if (csci_exist("ov16880.mirror")) {
					cust_mirror = csci_integer("ov16880.mirror", 0);
				}
			}
...
UINT32 OV16880_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
