	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			do {
				*sensor_id =return_sensor_id();
                ...
                printk("chench hi556 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
					return ERROR_NONE;
				}	
				printk("chench hi556 get_imgsensor_id Read sensor id fail, i2c write id: 0x%x,sensor id: 0x%x ,imgsensor_info.sensor_id :0x%x\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
				retry--;
			} while(retry > 0);
			i++;
			retry = 2;
...
static kal_uint32 open(void)
{
	...
	LOG_INF("[open]: hi556 PLATFORM:MT6750,MIPI 24LANE\n");
	LOG_INF("hi556 preview 1296*972@30fps,360Mbps/lane; capture 2593*1944@30fps,880Mbps/lane\n");
	printk("[hi556 into open] ");
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		spin_unlock(&imgsensor_drv_lock);
		do {
			...
			printk("chench hi556 Open:Read sensor id fail open i2c write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
