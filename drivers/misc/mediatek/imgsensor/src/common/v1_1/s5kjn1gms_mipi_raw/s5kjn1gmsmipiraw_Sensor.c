...
#if IS_ENABLED(CONFIG_S5KJN1_MIRROR_HV) || defined(CONFIG_S5KJN1_MIRROR_HV)
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_4CELL_BAYER_Gb,//SENSOR_OUTPUT_FORMAT_RAW_Gb,//sensor output first pixel color
#else
...
static kal_uint32 return_sensor_id(void)
{
	...
	if(cust_midmisc_get_board_id() == 1){
		return ((read_cmos_sensor_byte(0x0000) << 8) | read_cmos_sensor_byte(0x0001)) + 1;
	}else{
		return 0;
	}
#else
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if (*sensor_id == imgsensor_info.sensor_id) {
			...
			printk("s5kjn1 i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id, *sensor_id, imgsensor_info.sensor_id);
		return ERROR_NONE;
	}
			printk("s5kjn1 Read sensor id fail, i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id, *sensor_id, imgsensor_info.sensor_id);
	    retry--;
	} while (retry > 0);
	i++;
	retry = 1;
			     ...
			     UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		...
		switch (*feature_data_32) {
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
			pr_info("lzh Jesse+ CAPTURE_JPEG \n");
			memcpy((void *)pvcinfo, (void *)&SENSOR_VC_INFO[1],
			       sizeof(struct SENSOR_VC_INFO_STRUCT));
			break;
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			pr_info("lzh Jesse+ VIDEO_PREVIEW \n");
			memcpy((void *)pvcinfo, (void *)&SENSOR_VC_INFO[2],
			       sizeof(struct SENSOR_VC_INFO_STRUCT));
			break;
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		default:
			pr_info("lzh Jesse+ CAMERA_PREVIEW \n");
...
{
	LOG_INF("S5KJN1GMS_MIPI_RAW_SensorInit in\n");
