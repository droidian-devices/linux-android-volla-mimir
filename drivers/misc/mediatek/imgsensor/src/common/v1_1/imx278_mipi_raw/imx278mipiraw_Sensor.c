...
//static BYTE imx278_qsc_data[DATA_SIZE] = { 0 };
...
static struct imgsensor_info_struct imgsensor_info = { 
    ...
    .sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_R,//R Gr Gb B SENSOR_OUTPUT_FORMAT_RAW_R SENSOR_OUTPUT_FORMAT_RAW_Gb
    .mclk = 24,
    .mipi_lane_num = SENSOR_MIPI_4_LANE,
    .i2c_addr_table = {0x34, 0x40,0xff},
...
{{ 4208, 3120, 0, 0, 4208, 3120, 2104, 1560, 2, 0, 2100, 1560, 0, 0, 2100, 1560}, // Preview 2112*1558
 { 4208, 3120, 0, 0, 4208, 3120, 4208, 3120, 0, 0, 4208, 3120, 0, 0, 4208, 3120}, // capture 4206*3128
 { 4208, 3120, 0, 0, 4208, 3120, 2104, 1560, 0, 0, 2100, 1560, 0, 0, 2100, 1560}, // video 
 { 4208, 3120, 0, 0, 4208, 3120, 1052,  780, 2, 4, 1052,  780, 0, 0, 1052,  780}, //hight speed video 
 { 4208, 3120, 0, 0, 4208, 3120, 2104,  1560, 2, 0, 2100,  1560, 0, 0, 2100, 1560},// slim video
 };// slim video  
//#define IMX135_OTP_Enable 1



#define IMX278MIPI_MaxGainIndex (104)
kal_uint16 IMX278MIPI_sensorGainMapping[IMX278MIPI_MaxGainIndex][2] ={
{71 , 53 },//1.12
	{72 , 57 },//1.13
	{73 , 65 },//1.15
	{74 , 69 },//1.16
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	for (i = 0; i < IMX278MIPI_MaxGainIndex; i++) {
		if(gain <= IMX278MIPI_sensorGainMapping[i][0]){
			break;
		}
	}
		if (i >= IMX278MIPI_MaxGainIndex) {
		pr_debug("warning! use max sensor gain");
		return IMX278MIPI_sensorGainMapping[IMX278MIPI_MaxGainIndex - 1][1];
	}
	if(gain != IMX278MIPI_sensorGainMapping[i][0])
		LOG_INF("Gain mapping don't correctly:%d %d \n", gain, IMX278MIPI_sensorGainMapping[i][0]);
	return IMX278MIPI_sensorGainMapping[i][1];
...
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20220909
    if (csci_exist("camera.mirror.support.imx278")) {
        if (csci_integer("camera.mirror.support.imx278", 0) > 0) {
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            printk("[cam imx278mipiraw_Sensor ] get_imgsensor_id 000,i2c write id: 0x%x, sensor id: 0x%x imgsensor_info.sensor_id: 0x%x \n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
            if (*sensor_id == imgsensor_info.sensor_id) {               
                printk("cam imx278mipiraw_Sensor i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);      
                return ERROR_NONE;
            }   
            printk("cam imx278mipiraw_Sensor Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20220909
    if (csci_exist("camera.output.dataformat.imx278")) {
        sensor_info->SensorOutputDataFormat = csci_integer("camera.output.dataformat.imx278", 0);
    }
#endif
...
static void setIso(struct SET_SENSOR_ISO* para)
{    INT32 val;
    UINT32 iso;
    UINT32 sensorMode;
    iso = para->ISO;
...
break;
	...
	case SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME:
		set_shutter_frame_length(
			...
			(BOOL) (*(feature_data + 2)));
		break;
		case SENSOR_FEATURE_GET_FRAME_CTRL_INFO_BY_SCENARIO:   //??????????ش?1???????°?ramesync???ڶ???????ش?marginֵ??AE/CCUʹ?
   /*
...
break;		
	...
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		...
		break;
			...
			case SENSOR_FEATURE_GET_BINNING_TYPE:                   //by sensor mode?ش?binning_ratio
...
UINT32 IMX278_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
