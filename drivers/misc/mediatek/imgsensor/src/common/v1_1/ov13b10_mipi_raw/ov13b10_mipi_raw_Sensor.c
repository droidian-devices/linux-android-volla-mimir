...
#include "ov13b10_mipi_raw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.i2c_addr_table = {0x21,0x20,0x6C,0xff},
	.i2c_speed = 400,
};
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		printk("ov13b10 imgsensor id: 0x%x fail\n", *sensor_id);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			printk("ov13b10  sensor_id: 0x%x\n",sensor_id);
			if (sensor_id == imgsensor_info.sensor_id) {
				printk("ov13b10 i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			printk("ov13b10 Read sensor id fail: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id,
				sensor_id);
			retry--;
		} while (retry > 0);
	...
	if (imgsensor_info.sensor_id != sensor_id) {
		printk("ov13b10 Open sensor id: 0x%x fail\n", sensor_id);
		return ERROR_SENSOR_CONNECT_FAIL;
	}
