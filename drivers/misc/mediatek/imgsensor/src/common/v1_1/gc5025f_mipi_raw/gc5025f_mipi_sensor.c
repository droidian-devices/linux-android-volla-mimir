...
#include "gc5025f_mipi_sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC5025F_camera_sensor"
#define LOG_1 LOG_INF("GC5025F,MIPI 2LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool GC5025FDuringTestPattern = KAL_FALSE;

static kal_uint32 Dgain_ratio = 1;
static kal_bool DR_State = KAL_TRUE;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC5025F_SENSOR_ID,   /* record sensor id defined in Kd_imgsensor.h */
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_R, /* sensor output first pixel color */	.mclk = 24, /* mclk value, suggest 24 or 26 for 24Mhz or 26Mhz */
...
static struct gc5025f_otp gc5025f_otp_info;

static kal_uint8 gc5025f_read_otp(kal_uint8 addr)
{
	kal_uint8 value;
	kal_uint8 regd4;
...
static void gc5025f_read_otp_group(kal_uint8 addr, kal_uint8 *buff, int size)
{
	kal_uint8 i;
	kal_uint8 regd4;
	kal_uint16 realaddr = addr * 8;
...
static void gc5025f_select_page_otp(kal_uint8 otp_select_page)
{
	kal_uint8 page;
...
static void gc5025f_gcore_read_otp_info(void)
{
...
#if defined(GC5025FOTP_FOR_CUSTOMER)
	kal_uint8 flag1, flag_golden, index;
	kal_uint8 info[8];
	kal_uint8 wb[5];
	kal_uint8 golden[5];
#endif
	memset(&gc5025f_otp_info, 0, sizeof(gc5025f_otp_info));

	/* TODO */
	gc5025f_select_page_otp(otp_page0);
	flagdd = gc5025f_read_otp(0x00);
	LOG_INF("GC5025F_OTP_DD : flag_dd = 0x%x\n", flagdd);
	flag_chipversion = gc5025f_read_otp(0x7f);

	/* DD */
	switch (flagdd & 0x03) {
	case 0x00:
		LOG_INF("GC5025F_OTP_DD is Empty !!\n");
		gc5025f_otp_info.dd_flag = 0x00;
		break;
	case 0x01:
		LOG_INF("GC5025F_OTP_DD is Valid!!\n");
		total_number = gc5025f_read_otp(0x01) + gc5025f_read_otp(0x02);
		LOG_INF("GC5025F_OTP : total_number = %d\n", total_number);

		if (total_number <= 31) {
			gc5025f_read_otp_group(0x03, &ddtempbuff[0], total_number * 4);
		} else {
			gc5025f_read_otp_group(0x03, &ddtempbuff[0], 31 * 4);
			gc5025f_select_page_otp(otp_page1);
			gc5025f_read_otp_group(0x29, &ddtempbuff[31 * 4], (total_number - 31) * 4);
		}

		/* DD check sum */
		gc5025f_select_page_otp(otp_page1);
		ddchecksum = gc5025f_read_otp(0x61);
		check = total_number;
		for (i = 0; i < 4 * total_number; i++)
			check += ddtempbuff[i];
		if ((check % 255 + 1) == ddchecksum)
			LOG_INF("GC5025F_OTP_DD : DD check sum correct! checksum = 0x%x\n", ddchecksum);
		else
			LOG_INF("GC5025F_OTP_DD : DD check sum error! otpchecksum = 0x%x, sum = 0x%x\n",
				ddchecksum, (check % 255 + 1));

		for (i = 0; i < total_number; i++) {
			LOG_INF("GC5025F_OTP_DD:index = %d, data[0] = %x, data[1] = %x, data[2] = %x, data[3] = %x\n",
			...
			if (ddtempbuff[4 * i + 3] & 0x10) {
				...
				case 3:
					for (j = 0; j < 4; j++) {
						gc5025f_otp_info.dd_param_x[cnt] =
							(((kal_uint16)ddtempbuff[4 * i + 1] & 0x0f) << 8)
							+ ddtempbuff[4 * i];
						gc5025f_otp_info.dd_param_y[cnt] =
							((kal_uint16)ddtempbuff[4 * i + 2] << 4)
							+ ((ddtempbuff[4 * i + 1] & 0xf0) >> 4) + j;
						gc5025f_otp_info.dd_param_type[cnt++] = 2;
					}
					break;
				case 4:
					for (j = 0; j < 2; j++) {
				...
				default:
					...
					gc5025f_otp_info.dd_param_y[cnt] = ((kal_uint16)ddtempbuff[4 * i + 2] << 4)
						+ ((ddtempbuff[4 * i + 1] & 0xf0) >> 4);
					gc5025f_otp_info.dd_param_type[cnt++] = ddtempbuff[4 * i + 3] & 0x0f;
					break;
				}
			} else {
				LOG_INF("GC5025F_OTP_DD:check_id[%d] = %x,checkid error!!\n",
					i, ddtempbuff[4 * i + 3] & 0xf0);
			}
		}

		gc5025f_otp_info.dd_cnt = cnt;
		gc5025f_otp_info.dd_flag = 0x01;
		break;
	case 0x02:
	case 0x03:
		LOG_INF("GC5025F_OTP_DD is Invalid !!\n");
		gc5025f_otp_info.dd_flag = 0x02;
		break;
	default:
		break;
	}
	...
	/* For Chip Version */
	LOG_INF("GC5025F_OTP_CHIPVESION : flag_chipversion = 0x%x\n", flag_chipversion);

	switch ((flag_chipversion >> 4) & 0x03) {
	case 0x00:
		LOG_INF("GC5025F_OTP_CHIPVERSION is Empty !!\n");
		break;
	case 0x01:
		LOG_INF("GC5025F_OTP_CHIPVERSION is Valid !!\n");
		gc5025f_select_page_otp(otp_page1);
		i = 0;
		do {
			gc5025f_otp_info.reg_addr[i] = gc5025f_read_otp(REG_ROM_START + i*2);
			gc5025f_otp_info.reg_value[i] = gc5025f_read_otp(REG_ROM_START + i*2 + 1);
			LOG_INF("GC5025F_OTP_CHIPVERSION reg_addr[%d] = 0x%x,reg_value[%d] = 0x%x\n",
				i, gc5025f_otp_info.reg_addr[i], i, gc5025f_otp_info.reg_value[i]);
			i++;
		} while ((gc5025f_otp_info.reg_addr[i-1] != 0) && (i < 10));
		gc5025f_otp_info.reg_num = i - 1;
		break;
	case 0x02:
		LOG_INF("GC5025F_OTP_CHIPVERSION is Invalid !!\n");
		break;
	default:
		break;
	}
...
#if defined(GC5025FOTP_FOR_CUSTOMER)
	gc5025f_select_page_otp(otp_page1);
	flag1 = gc5025f_read_otp(0x00);
	flag_golden = gc5025f_read_otp(0x1b);
	LOG_INF("GC5025F_OTP: flag1 = 0x%x, flag_golden = 0x%x\n", flag1, flag_golden);

    /* INFO & WB */
	for (index = 0; index < 2; index++) {
		switch ((flag1 >> (4 + 2 * index)) & 0x03) {
		case 0x00:
			LOG_INF("GC5025F_OTP_INFO group %d is Empty !!\n", index + 1);
			break;
		case 0x01:
			LOG_INF("GC5025F_OTP_INFO group %d is Valid !!\n", index + 1);
			check = 0;
			gc5025f_read_otp_group(INFO_ROM_START + index * INFO_WIDTH, &info[0], INFO_WIDTH);
			for (i = 0; i < INFO_WIDTH - 1; i++)
				check += info[i];
			if ((check % 255 + 1) == info[INFO_WIDTH-1]) {
				gc5025f_otp_info.module_id = info[0];
				gc5025f_otp_info.lens_id = info[1];
				gc5025f_otp_info.vcm_driver_id = info[2];
				gc5025f_otp_info.vcm_id = info[3];
				gc5025f_otp_info.year = info[4];
				gc5025f_otp_info.month = info[5];
				gc5025f_otp_info.day = info[6];
			} else {
				LOG_INF("GC5025F_OTP_INFO Check sum %d Error !!\n", index + 1);
			}
			break;
		case 0x02:
		case 0x03:
			LOG_INF("GC5025F_OTP_INFO group %d is Invalid !!\n", index + 1);
			break;
		default:
			break;
		}
		...
		case 0x00:
			LOG_INF("GC5025F_OTP_WB group %d is Empty !!\n", index + 1);
			gc5025f_otp_info.wb_flag = gc5025f_otp_info.wb_flag | 0x00;
			break;
		case 0x01:
			LOG_INF("GC5025F_OTP_WB group %d is Valid !!\n", index + 1);
			check = 0;
			gc5025f_read_otp_group(WB_ROM_START + index * WB_WIDTH, &wb[0], WB_WIDTH);
			for (i = 0; i < WB_WIDTH - 1; i++)
				check += wb[i];
			if ((check % 255 + 1) == wb[WB_WIDTH - 1]) {
				gc5025f_otp_info.rg_gain = (((wb[0] << 8) & 0xff00) | wb[1]) > 0 ?
					(((wb[0] << 8) & 0xff00) | wb[1]) : 0x400;
				gc5025f_otp_info.bg_gain = (((wb[2] << 8) & 0xff00) | wb[3]) > 0 ?
					(((wb[2] << 8) & 0xff00) | wb[3]) : 0x400;
				gc5025f_otp_info.wb_flag = gc5025f_otp_info.wb_flag | 0x01;
			} else {
				LOG_INF("GC5025F_OTP_WB Check sum %d Error !!\n", index + 1);
			}
			break;
		case 0x02:
		case 0x03:
			LOG_INF("GC5025F_OTP_WB group %d is Invalid !!\n", index + 1);
			gc5025f_otp_info.wb_flag = gc5025f_otp_info.wb_flag | 0x02;
			break;
		default:
			break;
		}
		...
		case 0x00:
			LOG_INF("GC5025F_OTP_GOLDEN group %d is Empty !!\n", index + 1);
			gc5025f_otp_info.golden_flag = gc5025f_otp_info.golden_flag | 0x00;
			break;
		case 0x01:
			LOG_INF("GC5025F_OTP_GOLDEN group %d is Valid !!\n", index + 1);
			check = 0;
			gc5025f_read_otp_group(GOLDEN_ROM_START + index * GOLDEN_WIDTH, &golden[0], GOLDEN_WIDTH);
			for (i = 0; i < GOLDEN_WIDTH - 1; i++)
				check += golden[i];
			if ((check % 255 + 1) == golden[GOLDEN_WIDTH - 1]) {
				gc5025f_otp_info.golden_rg = (((golden[0] << 8) & 0xff00) | golden[1]) > 0 ?
					(((golden[0] << 8) & 0xff00) | golden[1]) : RG_TYPICAL;
				gc5025f_otp_info.golden_bg = (((golden[2] << 8) & 0xff00) | golden[3]) > 0 ?
					(((golden[2] << 8) & 0xff00) | golden[3]) : BG_TYPICAL;
				gc5025f_otp_info.golden_flag = gc5025f_otp_info.golden_flag | 0x01;
			} else {
				LOG_INF("GC5025F_OTP_GOLDEN Check sum %d Error !!\n", index + 1);
			}
			break;
		case 0x02:
		case 0x03:
			LOG_INF("GC5025F_OTP_GOLDEN group %d is Invalid !!\n", index + 1);
			gc5025f_otp_info.golden_flag = gc5025f_otp_info.golden_flag | 0x02;
			break;
		default:
			break;
		}
	...
	LOG_INF("GC5025F_OTP_INFO:module_id=0x%x\n", gc5025f_otp_info.module_id);
	LOG_INF("GC5025F_OTP_INFO:lens_id=0x%x\n", gc5025f_otp_info.lens_id);
	LOG_INF("GC5025F_OTP_INFO:vcm_id=0x%x\n", gc5025f_otp_info.vcm_id);
	LOG_INF("GC5025F_OTP_INFO:vcm_driver_id=0x%x\n", gc5025f_otp_info.vcm_driver_id);
	LOG_INF("GC5025F_OTP_INFO:data=%d-%d-%d\n", gc5025f_otp_info.year, gc5025f_otp_info.month, gc5025f_otp_info.day);
	LOG_INF("GC5025F_OTP_WB:r/g=0x%x\n", gc5025f_otp_info.rg_gain);
	LOG_INF("GC5025F_OTP_WB:b/g=0x%x\n", gc5025f_otp_info.bg_gain);
	LOG_INF("GC5025F_OTP_GOLDEN:golden_rg=0x%x\n", gc5025f_otp_info.golden_rg);
	LOG_INF("GC5025F_OTP_GOLDEN:golden_bg=0x%x\n", gc5025f_otp_info.golden_bg);
#endif
}

static void gc5025f_gcore_update_dd(void)
{
	kal_uint16 i = 0, j = 0;
	...
	/* TODO */
	if (0x01 == gc5025f_otp_info.dd_flag) {
#if defined(GC5025F_MIRROR_NORMAL)
		/* do nothing */
#elif defined(GC5025F_MIRROR_H)
		for (i = 0; i < gc5025f_otp_info.dd_cnt; i++) {
			if (gc5025f_otp_info.dd_param_type[i] == 0)
				gc5025f_otp_info.dd_param_x[i] = WINDOW_WIDTH - gc5025f_otp_info.dd_param_x[i] + 1;
			else if (gc5025f_otp_info.dd_param_type[i] == 1)
				gc5025f_otp_info.dd_param_x[i] = WINDOW_WIDTH - gc5025f_otp_info.dd_param_x[i] - 1;
			else
				gc5025f_otp_info.dd_param_x[i] = WINDOW_WIDTH - gc5025f_otp_info.dd_param_x[i];
		}
#elif defined(GC5025F_MIRROR_V)
		for (i = 0; i < gc5025f_otp_info.dd_cnt; i++)
			gc5025f_otp_info.dd_param_y[i] = WINDOW_HEIGHT - gc5025f_otp_info.dd_param_y[i] + 1;

#elif defined(GC5025F_MIRROR_HV)
		for (i = 0; i < gc5025f_otp_info.dd_cnt; i++) {
			if (gc5025f_otp_info.dd_param_type[i] == 0) {
				gc5025f_otp_info.dd_param_x[i] = WINDOW_WIDTH - gc5025f_otp_info.dd_param_x[i] + 1;
				gc5025f_otp_info.dd_param_y[i] = WINDOW_HEIGHT - gc5025f_otp_info.dd_param_y[i] + 1;
		...
		for (i = 0; i < gc5025f_otp_info.dd_cnt - 1; i++)
			for (j = i + 1; j < gc5025f_otp_info.dd_cnt; j++)
				if (gc5025f_otp_info.dd_param_y[i] * WINDOW_WIDTH
					+ gc5025f_otp_info.dd_param_x[i]
					> gc5025f_otp_info.dd_param_y[j] * WINDOW_WIDTH
					+ gc5025f_otp_info.dd_param_x[j]) {
					temp_x = gc5025f_otp_info.dd_param_x[i];
					gc5025f_otp_info.dd_param_x[i] = gc5025f_otp_info.dd_param_x[j];
					gc5025f_otp_info.dd_param_x[j] = temp_x;
					temp_y = gc5025f_otp_info.dd_param_y[i];
					gc5025f_otp_info.dd_param_y[i] = gc5025f_otp_info.dd_param_y[j];
					gc5025f_otp_info.dd_param_y[j] = temp_y;
					temp_type = gc5025f_otp_info.dd_param_type[i];
					gc5025f_otp_info.dd_param_type[i] = gc5025f_otp_info.dd_param_type[j];
					gc5025f_otp_info.dd_param_type[j] = temp_type;
				}
		...
		for (i = 0; i < gc5025f_otp_info.dd_cnt; i++) {
			temp_val0 = gc5025f_otp_info.dd_param_x[i] & 0x00ff;
			temp_val1 = (((gc5025f_otp_info.dd_param_y[i]) << 4) & 0x00f0)
						+ ((gc5025f_otp_info.dd_param_x[i] >> 8) & 0X000f);
			temp_val2 = (gc5025f_otp_info.dd_param_y[i] >> 4) & 0xff;
			write_cmos_sensor(0xaa, i);
			write_cmos_sensor(0xac, temp_val0);
			write_cmos_sensor(0xac, temp_val1);
			write_cmos_sensor(0xac, temp_val2);
			while ((i < gc5025f_otp_info.dd_cnt - 1)
				&& (gc5025f_otp_info.dd_param_x[i] == gc5025f_otp_info.dd_param_x[i + 1])
				&& (gc5025f_otp_info.dd_param_y[i] == gc5025f_otp_info.dd_param_y[i + 1])) {
					flag = 1;
					i++;
				}
			if (flag)
				...
				write_cmos_sensor(0xac, gc5025f_otp_info.dd_param_type[i]);

			LOG_INF("GC5025F_OTP_GC val0 = 0x%x, val1 = 0x%x, val2 = 0x%x\n",
				temp_val0, temp_val1, temp_val2);
			LOG_INF("GC5025F_OTP_GC x = %d, y = %d\n",
...
#if defined(GC5025FOTP_FOR_CUSTOMER)
static void gc5025f_gcore_update_wb(void)
{
	...
	if (0x02 == gc5025f_otp_info.golden_flag)
		return;

	if (0x00 == (gc5025f_otp_info.golden_flag & 0x01)) {
		rg_typical = RG_TYPICAL;
		bg_typical = BG_TYPICAL;
	}

	if (0x01 == (gc5025f_otp_info.golden_flag & 0x01)) {
		rg_typical = gc5025f_otp_info.golden_rg;
		bg_typical = gc5025f_otp_info.golden_bg;
		LOG_INF("GC5025F_OTP_UPDATE_AWB:rg_typical = 0x%x, bg_typical = 0x%x\n", rg_typical, bg_typical);
	}

	if (0x01 == (gc5025f_otp_info.wb_flag & 0x01)) {
		r_gain_current = 1024 * rg_typical / gc5025f_otp_info.rg_gain;
		b_gain_current = 1024 * bg_typical / gc5025f_otp_info.bg_gain;
		g_gain_current = 1024;
		...
		LOG_INF("GC5025F_OTP_UPDATE_AWB:r_gain_current = 0x%x, b_gain_current = 0x%x, base_gain = 0x%x\n",
		...
		LOG_INF("GC5025F_OTP_UPDATE_AWB:r_gain = 0x%x, g_gain = 0x%x, b_gain = 0x%x\n", r_gain, g_gain, b_gain);
...
static void gc5025f_gcore_update_chipversion(void)
{
	kal_uint8 i;

	LOG_INF("GC5025F_OTP_UPDATE_CHIPVERSION:reg_num = %d\n", gc5025f_otp_info.reg_num);

	/*write_cmos_sensor(0xfe, 0x00);*/

	for (i = 0; i < gc5025f_otp_info.reg_num; i++) {
		write_cmos_sensor(gc5025f_otp_info.reg_addr[i], gc5025f_otp_info.reg_value[i]);
		LOG_INF("GC5025F_OTP_UPDATE_CHIP_VERSION:{0x%x,0x%x}!!\n",
			gc5025f_otp_info.reg_addr[i], gc5025f_otp_info.reg_value[i]);
	}
	/*write_cmos_sensor(0xfe, 0x00);*/
}

static void gc5025f_gcore_update_otp(void)
{
	gc5025f_gcore_update_dd();
#if defined(GC5025FOTP_FOR_CUSTOMER)
	gc5025f_gcore_update_wb();
#endif
	gc5025f_gcore_update_chipversion();
}

static void gc5025f_gcore_enable_otp(kal_uint8 state)
{
	...
	if (state) {
		...
		LOG_INF("GC5025F_OTP: Enable OTP!\n");
	} else {
		otp_en = otp_en & 0x7f;
		...
		LOG_INF("GC5025F_OTP: Disable OTP!\n");
	}
}

static void gc5025f_gcore_initial_otp(void)
{
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xfe, 0x00);
...
static void gc5025f_gcore_identify_otp(void)
{
	gc5025f_gcore_initial_otp();
	gc5025f_gcore_enable_otp(otp_open);
	gc5025f_gcore_read_otp_info();
	gc5025f_gcore_enable_otp(otp_close);
}

static int gc5025f_gcore_check_version(void)
{
	kal_uint8 flag_doublereset = 0, flag_GC5025FA = 0;
	...
	flag_doublereset = gc5025f_read_otp(0x26);
	flag_GC5025FA = gc5025f_read_otp(0x27);

	if ((flag_GC5025FA & 0x01) == 0x01) {
		printk("GC5025FA sensor!\n");
		DR_State = KAL_FALSE;
	} else {
		if ((flag_doublereset & 0x03) == 0x01) {
			DR_State = KAL_FALSE;
			printk("GC5025F sensor double reset off\n");
		} else {
			DR_State = KAL_TRUE;
			printk("GC5025F sensor double reset on\n");
		}
	}
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if ((ANALOG_GAIN_1 <= iReg) && (iReg < ANALOG_GAIN_2)) {
		...
		LOG_INF("GC5025FMIPI analogic gain 1x, GC5025FMIPI add pregain = %d\n", temp);
	} else {
		write_cmos_sensor(0xfe,  0x00);
		...
		write_cmos_sensor(0xb1, temp >> 6);
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF("GC5025FMIPI analogic gain 1.4x, GC5025FMIPI add pregain = %d\n", temp);
	}
	return gain;
}    /*    set_gain  */
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x17, GC5025F_MIRROR);
	...
	write_cmos_sensor(0x92, GC5025F_STARTY);
	write_cmos_sensor(0x94, GC5025F_STARTX);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		#endif
		do {
			printk("--hxk----------try 5025 iRead sensor id\n");
			*sensor_id = return_sensor_id();
					//Leo 20210901
			if (*sensor_id == GC5025_SENSOR_ID) {
				*sensor_id = GC5025F_SENSOR_ID;
			}
			
			if (*sensor_id == imgsensor_info.sensor_id) {
                printk("--hxk----------try 5025 i2c write id: 0x%x, sensor id: 0x%x sensor_success\n", imgsensor.i2c_write_id,*sensor_id);
				return ERROR_NONE;
			}
            printk("--hxk----------try 5025 iRead sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
		    ...
		    if (sensor_id == imgsensor_info.sensor_id) {
				printk("--hxk----------i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
		    }
		    printk("--hxk----------Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
		    retry--;
		} while (retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
	...
	gc5025f_gcore_check_version();

	/* Don't Remove!! */
	gc5025f_gcore_identify_otp();

	/* initail sequence write in  */
	sensor_init();

	/* write registers from sram */
	gc5025f_gcore_update_otp();
	...
	spin_unlock(&imgsensor_drv_lock);
	GC5025FDuringTestPattern = KAL_FALSE;
...
UINT32 GC5025F_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}    /*    GC5025FMIPI_RAW_SensorInit    */
