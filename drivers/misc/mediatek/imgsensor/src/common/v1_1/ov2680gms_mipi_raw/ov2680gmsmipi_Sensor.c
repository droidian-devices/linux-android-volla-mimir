...
#include "ov2680gmsmipi_Sensor.h"

/*********************Modify Following Strings for Debug********************/
#define PFX "OV2680GMS_camera_sensor"
#define LOG_1 cam_pr_debug("OV2680GMS,MIPI 1LANE\n")
...
/*according toov2680gms datasheet p53 image cropping*/
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	{1616, 1216, 0000, 0000, 1616, 1216, 1616, 1216, 8, 8,
		1600, 1200, 0000, 0000, 1600, 1200},
	{1616, 1216, 0000, 0000, 1616, 1216, 1616, 1216, 8, 8,
		1600, 1200, 0000, 0000, 1600, 1200},
	{1616, 1216, 0000, 0000, 1616, 1216, 1616, 1216, 8, 8,
		1600, 1200, 0000, 0000, 1600, 1200},
	{1616, 1216, 0160, 0242, 1296, 0732, 1296, 0732, 8, 6,
...
static kal_uint32 set_test_pattern_mode(kal_uint32 modes)
{
	...
	if (modes == 5) {
		write_cmos_sensor(0x5000, 1<<6);
		write_cmos_sensor(0x5004, 0);
		write_cmos_sensor(0x5005, 0);
		write_cmos_sensor(0x5006, 0);
		write_cmos_sensor(0x5007, 0);
		write_cmos_sensor(0x5008, 0);
		write_cmos_sensor(0x5009, 0);
	} else if (modes != 5 && (imgsensor.test_pattern == 5)) {
		write_cmos_sensor(0x5000, 0x00);	/* test pattern off */
...
UINT32 OV2680GMS_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}				/*      OV2680GMS_MIPI_RAW_SensorInit      */
