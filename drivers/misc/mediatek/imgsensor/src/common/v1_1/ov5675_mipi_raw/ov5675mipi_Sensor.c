...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5]=
{
   ...
   { 2592, 1944,	 16,	 12, 2560, 1920,  640,  480, 0000, 0000,  640,   480,	   0,   	0,  640,  480}, //hight speed video 
...
static void write_shutter(kal_uint16 shutter)
{
	...
	write_cmos_sensor(0x3500, (Reg_shutter >> 12) & 0x0F);	
		...
		//write frame_length:0x7FFB
		write_cmos_sensor(0x380e, 0x7F);
		write_cmos_sensor(0x380f, 0xFB);
...
write_cmos_sensor(0x3503,0x0c);  //gain delay 1 frame, exposure delay 1 frame, real gain
...
write_cmos_sensor(0x4003,0x40);    //blc target 0x40
write_cmos_sensor(0x3107,0x01);    //keep clock on during dummy lines
...
write_cmos_sensor(0x3d8d,0xe7);    //Header address low byte  
...
{
	#if 1
	printk(" preview_setting OV5675PreviewSetting_2lane enter\n");
...
//@@2592x1944_15FPS_MIPI_2_LANE
//;Xclk 24Mhz
//;Pclk clock frequency: 45Mhz
//;linelength = 750(0x2ee)
//;framelength = 4000(0xfa0)
//;grabwindow_width  = 2592
//;grabwindow_height = 1944
//;max_framerate: 15fps	
//;mipi_datarate per lane: 900Mbps
		...
		write_cmos_sensor(0x380c,0x02);
		write_cmos_sensor(0x380d,0xee);
		write_cmos_sensor(0x380e,0x0f);     //VTS  4000
...
//;mipi_datarate per lane: 900Mbps
   ...
   write_cmos_sensor(0x4009,0x0d);
   write_cmos_sensor(0x4041,0x07);
   write_cmos_sensor(0x3501,0x46);    //max expo= ([380e,380f]-4)/2.
...
//@@640x480_120FPS_MIPI_2_LANE
//;Xclk 24Mhz
//;Pclk clock frequency: 45Mhz
//;linelength = 750(0x2ee)
//;framelength = 500(0x1f4)
//;grabwindow_width  = 640
//;grabwindow_height = 480
...
//VGA 120fps
	 ...
	 write_cmos_sensor(0x4041,0x03);
	 write_cmos_sensor(0x3501,0x0F);     //max expo= ([380e,380f]-4)/2.
	 write_cmos_sensor(0x3502,0x80);
	 write_cmos_sensor(0x0100,0x01);
...
//@@1280x720_60FPS_MIPI_2_LANE
//;Xclk 24Mhz
//;Pclk clock frequency: 45Mhz
//;linelength = 720(0x2d0)
...
//;mipi_datarate per lane: 900Mbps
    ...
    write_cmos_sensor(0x380c,0x02);
    write_cmos_sensor(0x380d,0xee);
    write_cmos_sensor(0x380e,0x03);     //VTS 1000
    ...
    write_cmos_sensor(0x4009,0x07);
    write_cmos_sensor(0x4041,0x03);
    write_cmos_sensor(0x3501,0x1F);    //max expo= ([380e,380f]-4)/2
		...
		do {
            *sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {				
				printk("ov5675 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			printk("ov5675 Read sensor id fail, write id: 0x%x, sensor id; 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
            sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {				
				printk("ov5675 open i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);	  
				break;
			}	
			printk("ov5675 open Read sensor id fail, write id: 0x%x, sensor id 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
    ...
    case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
        *(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 3000000;
		...
		case SENSOR_FEATURE_GET_PERIOD:
			...
			*feature_para_len=4;
		        LOG_INF("feature_Control imgsensor.line_length = %d,imgsensor.frame_length = %d\n",imgsensor.line_length,imgsensor.frame_length);
			break;
