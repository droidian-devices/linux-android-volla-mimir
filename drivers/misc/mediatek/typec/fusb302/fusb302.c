...
#if IS_ENABLED(CONFIG_WB_TYPEC_EARPIECE) || defined(CONFIG_WB_TYPEC_EARPIECE) //jnier add 20230512
...
static bool fusb302_probe_faile;
#endif
	...
	for (i = PD_HEADER_CNT(chip->rec_head) - 1; i >= 0; i--) {
		...
		case 0:
			...
			if ((CAP_FPDO_VOLTAGE(chip->rec_load[i]) * 50) <=
				...
				pos_find = 1;
				printk("----pd_output_vol=%d  pd_output_cur=%d----chip->pos_power=%d\n", chip->pd_output_vol, chip->pd_output_cur, chip->pos_power);
			}
			break;
		case 1:
			/* Battery */
	...
	if (memcmp(&chip->notify, &chip->notify_cmp,
		...
		if (chip->notify.power_role == POWER_ROLE_SINK &&
			...
			if(chip->notify.is_pps_support){
				property.intval = (chip->p_vmin << 15 | chip->p_vmax);
...
#if PRINTK_LOG
	printk("<<< 111 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x>>>\n",
...
#if IS_ENABLED(CONFIG_TCPC_HUSB311)
static int get_fusb302_chipid(struct fusb30x_chip *chip){
	u32 tmp;
...
static void fusb_state_snk_evaluate_caps(struct fusb30x_chip *chip, u32 evt)
{
	...
	for (tmp = 0; tmp < PD_HEADER_CNT(chip->rec_head); tmp++) {
		...
		case 0:
			...
			chip->mv[tmp] = CAP_FPDO_VOLTAGE(pdo) * 50;
			chip->ma[tmp] = CAP_FPDO_CURRENT(pdo) * 10;

			printk("--------chip->mv[%d]=%d  chip->ma[%d]=%d--------\n",tmp,chip->mv[tmp],tmp,chip->ma[tmp]);
			break;
		case 1:
			/* Battery */
			...
			chip->mv[tmp] = CAP_VPDO_VOLTAGE(pdo) * 50;
			chip->ma[tmp] = CAP_VPDO_CURRENT(pdo) * 10;
			break;
		case PDO_TYPE_APDO:
			if(APDO_TYPE(pdo) == APDO_TYPE_PPS){
				printk("-------caps surport pps----------\n");
				chip->p_ma = APDO_PPS_EXTRACT_CURR(pdo);
				chip->p_vmin = APDO_PPS_EXTRACT_MIN_VOLT(pdo);
				chip->p_vmax = APDO_PPS_EXTRACT_MAX_VOLT(pdo);
				if(chip->p_ma > 0 && chip->p_vmin > 0 && chip->p_vmax > 0){
					chip->notify.is_pps_support = 1;
				}
				printk("-------chip->p_ma=%d  chip->p_vmin=%d chip->p_vmax=%d----------\n", chip->p_ma, chip->p_vmin, chip->p_vmax);
			}
			break;
		default:
			/* not meet battery caps */
...
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //weibu 20230712
...
#if IS_ENABLED(CONFIG_TCPC_HUSB311)
	chip_id = get_fusb302_chipid(chip) >> 4;
	dev_err(&client->dev, "chip_id = 0x%x\n",chip_id);
	if(chip_id != 0x9){
		fusb302_probe_faile = true;
		return 0;
	}
#endif
...
#if IS_ENABLED(CONFIG_CM_HARDWAREINFO_SUPPORT)
	Hwinfo_update_info_cust(HW_TYPE_CC, "fusb302");
#endif
...
#if IS_ENABLED(CONFIG_FUSB302_SHUTDOWN_DIS_CC)
#else
	if(chip->notify.is_pd_connected){
		tcpm_set_cc_pull_mode(chip, CC_PULL_NONE);
		regmap_update_bits(chip->regmap, FUSB_REG_CONTROL2,
