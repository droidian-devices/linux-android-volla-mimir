...
struct fusb30x_chip *g_chip;
#ifdef pr_fmt
#undef pr_fmt
#endif
#define pr_fmt(fmt) \
    KBUILD_MODNAME "_Leo: [%s:%d] " fmt, __func__, __LINE__
...
static void dump_notify_info(struct fusb30x_chip *chip)
{
	...
	dev_dbg(chip->dev, "pps         %d\n", chip->notify.is_pps_support);
...
static void dpm_extract_pdo_info(
			uint32_t pdo, struct dpm_pdo_info_t *info)
{
...
void dump_pdo_info(struct fusb30x_chip *chip) 
{
	int i;
	struct dpm_pdo_info_t *info = chip->pdo_info;

	for(i = 0; i < g_chip->nr; i++) {
		switch (PDO_TYPE(info[i].type << 30)) {
		case PDO_TYPE_FIXED:
			pr_info("PDO_TYPE_FIXED: index:%d  ma:%d vmax:%d uw:%d \n",
					i,info[i].ma,info[i].vmax,info[i].uw);
			break;
		case PDO_TYPE_VARIABLE:
			pr_info("PDO_TYPE_VARIABLE: index:%d  ma:%d vmin:%d vmax:%d uw:%d \n",
					i,info[i].ma,info[i].vmin,info[i].vmax,info[i].uw);
			break;
		case PDO_TYPE_BATTERY:
			pr_info("PDO_TYPE_BATTERY: index:%d  ma:%d vmin:%d vmax:%d uw:%d \n",
					i,info[i].ma,info[i].vmin,info[i].vmax,info[i].uw);
			break;
		case PDO_TYPE_APDO:
			pr_info("PDO_TYPE_APDO: index:%d ma:%d vmin:%d vmax:%d uw:%d pwr_limit:0x%X apdo_type:0x%X\n",
					i, info[i].ma,info[i].vmin,info[i].vmax,info[i].uw,info[i].pwr_limit,info[i].apdo_type);
			break;
		}
	}
}
...
void cust_record_pdo_info(struct fusb30x_chip *chip)
{
	int i;
	uint32_t pdo;

	memcpy(chip->apdo_cap, chip->rec_load, chip->nr * sizeof(chip->rec_load[0]));

	for (i = 0; i < chip->nr; i++) {
		pdo = chip->apdo_cap[i];
		dpm_extract_pdo_info(pdo, &chip->pdo_info[i]);
	}
			...
			uint8_t *cap_i, struct tcpm_power_cap_val *cap_val)
{
	...
	memcpy(cap.pdos, g_chip->apdo_cap, g_chip->nr * sizeof(g_chip->apdo_cap[0]));
...
EXPORT_SYMBOL(fusb302_inquire_pd_source_apdo);

int fusb302_dpm_pd_request(int mv, int ma)
{
	...
	pr_info("Leo debug g_chip->nr:%d \n",g_chip->nr);
	mutex_lock(&g_chip->pd_charger_lock);

	if (g_chip->nr == 0)
		return -ENODEV;

	if (PDO_TYPE(g_chip->apdo_cap[g_chip->nr - 1]) == PDO_TYPE_APDO) {
		g_chip->pos_power = g_chip->nr;
		cap_idx = g_chip->pos_power - 1;
		ret = fusb302_inquire_pd_source_apdo(
						TCPM_POWER_CAP_APDO_TYPE_PPS,
						&cap_idx, &apdo_cap);
		if (ret != TCP_DPM_RET_SUCCESS) {
			pr_err("PDO_TYPE inquire pd apdo fail(%d)\n", ret);
			mutex_unlock(&g_chip->pd_charger_lock);
			return -ENODEV;
		}
	} else {
		cap_idx = 0;
	...
	if (ma > apdo_cap.ma) {
		pr_err("ma(%dma) > apdo_cap.ma(%dma) \n",ma,apdo_cap.ma);
		mutex_unlock(&g_chip->pd_charger_lock);
		return -ENODEV;
	}

	if (mv < apdo_cap.min_mv || mv > apdo_cap.max_mv) {
		pr_err("mv(%dmv) out of pps range apdo_cap.min_mv(%dmv) ~ apdo_cap.max_mv(%dmv)\n",
				mv,apdo_cap.min_mv,apdo_cap.max_mv);
		mutex_unlock(&g_chip->pd_charger_lock);
		return -ENODEV;
	}

	g_chip->pd_output_vol = mv;
	g_chip->pd_output_cur = ma;
	set_state(g_chip,policy_snk_select_cap);
	state_machine_typec(g_chip);
	mutex_unlock(&g_chip->pd_charger_lock);

	return TCP_DPM_RET_SUCCESS;
}
EXPORT_SYMBOL(fusb302_dpm_pd_request);

int fusb302_set_apdo_charging_policy(uint8_t policy, int mv, int ma)
{
	return fusb302_dpm_pd_request(mv, ma);
}
EXPORT_SYMBOL(fusb302_set_apdo_charging_policy);

static void fusb302_set_attach_state(struct fusb30x_chip *chip, enum typec_attach_type attach_type)
{
	chip->typec_attach_old = chip->typec_attach_new;
	chip->typec_attach_new = attach_type;
}

uint8_t fusb302_inquire_typec_attach_state(void)
{
	return (g_chip->notify.is_cc_connected ? g_chip->typec_attach_new : TCPM_ERROR_UNATTACHED);
}
EXPORT_SYMBOL(fusb302_inquire_typec_attach_state);
void fusb32_get_pps_status(void)
{
	mutex_lock(&g_chip->pd_charger_lock);
	set_state(g_chip, policy_snk_get_pps_status);
	state_machine_typec(g_chip);
	mutex_unlock(&g_chip->pd_charger_lock);
}
EXPORT_SYMBOL(fusb32_get_pps_status);
#endif //Leo 20230530

int get_uisoc(struct fusb30x_chip *chip)
{
	union power_supply_propval prop;
	struct power_supply *bat_psy = NULL;
	int ret;

	bat_psy = chip->bat_psy;

	if (bat_psy == NULL || IS_ERR(bat_psy)) {
		pr_info("%s retry to get bat_psy\n", __func__);
		bat_psy = devm_power_supply_get_by_phandle(chip->dev, "gauge");
		chip->bat_psy = bat_psy;
	}

	if (bat_psy == NULL || IS_ERR(bat_psy)) {
		pr_info("%s Couldn't get bat_psy\n", __func__);
		ret = 100;
	} else {
...
int fusb302_get_vbat(struct fusb30x_chip *chip)
{
	union power_supply_propval prop;
	struct power_supply *bat_psy = NULL;
	int ret;
...
#if PRINTK_LOG //Leo 20230530
	for (i = PD_HEADER_CNT(chip->rec_head) - 1; i >= 0; i--) {
		pr_info(" dump all cap:-----chip->rec_load[%d]=0x:%X-------\n",i, chip->rec_load[i]);
	}
#endif

	for (i = PD_HEADER_CNT(chip->rec_head) - 1; i >= 0; i--) {
#if PRINTK_LOG
		printk("-----chip->rec_load[%d]=0x:%X-------\n",i, chip->rec_load[i]);
#endif
		switch (PDO_TYPE(chip->rec_load[i])) {
		case PDO_TYPE_FIXED:
			/* Fixed Supply */
			if ((CAP_FPDO_VOLTAGE(chip->rec_load[i]) * 50) <=
				...
				pos_find = 1;
				printk("PDO_TYPE_FIXED ----pd_output_vol=%d  pd_output_cur=%d----\n", chip->pd_output_vol, chip->pd_output_cur);
			}
			break;
		case PDO_TYPE_BATTERY:
			/* Battery */
			...
			break;
		case PDO_TYPE_VARIABLE:
				pr_err("PDO_TYPE_VARIABLE not achieved \n");
			break;
		case PDO_TYPE_APDO:
			if (max_vol <= (APDO_PPS_EXTRACT_MAX_VOLT(chip->rec_load[i])) &&
				(max_cur <= APDO_PPS_EXTRACT_CURR(chip->rec_load[i]))) {
				chip->pos_power = i + 1;
				chip->pd_output_vol = max_vol;
				chip->pd_output_cur = max_cur;
				pr_info(" PDO_TYPE_APDO :pd_output_vol=%d  pd_output_cur=%d \n", chip->pd_output_vol, chip->pd_output_cur);
				pos_find = 1;
			}
			break;
		default:
...
{
	int max_vol = chip->max_vol, max_cur = chip->max_cur;
	int set_cap_vol, vbat = 0;

	if(chip->selected_cap_idx >= 0 && chip->nr >= 0){
		max_vol = chip->mv[chip->selected_cap_idx];
		max_cur = chip->ma[chip->selected_cap_idx];
	}

#if 1 //Leo 20230608
	if (chip->notify.is_pps_support == true) {
		vbat = fusb302_get_vbat(chip);
		set_cap_vol = vbat * 2;
	...
	if(get_uisoc(chip) >= chip->fast_stop_soc){
		if(chip->selected_cap_idx >= 0 && chip->nr >= 0){
			max_vol = chip->mv[0];
			max_cur = chip->ma[0];
			chip->pd_output_vol = max_vol;
	...
	printk("fusb302 max_vol==%d  max_cur==%d  chip->selected_cap_idx=%d\n",chip->max_vol,chip->max_cur,chip->selected_cap_idx);

	if (max_vol > 0 && max_cur > 0)
		fusb_set_pos_power(chip, chip->pd_output_vol, chip->pd_output_cur);
...
static void platform_fusb_notify(struct fusb30x_chip *chip)
{
	...
	if (memcmp(&chip->notify, &chip->notify_cmp,
		...
		if (chip->notify.power_role == POWER_ROLE_SINK &&
			...
			if(chip->notify.is_pps_support){
				property.intval = (chip->pps_vmin << 15 | chip->pps_vmax);
				extcon_set_state(chip->extcon, EXTCON_CHG_USB_PD, true);
				extcon_set_property(chip->extcon, EXTCON_CHG_USB_PD,
					    EXTCON_PROP_USB_TYPEC_POLARITY,
					    property);
				extcon_sync(chip->extcon, EXTCON_CHG_USB_PD);

				property.intval = chip->pps_ma;
				extcon_set_state(chip->extcon, EXTCON_DISP_DP, true);
				extcon_set_property(chip->extcon, EXTCON_DISP_DP,
					    EXTCON_PROP_USB_TYPEC_POLARITY,
					    property);
				extcon_sync(chip->extcon, EXTCON_DISP_DP);
					 ...
					 int vbus_other)
{
	...
	if (chip->gpio_iddig) {
		dev_info(chip->dev, "fusb302 send gpio_iddig to %s vbus 5v\n", vbus_5v ? "enable" : "disable");
		gpiod_set_value(chip->gpio_iddig, vbus_5v);
	}
...
#if PRINTK_LOG
	printk("<<< Leo 111 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x>>>\n",
...
static void mux_alert(struct fusb30x_chip *chip, u32 *evt)
{
	...
	if (!chip->timer_state) { //Leo 20230614
...
static void set_state_unattached(struct fusb30x_chip *chip)
{
	...
	if (chip->notify.is_cc_connected &&
		...
		//Audio
#if IS_ENABLED(CONFIG_WB_TYPEC_EARPIECE) //jnier add 20230512
		dev_info(chip->dev, "CC connection has disconnected as Audio\n");
	...
	if (chip->gpio_iddig)
		gpiod_set_value(chip->gpio_iddig, 0);
...
static void set_mesg(struct fusb30x_chip *chip, int cmd, int is_DMT)
{
	...
	if (is_DMT) {
		...
		case DMT_REQUEST:
			...
			switch (PDO_TYPE(chip->rec_load[chip->pos_power - 1])) {
				pr_info("PDO_TYPE(chip->rec_load[chip->pos_power - 1]): %x \n",PDO_TYPE(chip->rec_load[chip->pos_power - 1]));
			case PDO_TYPE_FIXED:
				/* Fixed Supply */
				//chip->send_load[0] |= ((CAP_FPDO_VOLTAGE(chip->rec_load[chip->pos_power - 1]) << 10) & 0x3ff);
				chip->send_load[0] |= (CAP_FPDO_CURRENT(chip->rec_load[chip->pos_power - 1]) & 0x3ff) << 10;
			...
			case PDO_TYPE_APDO:
				/*SOP:0x1082*/
				/*data object: 0x6102583C 6V3A*/
				chip->send_head = (chip->send_head & (~(0x3 << 6))); //clean bits [7:6]:Specification Revision
				chip->send_head |= ((0x02 & 0x3) << 6); //use PD3.0
				...
				chip->send_load[0] = (chip->pos_power << 28) |
							...
							(0 << 25) |
							(1 << 24);//NO USB SUSPEND;
				chip->send_load[0] |= APDO_PPS_OBJ_POS_RAW(chip->pos_power);
				chip->send_load[0] |= APDO_PPS_SET_VOLT_RAW(chip->pd_output_vol);
				chip->send_load[0] |= APDO_PPS_SET_CURR_RAW(chip->pd_output_cur);
				pr_info("chip->send_head:0x%X chip->send_load[0]:0x%X PDO_TYPE_APDO vol:0x%X cur:0x%X \n",
					chip->send_head,chip->send_load[0],APDO_PPS_SET_VOLT_RAW(chip->pd_output_vol),APDO_PPS_SET_CURR_RAW(chip->pd_output_cur));
#endif
				break;
			default:
				/* not meet battery caps */
		...
		if (cmd >= 14) { //Leo add for pd3.0 20230614 
			chip->send_head = (chip->send_head & (~(0x3 << 6))); //clean bits [7:6]:Specification Revision
			chip->send_head |= ((0x02 & 0x3) << 6); 
			chip->send_head |= (cmd & 0x1f);
			pr_info("PPS_STATUS chip->send_head:0x%X!\n",chip->send_head);
		} else {
...
#if 1 //Leo 20230610
	chip->nr = 0;
	memset(chip->apdo_cap, 0x00, sizeof(chip->apdo_cap));
	memset(chip->pdo_info, 0x00, sizeof(chip->pdo_info));
#endif
					     ...
					     u32 evt)
{
	...
	case 2:
		if (evt & EVENT_TIMER_STATE) {
			...
			wake_up_interruptible(&chip->pd_task_que);
			platform_set_vbus_lvl_enable(chip, 1, 0);
...
static void fusb_state_snk_evaluate_caps(struct fusb30x_chip *chip, u32 evt)
{
	...
	chip->nr = PD_HEADER_CNT(chip->rec_head);
	pr_info(" chip->nr:%d \n",chip->nr);

	cust_record_pdo_info(chip);

	for (tmp = 0; tmp < PD_HEADER_CNT(chip->rec_head); tmp++) {
		pdo = chip->rec_load[tmp];
		pr_info("Leo PDO_TYPE(pdo):0x%X chip->rec_load[%d]:0x%X \n",PDO_TYPE(pdo),tmp,chip->rec_load[tmp]);
		switch (PDO_TYPE(pdo)) {
		case PDO_TYPE_FIXED:
			/* Fixed Supply */
			if (CAP_FPDO_VOLTAGE(pdo) <= 100)
		...
		case PDO_TYPE_VARIABLE:
			...
			break;
		case PDO_TYPE_APDO:
			if(APDO_TYPE(pdo) == APDO_TYPE_PPS){
				printk("-------caps surport pps----------\n");
				chip->pps_ma = APDO_PPS_EXTRACT_CURR(pdo);
				chip->pps_vmin = APDO_PPS_EXTRACT_MIN_VOLT(pdo);
				chip->pps_vmax = APDO_PPS_EXTRACT_MAX_VOLT(pdo);
				if(chip->pps_ma > 0 && chip->pps_vmin > 0 && chip->pps_vmax > 0){
					chip->notify.is_pps_support = 1;
					chip->pos_power = tmp + 1;
				}
				pr_info(" Leo -------chip->pps_ma=%d  chip->pps_vmin=%d chip->pps_vmax=%d----------\n", chip->pps_ma, chip->pps_vmin, chip->pps_vmax);
			}
			break;
		default:
			/* not meet battery caps */
...
static void fusb_state_snk_select_cap(struct fusb30x_chip *chip, u32 evt)
{
	...
	case 1:
		...
		if (tmp == tx_success) {
			pr_info("fusb_state_snk_select_cap tmp == tx_success \n");
			chip->timer_state = T_SENDER_RESPONSE;
			fusb_timer_start(&chip->timer_state_machine,
					 chip->timer_state);
			chip->sub_state++;
	...
	default:
		if (evt & EVENT_RX) {
			...
			pr_info("Leo status EVENT_TIMER_STATE policy_snk_send_hardrst \n");
			set_state(chip, policy_snk_send_hardrst);
		}
		break;
	}
...
#if 1//Leo add for test 20230614
static void fusb_state_snk_get_pps_status(struct fusb30x_chip *chip, u32 evt)
{
	u32 tmp;

	switch (chip->sub_state) {
	case 0:
		set_mesg(chip, CMT_GET_PPS_STATUS, CONTROLMESSAGE);
		chip->tx_state = tx_idle;
		chip->sub_state++;
		/* without break */
	case 1:
		...
		if (tmp == tx_success) {
			...
			//set_state(chip, policy_src_send_hardrst);
				pr_info("PPS_STATUS tmp == tx_failed!\n");
		}
	...
	default:
		if (evt & EVENT_RX) {
				pr_info("PPS_STATUS EVENT_RX! head:0x%X \n",chip->rec_head);
			if (PACKET_IS_DATA_MSG(chip->rec_head, EMT_PPS_STATUS)) {
				chip->timer_state = T_DISABLED;
				pr_info("PPS_STATUS PACKET_IS_DATA_MSG!\n");
			}
		} else if (evt & EVENT_TIMER_STATE) {
			//set_state(chip, policy_src_send_hardrst);
			pr_info("PPS_STATUS EVENT_TIMER_STATE!\n");
		}
		break;
	}
}
...
static void state_machine_typec(struct fusb30x_chip *chip)
{
	...
	pr_info("Leo %s \n", connection_state_chr[chip->conn_state]);
	...
	case unattached:
#if 1 //Leo 20230612
		fusb302_set_attach_state(chip, TYPEC_UNATTACHED);
#endif
...
#if 1 //Leo 20230612
		fusb302_set_attach_state(chip, TYPEC_ATTACHED_SRC);
#endif
		fusb_state_attached_source(chip, evt);
		break;
	case attached_sink: //Leo 20230530
#if 1 //Leo 20230612
		fusb302_set_attach_state(chip, TYPEC_ATTACHED_SNK);
#endif
...
#if 1 //Leo 20230612
		fusb302_set_attach_state(chip, TYPEC_ATTACHED_AUDIO);
#endif
	...
	case policy_snk_softrst:
		...
		break;
#if 1 //Leo 20230614
	case policy_snk_get_pps_status:
		fusb_state_snk_get_pps_status(chip, evt);
		break;
#endif
...
static int fusb_initialize_gpio(struct fusb30x_chip *chip)
{
	...
	/* some board support vbus with other ways */
	chip->gpio_iddig = devm_gpiod_get_optional(chip->dev, "iddig-c",
						     GPIOD_OUT_LOW);
	if (IS_ERR(chip->gpio_iddig))
		dev_warn(chip->dev,
			 "Could not get named GPIO for iddig_control!\n");
	else
		gpiod_set_value(chip->gpio_iddig, 0);
	...
	ret = of_property_read_u32(chip->dev->of_node, "tcpc,max_vol", &chip->max_vol);
    if (ret) {
		chip->max_vol = 5000;
        dev_warn(chip->dev, "failed to fusb302 tcpc,max_vol=%d\n",chip->max_vol);
        //return ret;
    }

    ret = of_property_read_u32(chip->dev->of_node, "tcpc,max_cur", &chip->max_cur);
    if (ret) {
		chip->max_vol = 3000;
        dev_warn(chip->dev,"failed to fusb302 tcpc,max_cur=%d\n",chip->max_cur);
        //return ret;
    }

	ret = of_property_read_u32(chip->dev->of_node, "fast_stop_soc", &chip->fast_stop_soc);
    if (ret) {
        dev_warn(chip->dev,"failed to get fusb302 fast_stop_soc\n");
        chip->fast_stop_soc = 90;
        //return ret;
    }

    ret = of_property_read_u32(chip->dev->of_node, "tcpc,selected_cap_idx", &chip->selected_cap_idx);
    if (ret) {
		chip->selected_cap_idx = -1;
        dev_warn(chip->dev,"failed to fusb302 tcpc,selected_cap_idx=%d\n",chip->selected_cap_idx);
    }

#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT)
	if (csci_exist("fusb302_fast_stop_soc")) {
		if (csci_integer("fusb302_fast_stop_soc", 0) > 0) {
			chip->fast_stop_soc = csci_integer("fusb302_fast_stop_soc", 0);
			printk("chip->fast_stop_soc=%d\n",chip->fast_stop_soc);
		}
	}

	if (csci_exist("fusb302_selected_cap_idx")) {
		if (csci_integer("fusb302_selected_cap_idx", 0) >= 0) {
			chip->selected_cap_idx = csci_integer("fusb302_selected_cap_idx", 0);
			printk("chip->selected_cap_idx=%d\n",chip->selected_cap_idx);
		}
	}
#endif
...
static void request_irq_work_fuction(struct work_struct *work)
{
	struct fusb30x_chip *chip;
	int ret;

	chip = (struct fusb30x_chip *) container_of(work, struct fusb30x_chip, request_irq_work.work);
	...
	ret = devm_request_threaded_irq(chip->dev,
					...
					chip);
	if (ret) {
		printk("fusb302 irq request failed\n");
	}
}

//jnier add 20230509 >>>>>
static ssize_t fusb302_pd_status_show(struct device_driver *ddri, char *buf)
{
	ssize_t res = 0;

	res = snprintf(buf, PAGE_SIZE, "%d\n",fusb30x_port_info[0]->notify.is_pd_connected);

	return res;
}

#if 0 
static ssize_t fusb302_pd_status_store(struct device_driver *ddri, const char *buf, size_t count)
{
	return count;
}
#endif

static ssize_t debug_info_show(struct device_driver *ddri, char *buf)
{
	int i,len = 0;
	struct dpm_pdo_info_t *info = g_chip->pdo_info;

	//g_chip->nr
	len += snprintf(buf+len, PAGE_SIZE-len, "g_chip->nr:%d\n",g_chip->nr);

	//apdo_cap
	for (i = 0; i < g_chip->nr; i++) {
		len += snprintf(buf+len, PAGE_SIZE-len, "g_chip->apdo_cap:0x%X \n",g_chip->apdo_cap[i]);
	}
	...
	for(i = 0; i < g_chip->nr; i++) {
		switch (PDO_TYPE(info[i].type << 30)) {
		case PDO_TYPE_FIXED:
			len += snprintf(buf+len, PAGE_SIZE-len,"PDO_TYPE_FIXED: index:%d  ma:%d vmax:%d uw:%d \n",
					i,info[i].ma,info[i].vmax,info[i].uw);
			break;
		case PDO_TYPE_VARIABLE:
			len += snprintf(buf+len, PAGE_SIZE-len,"PDO_TYPE_VARIABLE: index:%d  ma:%d vmin:%d vmax:%d uw:%d \n",
					i,info[i].ma,info[i].vmin,info[i].vmax,info[i].uw);
			break;
		case PDO_TYPE_BATTERY:
			len += snprintf(buf+len, PAGE_SIZE-len,"PDO_TYPE_BATTERY: index:%d  ma:%d vmin:%d vmax:%d uw:%d \n",
					i,info[i].ma,info[i].vmin,info[i].vmax,info[i].uw);
			break;
		case PDO_TYPE_APDO:
			len += snprintf(buf+len, PAGE_SIZE-len,"PDO_TYPE_APDO: index:%d ma:%d vmin:%d vmax:%d"
				" uw:%d pwr_limit:0x%X apdo_type:0x%X\n",i, info[i].ma,info[i].vmin,info[i].vmax,info[i].uw,info[i].pwr_limit,info[i].apdo_type);
			break;
		}
	}
	
	//fusb302 info
	len += snprintf(buf+len, PAGE_SIZE-len, "try_role_complete:%d is_cc_connected:%d power_role:%d data_role:%d \n",
				g_chip->try_role_complete, g_chip->notify.is_cc_connected, g_chip->notify.power_role, g_chip->notify.data_role);

	return len;
}

static ssize_t debug_info_store(struct device_driver *ddri, const char *buf, size_t count)
{
	int cmd = 0;
	int num = 0;

	if (2 == sscanf(buf,"%d %d",&cmd, &num)) {
	
	pr_info("Leo cmd:%d vol:%d rec_head:0x%X\n",cmd, num,g_chip->rec_head);

		if (cmd == 1) {
			g_chip->pos_power = 6;
			g_chip->pd_output_vol = num;
			g_chip->pd_output_cur = 3000;
			set_state(g_chip,policy_snk_select_cap);
			state_machine_typec(g_chip);
		} else if (cmd == 2) {
			struct tcpm_power_cap_val apdo_cap = {};
			...
			do {
				ret = fusb302_dpm_pd_request(num, 3000);
				cnt++;
			...
			pr_info("fusb302_inquire_typec_attach_state :%d \n" ,fusb302_inquire_typec_attach_state());
		} else if (cmd == 5) {
			set_state(g_chip, num);
			state_machine_typec(g_chip);
		}
	}
...
static DRIVER_ATTR_RO(fusb302_pd_status);
static DRIVER_ATTR_RW(debug_info);

static struct driver_attribute *fusb302_attr_list[] = {
	&driver_attr_fusb302_pd_status,	/*chip information */
	&driver_attr_debug_info,	
};

static int fusb302_create_attr(struct device_driver *driver)
{
	int idx, err = 0;
	int num = (int)(ARRAY_SIZE(fusb302_attr_list));

	if (driver == NULL)
		return -EINVAL;

	for (idx = 0; idx < num; idx++) {
		err = driver_create_file(driver, fusb302_attr_list[idx]);
		if (err != 0) {
			pr_info("driver_create_file (%s) = %d\n",
				fusb302_attr_list[idx]->attr.name, err);
			break;
		}
	}
	return err;
...
//jnier add 20230509 <<<<<<
#if 0
static int pd_charger_routine_thread(void *arg)
{
	struct fusb30x_chip *chip = arg;
	union extcon_property_value property;
	int ret;

	while (1) {
		ret = wait_event_interruptible(chip->pd_task_que, chip->notify.is_pd_connected);
		...
		mutex_lock(&chip->pd_charger_lock);

		if (chip->notify.is_pps_support == true) {
			if (PDO_TYPE(chip->rec_load[chip->nr]) == PDO_TYPE_APDO)
				chip->pos_power = chip->nr;
	
			//chip->pd_output_vol = fusb302_get_vbat(chip) * 2;
			//chip->pd_output_cur = 3000;
			set_state(chip,policy_snk_select_cap);
			state_machine_typec(chip);
		}
	

		if(get_uisoc(chip) >= chip->fast_stop_soc && chip->pd_output_vol >= 9000){
			chip->pos_power = 1;
			chip->pd_output_vol = 5000;
			chip->pd_output_cur = 3000;
			set_state(chip,policy_snk_select_cap);
			state_machine_typec(chip);
			...
			pr_info("%s: chip->fast_stop_soc=%d\n",__func__, chip->fast_stop_soc);
		}

		mutex_unlock(&chip->pd_charger_lock);

		pr_info("%s: fusb302_get_uisoc=%d  is_pd_connected=%d\n",__func__, get_uisoc(chip), chip->notify.is_pd_connected);
	}
			 ...
			 const struct i2c_device_id *id)
{
	...
	dev_err(&client->dev, "chip->port_num = %d \n",chip->port_num);
	...
	//Leo add for default 20230531
	chip->pd_output_vol = 8000;
	chip->pd_output_cur = 3000;
	...
	ret = extcon_set_property_capability(chip->extcon, EXTCON_CHG_USB_PD,
					     EXTCON_PROP_USB_TYPEC_POLARITY);
	if (ret) {
		dev_err(&client->dev,
			"failed to set EXTCON_CHG_USB_PD property capability: %d\n", ret);
		return ret;
	}
	...
	// 	goto IRQ_ERR;
	// }
	chip->fusb302_irq_wq = create_singlethread_workqueue("fusb302_irq_wq");
	INIT_DELAYED_WORK(&chip->request_irq_work, request_irq_work_fuction);
	queue_delayed_work(chip->fusb302_irq_wq, &chip->request_irq_work, msecs_to_jiffies(2000));
	...
	input_set_capability(chip->input, EV_SW, SW_MICROPHONE_INSERT);
	...
	//jnier add 
    ret = fusb302_create_attr(chip->client->dev.driver);
    if (ret)
        dev_err(chip->dev, "fusb302_create_attr to failed : %d\n", ret);	

    chip->bat_psy = devm_power_supply_get_by_phandle(chip->dev, "gauge");
    //kthread_run(pd_charger_routine_thread, chip, "pd_charger_thread");
    init_waitqueue_head(&chip->pd_task_que);
    mutex_init(&chip->pd_charger_lock);
...
static void fusb30x_shutdown(struct i2c_client *client)
{
	...
	dev_info(chip->dev, "---------fusb30x_shutdown---------\n");
	if (chip->gpio_vbus_5v)
		gpiod_set_value(chip->gpio_vbus_5v, 0);
	if (chip->gpio_discharge) {
		gpiod_set_value(chip->gpio_discharge, 1);
