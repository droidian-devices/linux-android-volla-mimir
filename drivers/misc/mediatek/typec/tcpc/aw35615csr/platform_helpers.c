...
#if defined(WB_PINCTRL_SUPPORT) //Leo 2024308
	chip->pinctrl = devm_pinctrl_get(&chip->client->dev);
	if (IS_ERR(chip->pinctrl)) {
		ret = PTR_ERR(chip->pinctrl);
		dev_info(&chip->client->dev, "Cannot find chip->pinctrl!\n");
		return ret;
	}

	chip->int_pullup = pinctrl_lookup_state(chip->pinctrl, "int_n_pullup");
	if (IS_ERR(chip->int_pullup)) {
		ret = PTR_ERR(chip->int_pullup);
		dev_info(&chip->client->dev,"Cannot find pinctrl chip->int_pullup %d!\n", ret);
		return ret;
	}

	pinctrl_select_state(chip->pinctrl, chip->int_pullup);
	dev_info(&chip->client->dev,"select chip->int_pullup ! \n");
#endif
...
AW_BOOL aw_IsDeviceValid(struct aw35615_chip *chip)
{
	...
	for (i = 0; i <= 5; i++) {
		...
		if (ret < 0) {
			...
			id_ret = AW_TRUE;
			chip->chip_id = 0x344f;
			aw35615_setchip_id(chip->chip_id);//jnier add 20240306
			break;
		}
	}
