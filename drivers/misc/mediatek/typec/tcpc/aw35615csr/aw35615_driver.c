...
static struct i2c_client *g_aw35615_client = NULL;
/******************************************************************************
 * Driver functions
 ******************************************************************************/
...
#if IS_ENABLED(CONFIG_CM_HARDWAREINFO_SUPPORT)
	Hwinfo_update_info_cust(HW_TYPE_CC, "aw35615");
#endif
    g_aw35615_client = client;
...
static void aw35615_shutdown(struct i2c_client *client)
{
	...
	printk("cch-->  aw35615_shutdown:aw35615 device shutdown!!!\n");
}
static void aw35615_pmu_core_syscore_shutdown(void)
{
	if (!g_aw35615_client) {
		pr_err("%s: is NULL\n", __func__);
		return;
	}
	pr_err("%s cch--> aw35615 shutdown start!!\n", __func__);
	aw35615_shutdown(g_aw35615_client);
}
...
struct syscore_ops aw35615_pmu_core_syscore_ops = {
	.shutdown = aw35615_pmu_core_syscore_shutdown,
};
static int __init aw35615_init(void)
{
	...
	register_syscore_ops(&aw35615_pmu_core_syscore_ops);
...
static void __exit aw35615_exit(void)
{
	...
	unregister_syscore_ops(&aw35615_pmu_core_syscore_ops);
