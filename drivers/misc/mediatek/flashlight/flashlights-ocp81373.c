...
#if defined(M126TBUR100_RMX_1281)
#define WB_TEST //Leo 20240412
#endif
...
static const unsigned char aw36514_torch_level[AW36514_LEVEL_NUM] = {
	0x19, 0x3C, 0x5B, 0x7A, 0x99, 0xB8, 0xD7, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00};//x*1.51+0.75
...
static const unsigned char aw36514_flash_level[AW36514_LEVEL_NUM] = {
	...
	0xF7, 0xFF};						//x*.5.87+2.94
static const unsigned char ocp81373_torch_level[AW36514_LEVEL_NUM] = {
	0x0B, 0x19, 0x2B, 0x3B, 0x4B, 0x5B, 0x6B, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00};//y*2.92+2.92

static const unsigned char ocp81373_flash_level[AW36514_LEVEL_NUM] = {
	0x02, 0x06, 0x0A, 0x0E, 0x12, 0x16, 0x1A, 0x1E, 0x22, 0x26,
	0x2A, 0x2E, 0x32, 0x36, 0x3A, 0x3E, 0X42, 0x46, 0x4A, 0x4E,
	0x52, 0x56, 0x5A, 0x5E, 0x62, 0x66, 0x6A, 0x6E, 0x72, 0x76,
	0x7A, 0x7E};						//y*11.72+11.72
...
static void aw36514_soft_reset(void)
{
	int ret;
	unsigned char cnt = 0;
	unsigned char reg_val = 0, reg_val1;

	aw36514_i2c_write(aw36514_i2c_client, AW36514_REG_TIMING_CONF, 0x1b); //Leo add for judge softrst 20240407
	aw36514_i2c_read(aw36514_i2c_client, AW36514_REG_TIMING_CONF, &reg_val1);
	pr_info("%s aw36514 reg:0x%x value:0x%x \n",__func__,AW36514_REG_TIMING_CONF, reg_val1);
	...
	while (cnt < AW_I2C_RETRIES) {
		ret = i2c_smbus_write_byte_data(aw36514_i2c_client, AW36514_REG_BOOST_CONFIG, reg_val);
		if (ret < 0) {
			msleep(5);
			aw36514_i2c_read(aw36514_i2c_client, AW36514_REG_TIMING_CONF, &reg_val1);
			if (reg_val1 == 0x1a) {
				pr_info("aw36514 softrst successful \n");
				break;
			}
			pr_info("%s: i2c_write addr=0x%02X, data=0x%02X, cnt=%d, error=%d\n",
				   __func__, AW36514_REG_BOOST_CONFIG, reg_val, cnt, ret);
		} else {
			break;
		}
		cnt++;
...
static int aw36514_disable(int channel)
{
...
#if defined(WB_TEST) //Leo 20240412
	unsigned char val = 0;
#endif
	...
	//Leo 20240412
#if defined(WB_TEST) //Leo 20240412
	aw36514_i2c_read(aw36514_i2c_client, 0x0A, &val);
	pr_info("%s : reg:0x0A val:%.2x \n",__func__,val);
#endif
