...
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20240125
...
#if IS_ENABLED(CONFIG_MID_CSCI_SUPPORT) //Leo 20240125
	if (csci_integer("ro.vendor.co_clock_support", 0) == 1) {
		osal_strncat(&(gDevWmt.cWmtcfgName[0]), CUST_CFG_WMT_SOC_CO_CLK, osal_sizeof(CUST_CFG_WMT_SOC_CO_CLK));
	} else {
